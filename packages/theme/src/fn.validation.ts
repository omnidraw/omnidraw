/** Pure validation for complete theme registrations and canvas color codes. */
...
export function fnIsCanvasInkColorCode(
  value: unknown,
): value is TCanvasInkColorCode {
  return typeof value === "string"
    && CANVAS_INK_COLOR_CODES.includes(value as TCanvasInkColorCode);
}

export function fnThemeSrgbColorValid(
  value: unknown,
): value is TThemeSrgbColor {
...
export function fnThemeDefinitionIssues(value: unknown): readonly string[] {
  ...
  for (const role of THEME_UI_COLOR_ROLES) {
    if (
      typeof ui?.[role] !== "string"
      || (ui[role] as string).trim().length === 0
    ) {
  ...
  for (const [path, candidate, roles] of stringGroups) {
    ...
    for (const role of roles) {
      if (
        ...
        || (group[role] as string).trim().length === 0
      ) {
  ...
  const appearances = [
    ...
    },
    {
      ...
      colorRoles: [
        "outline", "anchorFill", "midpointFill", "handleStroke",
      ],
