...
async function fxReadDatabaseChecks(
  portal: TPortal,
  args: TArgs,
): Promise<TDatabaseChecks> {
  void args;
  const integrity = checkValues(
    await (await portal.db.prepare('PRAGMA integrity_check')).all() as Record<string, unknown>[],
    'integrity_check',
  );
  const quick = checkValues(
    await (await portal.db.prepare('PRAGMA quick_check')).all() as Record<string, unknown>[],
    'quick_check',
  );
