...
function ApprovalCard(props: {
  ...
  return (
    <article class="omnidraw-ai-chat-approval" data-status={props.approval.status} aria-label="Protected resource operation">
      <header class="omnidraw-ai-chat-approval__header">
        <div>
          <span>{approvalKindLabel(props.approval.kind)}</span>
          <strong>{props.approval.summary}</strong>
        </div>
        <span class="omnidraw-ai-chat-approval__status">{props.approval.status}</span>
      </header>
      <div class="omnidraw-ai-chat-approval__meta">
      ...
      <Show when={props.approval.reviewerReason}>
        <p class="omnidraw-ai-chat-approval__message">Reviewer: {props.approval.reviewerReason}</p>
      </Show>
      ...
      <Show when={details() && details() !== "{}"}>
        <button type="button" class="omnidraw-ai-chat-approval__details-toggle" onClick={() => setIsExpanded((value) => !value)}>
      ...
      <Show when={props.approval.statusMessage}>
        <p class="omnidraw-ai-chat-approval__message" role={props.approval.status === "failed" || props.approval.status === "stale" ? "alert" : "status"}>
      ...
      <Show when={!props.onOpenResource || props.approval.kind === "resource-delete" ? undefined : props.approval.resourceId}>
        {(resourceId) => (
          <div class="omnidraw-ai-chat-approval__resource-action">
            <button type="button" onClick={() => props.onOpenResource?.(resourceId())}>Open resource</button>
          </div>
        )}
      </Show>
      <Show when={props.approval.status === "pending"}>
        <div class="omnidraw-ai-chat-approval__actions">
          <button type="button" onClick={() => void props.onResolve(props.approval.id, "reject")}>Reject</button>
          <button type="button" class="omnidraw-ai-chat-primary-button" onClick={() => void props.onResolve(props.approval.id, "approve")}>Approve</button>
        </div>
      </Show>
    </article>
  )
...
export function ApprovalList(props: IProps) {
  return (
    <Show when={props.approvals.length > 0}>
      <section class={`omnidraw-ai-chat-approvals omnidraw-ai-chat-approvals--${props.variant ?? "inline"}`} aria-label="Protected operations">
