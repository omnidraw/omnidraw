  ...
  nodes: {
    ...
    mention: {
      ...
      toDOM: (node) => [
        "span",
        {
          class: "omnidraw-ai-chat-composer__mention",
...
const suggestionKey = new PluginKey<TPromptSuggestion | undefined>("omnidraw-ai-chat-composer-suggestion")
...
function getDefaultModel(models: readonly TChatComposerModel[], defaultModel?: string, defaultProvider?: string) {
  ...
  return (
    <Show when={icon()} fallback={<FileText size={15} aria-hidden="true" />}>
      <Show when={isWidget()} fallback={(
        ...
        <AiChatWidgetIcon icon={widgetIcon()} />
      </Show>
    </Show>
  )
}
...
export function ChatComposer(props: TChatComposerProps) {
  ...
  const updateSuggestionMenuMaxHeight = () => {
    ...
    const boundary = root?.closest<HTMLElement>(".omnidraw-ai-chat-tab--chat")
      ?? root?.closest<HTMLElement>(".omnidraw-ai-chat-shell")
  ...
  onMount(() => {
    ...
    const suggestionBoundary = root.closest<HTMLElement>(".omnidraw-ai-chat-tab--chat")
      ?? root.closest<HTMLElement>(".omnidraw-ai-chat-shell")
    ...
    view = new ProseMirrorEditorView(editorRoot, {
      ...
      attributes: {
        ...
        class: "omnidraw-ai-chat-composer__editor",
        role: "combobox",
      },
    })
  ...
  return (
    <section ref={root} class="omnidraw-ai-chat-composer" aria-label="Chat prompt composer">
      <div class="omnidraw-ai-chat-composer__body" onClick={() => view?.focus()}>
        <Show when={shouldShowPlaceholder()}>
          <div class="omnidraw-ai-chat-composer__placeholder">{placeholder()}</div>
        </Show>
        <div ref={editorRoot} class="omnidraw-ai-chat-composer__editor-root" />

        <Show when={images().length > 0}>
          <div class="omnidraw-ai-chat-composer__attachments" aria-label="attached images">
            <For each={images()}>
              {(image) => (
                <div class="omnidraw-ai-chat-composer__attachment">
        ...
        <Show when={suggestion() && suggestions().length > 0}>
          <div
            ...
            class="omnidraw-ai-chat-composer__suggestions"
            role="listbox"
          ...
          >
            <For each={suggestions()}>
              {(item, index) => (
                ...
                >
                  <span class="omnidraw-ai-chat-composer__suggestion-icon">
        ...
        <input
          ref={imageInput}
          class="omnidraw-ai-chat-composer__file-input"
          type="file"
          accept="image/*"
          multiple
          aria-hidden="true"
        ...
        <div class="omnidraw-ai-chat-composer__controls">
          <div class="omnidraw-ai-chat-composer__controls-right">
                        ...
                        <For each={activeProviderModels()}>
                          {(model) => (
                            ...
                            >
                              ...
                              <span class="omnidraw-ai-chat-composer__model-capabilities" aria-label={model.input.includes("image") ? "Text and image input" : "Text input only"}>
                                <FileText size={13} />
            ...
            <button
              class="omnidraw-ai-chat-composer__icon-button"
              type="button"
            ...
            <button
              class="omnidraw-ai-chat-composer__send"
              type="button"
