...
export async function fxDecodeAndVerifyWidgetSourceMap(
  portal: TPortal,
  args: TArgs,
  ...
  if (await portal.digestSha256(bytes) !== args.expectedDigestSha256) {
  ...
  const decodedMaps = maps.map((item) => {
    ...
    const mapBytes = portal.decodeBase64(entry.mapBase64);
