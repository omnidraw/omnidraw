import { SERIALIZED_OPERATION_SCOPES } from './serialized-operation/CONSTANTS';
...
export function txRunSerializedOperation<TResult>(
  ...
  const previous = operationTails.get(portal.scope) ?? Promise.resolve();
  const result = previous.then(run, run);
  const tail = result.then(
    () => undefined,
    () => undefined,
  );
  operationTails.set(portal.scope, tail);
  void tail.then(() => {
    if (operationTails.get(portal.scope) === tail) {
      operationTails.delete(portal.scope);
    }
  });
  return result;
}

export type { TArgs, TPortal };
