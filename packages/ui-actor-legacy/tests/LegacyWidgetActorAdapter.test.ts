...
function createAdapter(
  ...
  browser: { setTimeout(callback: () => void, timeout: number): unknown; clearTimeout(timer: unknown): void } = {
...
describe('LegacyWidgetActorAdapter listener ownership', () => {
  ...
  test('does not let an old reconnect wait start a second listener after restart', async () => {
    ...
    const adapter = createAdapter(actorEvents, { setTimeout, clearTimeout });
