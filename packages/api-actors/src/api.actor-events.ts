...
const apiActorEvents = baseActorsOs.events.handler(async function* ({ input, context }) {
  if (!fxCanViewCanvas({ db: getActorsDb(context.db) }, { canvasId: input.canvasId, accountId: context.accountId })) {
  ...
  yield {
    ...
    instances: fxListActorInstances({ db: getActorsDb(context.db) }, { canvasId: input.canvasId, accountId: context.accountId }),
    connections: fxListActorConnections({ db: getActorsDb(context.db) }, { canvasId: input.canvasId, accountId: context.accountId }),
  };
