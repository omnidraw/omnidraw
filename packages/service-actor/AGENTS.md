...
KV and secret-store files use a host-private format with exact resource ID, kind, and version metadata. Entry values use Turso's native `JSON` type in a `STRICT` table, so malformed JSON is rejected at the storage boundary. Ordinary opens verify that identity and schema and never initialize a missing file. Handles are lazy, bounded, and evicted only while idle; writes serialize per resource.

KV files remain unencrypted format v1. Secret-store files use Turso AEGIS-256 page encryption and format v2. Each resource receives an independent random 32-byte database key stored in the main database's general-purpose `encryption_keys` table. That table has no actor columns; `actor_resource_encryption_keys` owns the resource-to-key link. Legacy plaintext-v1 secret files convert through a lifecycle-locked, verified temporary/recovery copy-and-swap. Never open a known encrypted file without its linked key or recreate an unreadable secret database. Report stable `SECRET_STORE_KEY_UNAVAILABLE` / `SECRET_STORE_DECRYPTION_FAILED` conditions without native paths or errors.

Use only the narrow actor-resource key-persistence interface: atomically create the general key row and separate link or read the linked winner, validate stored key material, and do not expose key listing or serialization through actor, API, SDK, tool, or UI surfaces.

The database-backed design favors simple operation over local ciphertext/key separation. A usable restore needs the main database and corresponding encrypted actor-resource data. Encryption protects a secret database/WAL copied without the main database, but not filenames/directories, plaintext in memory or UI, or an attacker who can read both stores or control the host process. It does not replace database access control, host security, API authorization, or redaction.
