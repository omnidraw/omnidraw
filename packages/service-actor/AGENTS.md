...
KV files remain unencrypted format v1. Secret-store files use Turso AEGIS-256 page encryption and format v2. The 32-byte installation master key lives at `<configRoot>/keys/secret-store-master-key.v1.hex` with owner-only permissions; each resource receives a distinct deterministic HKDF-SHA-256 key using its immutable resource ID. Legacy plaintext-v1 secret files convert through a lifecycle-locked, verified temporary/recovery copy-and-swap. Never open a known encrypted file without its derived key, replace a missing master key, or recreate an unreadable secret database. Report stable `SECRET_STORE_KEY_UNAVAILABLE` / `SECRET_STORE_DECRYPTION_FAILED` conditions without native paths or errors.

Configuration and data roots must remain distinct so the key cannot fall inside the protected data tree. Fail closed when that separation or current-user-only key access cannot be enforced; the current Node host has no Windows ACL implementation for secret-store keys.

Back up the versioned key file and encrypted secret-store directory as one unit. Encryption protects copied database pages, WAL data, and data-only backups, but not filenames/directories, plaintext in memory or UI, or an attacker who can read both configuration and data roots or control the host process. It does not replace filesystem permissions, host security, API authorization, or redaction.
