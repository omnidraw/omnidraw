...
function validateHostId(value: string, label = 'DbResource'): string {
  if (
    ...
    throw new ActorResourceError('DB_RESOURCE_UNAVAILABLE', `${label} has an invalid host identity.`);
  }
  return value;
}
...
function validateIdentifier(value: unknown): string {
  if (
    ...
    throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Database object name is invalid.');
  }
  return value;
}
...
function boundedInspectionSql(value: unknown): string | null {
  ...
  if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') > INSPECTION_SQL_MAX_LENGTH) {
    throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database structure SQL exceeds the inspection limit.');
  }
  return value;
}
...
function boundedActorSql(value: unknown): string {
  if (typeof value !== 'string' || value.trim().length === 0 || value.length > ACTOR_SQL_MAX_LENGTH) {
    throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Database SQL is blank or exceeds the host limit.');
  }
...
function boundedDraftSql(value: unknown): string {
  if (typeof value !== 'string' || value.trim().length === 0 || Buffer.byteLength(value, 'utf8') > DRAFT_SQL_MAX_LENGTH) {
    throw new ActorResourceError('DB_RESOURCE_DRAFT_INVALID', 'Draft SQL is blank or exceeds the host limit.');
  }
  const summary = sqlSummary(value);
  if (summary.statementCount < 1 || summary.statementCount > DRAFT_STATEMENT_MAX_COUNT || hasHostFileSql(summary.tokens)) {
    throw new ActorResourceError('DB_RESOURCE_DRAFT_INVALID', 'Draft SQL is invalid or attempts to control host files.');
  }
  if (summary.tokens.some((token) => token === 'BEGIN' || token === 'COMMIT' || token === 'ROLLBACK' || token === 'SAVEPOINT' || token === 'RELEASE')) {
    throw new ActorResourceError('DB_RESOURCE_DRAFT_INVALID', 'Draft SQL may not control the host apply transaction.');
  }
  return value;
}
...
function assertBoundBytes(values: readonly TDbBindValue[], message: string): void {
  if (values.reduce<number>((total, value) => total + parameterBytes(value), 0) > PARAMETER_BYTES_MAX) {
    throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', message);
  }
}
...
function bindParameters(value: unknown): TDbBindParameters {
  ...
  for (const [name, raw] of Object.entries(value)) {
    ...
    if (bytes > PARAMETER_BYTES_MAX) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Database parameters exceed the size limit.');
...
function bindNamedParameters(requirement: TActorDbResourceRequirement, operationName: string, value: unknown): TDbBindParameters {
  ...
  if (!operation) throw new ActorResourceError('DB_NAMED_OPERATION_UNKNOWN', `Named database operation "${operationName}" is not declared.`);
  const raw = value === undefined ? {} : recordArgs(value);
  const declarations = operation.parameters ?? {};
  for (const name of Object.keys(raw)) {
    if (!(name in declarations)) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Unknown parameter "${name}".`);
  }
  const result: TDbBindParameters = {};
  for (const [name, declaration] of Object.entries(declarations)) {
    const supplied = Object.prototype.hasOwnProperty.call(raw, name) && raw[name] !== undefined;
    if (!supplied) {
      if (declaration.required !== false) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Required parameter "${name}" is missing.`);
      continue;
    }
    ...
    if (!valid) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Parameter "${name}" must have type ${declaration.type}.`);
...
function toWireValue(value: unknown): TDbCellValue {
  ...
  if (typeof value === 'bigint') {
    if (value < SQLITE_INTEGER_MIN || value > SQLITE_INTEGER_MAX) throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned an out-of-range SQLite integer.');
    return { type: 'integer', value: value.toString() };
  }
  if (typeof value === 'number' && Number.isFinite(value)) {
    if (Number.isInteger(value)) {
      if (!Number.isSafeInteger(value)) throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned an unsafe integer value.');
      return { type: 'integer', value: value.toString() };
    }
  ...
  throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned an unsupported SQLite value.');
}

function toWireInteger(value: unknown): Extract<TDbCellValue, { type: 'integer' }> {
  const wire = toWireValue(value);
  if (wire.type !== 'integer') throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned a non-integer rowid.');
  return wire;
}
...
function toSqlBlobPreview(byteLength: unknown, preview: unknown): TDbPreviewCellValue | null {
  ...
  if (!Number.isSafeInteger(numericLength) || numericLength < 0) {
    throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned an invalid BLOB length.');
  }
  const bytes = preview instanceof Uint8Array ? preview : preview instanceof ArrayBuffer ? new Uint8Array(preview) : null;
  if (!bytes) throw new ActorResourceError('DB_QUERY_FAILED', 'Database returned an invalid BLOB preview.');
  return {
    type: 'blobPreview',
...
function fromWireValue(value: TDbCellValue): TDbBindValue {
  if (!isPlainObject(value) || typeof value.type !== 'string') {
    throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Database cell value is invalid.');
  }
  if (value.type === 'null') return null;
  if (value.type === 'integer') {
    if (!/^-?(?:0|[1-9][0-9]*)$/.test(value.value)) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Integer cell value is invalid.');
    try {
      const integer = BigInt(value.value);
      if (integer < SQLITE_INTEGER_MIN || integer > SQLITE_INTEGER_MAX) throw new Error('out of range');
      return integer;
    } catch { throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Integer cell value is outside SQLite integer range.'); }
  }
  ...
  if (value.type === 'blob' && typeof value.base64 === 'string') {
    if (value.base64.length > 1_398_104 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(value.base64)) {
      throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Blob cell value is not bounded canonical base64.');
    }
...
function columnSql(column: TDbColumnDefinition, includePrimaryKey = true): string {
  ...
  if (declaredType && !/^[A-Za-z][A-Za-z0-9_ (),]*$/.test(declaredType)) {
    throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Declared SQLite type is invalid.');
  }
  ...
  if (defaultSql && !/^(?:NULL|TRUE|FALSE|CURRENT_(?:TIME|DATE|TIMESTAMP)|[-+]?(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)|'(?:''|[^'])*'|x'[0-9a-fA-F]*')$/.test(defaultSql)) {
    throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Column default must be a bounded SQLite literal or current-time keyword.');
  }
...
function referentialAction(value: string | undefined): string {
  ...
  if (!['NO ACTION', 'RESTRICT', 'SET NULL', 'SET DEFAULT', 'CASCADE'].includes(action)) {
    throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Foreign-key action is invalid.');
  }
  return action;
}
...
export class DbResource implements IActorResourceProvider {
  ...
  async provision(resource: TActorResource, _args: TActorResourceProviderCreateArgs): Promise<void> {
    ...
    if (resource.kind !== 'db') throw new ActorResourceError('DB_RESOURCE_UNAVAILABLE', 'Resource kind is not db.');
  ...
  async delete(resource: TActorResource): Promise<void> {
    ...
    try {
      ...
      do {
        ...
        before = drafts.length === 100 && last ? { createdAt: last.created_at, id: last.id } : undefined;
  ...
  effect(operation: string, requirement: TActorResourceRequirement, rawArgs: unknown): 'read' | 'write' | null {
    ...
    return requirement.operations?.[name]?.effect ?? (requirement.scope.includes('read') ? 'read' : 'write');
  }
  ...
  async dispatch(context: TActorResolvedResourceCall, operation: string, rawArgs: unknown): Promise<unknown> {
    ...
    if (context.requirement.kind !== 'db') throw new ActorResourceError('DB_RESOURCE_UNAVAILABLE', 'Bound resource is not a DbResource.');
  ...
  async listRows(args: { resourceId: string; object: string; cursor?: TDbRowIdentity | null; limit?: number }): Promise<TDbRowsPage> {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > ROW_PAGE_MAX) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Row page limit must be between 1 and ${ROW_PAGE_MAX}.`);
    return this.#withDatabase(this.#databasePath(resourceId), true, async (database) => {
      ...
      if (visibleColumns.length > RESULT_COLUMN_MAX_COUNT) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database table has too many columns to browse safely.');
  ...
  async getRow(args: { resourceId: string; object: string; identity: TDbRowIdentity }): Promise<TDbRow> {
    ...
    return this.#withDatabase(this.#databasePath(resourceId), true, async (database) => {
      ...
      try {
        ...
        if (error instanceof ActorResourceError && error.code === 'DB_RESULT_LIMIT_EXCEEDED') {
          throw new ActorResourceError('DB_RESOURCE_ROW_TOO_LARGE', `The selected row exceeds the ${ROW_HYDRATION_BYTES_MAX / 1_048_576} MiB hydration limit.`);
        }
        throw error;
      }
      if (nativeRows.length !== 1) throw new ActorResourceError('DB_RESOURCE_ROW_CONFLICT', 'The selected row changed or disappeared before it could be loaded.');
    ...
    const execute = args.approved
      ...
      : (async () => {
        if (!await this.#liveSqlStatementProducesRows(resourceId, sql)) {
          throw new ActorResourceError('DB_LIVE_SQL_APPROVAL_REQUIRED', 'Live database mutations require explicit user approval.');
        }
        return this.#withDatabase(this.#databasePath(resourceId), true, (database) => this.#executeLiveSqlDatabase(database, sql, parameters, false));
      })();
  ...
  updateRow(args: { resourceId: string; object: string } & Omit<TDbRowUpdate, 'kind'>) {
    return this.#rowWrite(args.resourceId, async (database) => {
      ...
      if (Object.keys(values).length === 0) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Row update has no changed values.');
      ...
      if (result.rowsAffected !== 1) throw new ActorResourceError('DB_RESOURCE_ROW_CONFLICT', 'The row changed or disappeared before the update could be applied.');
  ...
  deleteRow(args: { resourceId: string; object: string } & Omit<TDbRowDelete, 'kind'>) {
    return this.#rowWrite(args.resourceId, async (database) => {
      ...
      if (result.rowsAffected !== 1) throw new ActorResourceError('DB_RESOURCE_ROW_CONFLICT', 'The row changed or disappeared before it could be deleted.');
      return { rowsAffected: result.rowsAffected };
    });
  }

  bulkRows(args: { resourceId: string; object: string; operations: readonly (TDbRowCreate | TDbRowUpdate | TDbRowDelete)[] }) {
    if (args.operations.length < 1 || args.operations.length > ROW_BULK_MAX) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Bulk row writes require between 1 and ${ROW_BULK_MAX} operations.`);
    if (Buffer.byteLength(JSON.stringify(args.operations), 'utf8') > RESULT_BYTES_MAX) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Bulk row writes exceed the payload size limit.');
    return this.#rowWrite(args.resourceId, async (database) => {
      const object = await this.#requireEditableObject(database, args.object);
      ...
      for (const operation of args.operations) {
        ...
        if (operation.kind === 'delete') {
          ...
          const optimistic = this.#expectedPredicate(expected);
          const result = await this.#runNative(database, `DELETE FROM ${quoteIdentifier(object.name)} WHERE ${identity.sql}${optimistic.sql}`, [...identity.parameters, ...optimistic.parameters]);
          if (result.rowsAffected !== 1) throw new ActorResourceError('DB_RESOURCE_ROW_CONFLICT', 'A bulk row target changed or disappeared.');
          results.push({ rowsAffected: result.rowsAffected });
          continue;
        }
        const values = this.#validatedValues(object, operation.values, false);
        const names = Object.keys(values);
        if (names.length === 0) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Bulk row update has no changed values.');
  ...
  async createDraft(resourceIdValue: string, draftIdValue: string): Promise<void> {
    ...
    try {
      ...
      throw toActorResourceError(error, 'DB_RESOURCE_DRAFT_INVALID', 'DbResource structure draft could not be created.');
    } finally {
      this.#blocked.delete(resourceId);
    }
  }
  ...
  async executeDraftSql(draftIdValue: string, sqlValue: string): Promise<TDbDraftChangeEvidence> {
    ...
    return this.#withDatabase(this.#draftDatabasePath(draftId), false, async (database) => {
      ...
      try {
        ...
        throw new ActorResourceError('DB_RESOURCE_DRAFT_INVALID', 'Advanced SQL failed without changing the draft.');
      }
    });
  }
    ...
    return this.#withDatabase(this.#draftDatabasePath(draftId), true, async (database) => {
      ...
      return rows.map((row): TDbDraftChangeEvidence => {
        ...
        if (!Number.isSafeInteger(sequence) || Number(sequence) < 1 || (row.kind !== 'structure' && row.kind !== 'sql') || typeof row.sql !== 'string') {
          throw new ActorResourceError('DB_RESOURCE_DRAFT_INVALID', 'Draft change evidence is invalid.');
        }
  ...
  async applyDraft(args: { resourceId: string; draftId: string; applyId: string; changes: readonly TDbResourceDraftChange[] }): Promise<{ outcome: 'succeeded' | 'recovered'; backupRetained: boolean; error: { code: string; message: string } | null }> {
    const resourceId = validateHostId(args.resourceId);
      ...
      return await this.#serializeWrite(resourceId, async () => {
        ...
        try {
          ...
          try {
            ...
            throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'The apply failed and the retained backup could not be restored safely.');
          }
        }
      });
    } finally {
  ...
  async restoreBackup(resourceIdValue: string, applyIdValue: string, restoreIdValue: string): Promise<void> {
    ...
    try {
      ...
      throw new ActorResourceError('DB_RESOURCE_RESTORE_FAILED', 'The retained DbResource backup could not be restored safely.');
    } finally {
      this.#blocked.delete(resourceId);
    }
  }
  ...
  async close(): Promise<void> {
    ...
    await Promise.allSettled([...this.#handles.keys()].map((resourceId) => this.#closeHandle(resourceId)));
  }

  async #dispatchGuest(context: TActorResolvedResourceCall, requirement: TActorDbResourceRequirement, operation: string, rawArgs: unknown): Promise<unknown> {
    try {
      const args = recordArgs(rawArgs);
      if (operation === 'invoke') {
        if (typeof args.operation !== 'string') throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Named database operation is invalid.');
        const declared = requirement.operations?.[args.operation];
        if (!declared) throw new ActorResourceError('DB_NAMED_OPERATION_UNKNOWN', `Named database operation "${args.operation}" is not declared.`);
        if (declared.effect === 'read' && !context.canRead) throw new ActorResourceError('DB_READ_NOT_ALLOWED', 'Read access is not allowed for this DbResource slot.');
        if (declared.effect === 'write' && !context.canWrite) throw new ActorResourceError('DB_WRITE_NOT_ALLOWED', 'Write access is not allowed for this DbResource slot.');
      ...
      if (operation === 'query') {
        if (requirement.arbitrarySql !== true) throw new ActorResourceError('DB_ARBITRARY_SQL_NOT_ALLOWED', 'Arbitrary SQL is not enabled for this DbResource slot.');
      ...
      if (operation === 'execute') {
        ...
        if (args.operations !== undefined) {
          if (!Array.isArray(args.operations) || args.operations.length < 1 || args.operations.length > EXECUTE_OPERATION_MAX_COUNT) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Database execute operations are invalid.');
          let totalSql = 0;
          const operations: TDbExecuteOperation[] = args.operations.map((value) => {
            ...
            totalSql += sql.length;
            if (totalSql > EXECUTE_TOTAL_SQL_MAX_LENGTH) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Database execute SQL exceeds the host limit.');
  ...
  async #executeLiveSqlDatabase(
    ...
    try {
      ...
      if (!approved) throw new ActorResourceError('DB_LIVE_SQL_APPROVAL_REQUIRED', 'Live database mutations require explicit user approval.');
      throw new ActorResourceError('DB_QUERY_FAILED', 'Live database SQL could not be prepared.');
    }
    ...
    try {
      ...
      if (columns.length > RESULT_COLUMN_MAX_COUNT || columns.some((column) => typeof column !== 'string' || column.length > 1_024)) {
        throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Live database SQL returned too many or invalid columns.');
      }
      ...
      try {
        for await (const raw of statement.iterate(parameters, { queryTimeout: QUERY_TIMEOUT_MS })) {
          rowCount += 1;
          if (rowCount > RESULT_ROW_MAX_COUNT) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', `Live database SQL returned more than ${RESULT_ROW_MAX_COUNT} rows.`);
          const native = normalizeNativeRow(raw);
          bytes += nativeRowBytes(native);
          if (bytes > LIVE_SQL_BYTES_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', `Live database SQL exceeded the ${LIVE_SQL_BYTES_MAX / 1_048_576} MiB result limit.`);
        ...
        if (!approved && !(error instanceof ActorResourceError)) {
          throw new ActorResourceError('DB_LIVE_SQL_APPROVAL_REQUIRED', 'Live database mutations require explicit user approval.');
        }
        throw error;
      }
      const changes = statement.database.totalChanges() - beforeChanges;
      if (!Number.isSafeInteger(changes) || changes < 0) throw new ActorResourceError('DB_QUERY_FAILED', 'Live database SQL returned invalid change metadata.');
      return {
        kind: 'rows',
        columns,
        rows,
      ...
      if (error instanceof ActorResourceError) throw error;
      if (!approved) throw new ActorResourceError('DB_LIVE_SQL_APPROVAL_REQUIRED', 'Live database mutations require explicit user approval.');
      throw new ActorResourceError('DB_EXECUTE_FAILED', 'Live database SQL failed.');
    } finally {
      statement.close();
    }
  }
  ...
  async #queryGuest(resourceId: string, sql: string, parameters: TDbBindParameters) {
    try {
      return await this.#withDatabase(this.#databasePath(resourceId), true, (database) => this.#queryGuestRows(database, sql, parameters));
    } catch (error) {
      if (error instanceof ActorResourceError) throw error;
  ...
  async #queryWriteGuest(resourceId: string, sql: string, parameters: TDbBindParameters) {
    try {
      ...
      if (error instanceof ActorResourceError) throw error;
      throw new ActorResourceError('DB_EXECUTE_FAILED', 'Database write query failed.');
    }
  }
  ...
  async #queryGuestRows(database: Database, sql: string, parameters: TDbBindParameters) {
    ...
    statement.safeIntegers(true);
    try {
      if (!statement.reader) throw new ActorResourceError('DB_READ_NOT_ALLOWED', 'Database query requires a row-producing statement.');
      const rows: TNativeRow[] = [];
      let bytes = 0;
      for await (const raw of statement.iterate(parameters, { queryTimeout: QUERY_TIMEOUT_MS })) {
        if (rows.length >= RESULT_ROW_MAX_COUNT) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database result has too many rows.');
        const row = normalizeNativeRow(raw);
        bytes += nativeRowBytes(row);
        if (bytes > RESULT_BYTES_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database result exceeds the size limit.');
        rows.push(row);
      }
      return rows;
  ...
  async #executeReadonlyGuest(resourceId: string, sql: string, parameters: TDbBindParameters) {
    try {
      return await this.#withDatabase(this.#databasePath(resourceId), true, (database) => this.#runNative(database, sql, parameters));
    } catch { throw new ActorResourceError('DB_QUERY_FAILED', 'Read-only database operation failed.'); }
  }
  ...
  async #executeGuest(resourceId: string, sql: string, parameters: TDbBindParameters) {
    ...
    } catch { throw new ActorResourceError('DB_EXECUTE_FAILED', 'Database execute failed.'); }
  }
  ...
  async #inspectDatabase(database: Database): Promise<TDbObject[]> {
    ...
    `, []);
    if (schemaRows.length > INSPECTION_OBJECT_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database has too many user objects to inspect safely.');
    ...
    for (const schemaRow of schemaRows) {
      ...
      if (columnRows.length > INSPECTION_MEMBER_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database object has too many columns to inspect safely.');
      ...
      if (kind === 'table') {
        ...
        if (indexRows.length > INSPECTION_MEMBER_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database object has too many indexes to inspect safely.');
        for (const indexRow of indexRows) {
          ...
          if (indexColumnRows.length > INSPECTION_MEMBER_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database index has too many columns to inspect safely.');
      ...
      if (foreignRows.length > INSPECTION_MEMBER_MAX) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database object has too many foreign-key columns to inspect safely.');
    ...
    if (Buffer.byteLength(JSON.stringify(objects), 'utf8') > RESULT_BYTES_MAX) {
      throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database structure inspection exceeds the response size limit.');
    }
    return objects;
  }
  ...
  async #requireObject(database: Database, nameValue: string): Promise<TDbObject> {
    ...
    if (!object) throw new ActorResourceError('DB_RESOURCE_TABLE_READ_ONLY', 'Database table or view was not found.');
    return object;
  }
  ...
  async #requireEditableObject(database: Database, name: string): Promise<TDbObject> {
    ...
    if (!object.editable || !object.identity) throw new ActorResourceError('DB_RESOURCE_TABLE_READ_ONLY', object.readOnlyReason ?? 'Database object is read-only.');
    return object;
  }
  ...
  #validatedValues(object: TDbObject, values: Readonly<Record<string, TDbCellValue>>, allowEmpty: boolean): Record<string, TDbBindValue> {
    if (!isPlainObject(values)) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Row values must be an object.');
    ...
    for (const [name, value] of Object.entries(values)) {
      if (!columns.has(name)) throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', `Unknown row column "${name}".`);
  ...
  #identityPredicate(object: TDbObject, identity: TDbRowIdentity): { sql: string; parameters: TDbBindValue[] } {
    if (object.identity?.kind === 'rowid' && identity.kind === 'rowid') {
      if (identity.value.type !== 'integer') throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Rowid identity must be an SQLite integer.');
    ...
    if (object.identity?.kind === 'primaryKey' && identity.kind === 'primaryKey') {
      ...
      if (names.length !== object.identity.columns.length || object.identity.columns.some((name) => !Object.prototype.hasOwnProperty.call(identity.values, name))) {
        throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Complete primary-key identity is required.');
      }
    ...
    throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Stable row identity is required.');
  }
  ...
  #cursorPredicate(object: TDbObject, cursor: TDbRowIdentity | null): { sql: string; parameters: TDbBindValue[] } {
    ...
    if (object.identity?.kind === 'rowid' && cursor.kind === 'rowid') {
      if (cursor.value.type !== 'integer') throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Rowid cursor must be an SQLite integer.');
    ...
    if (object.identity?.kind === 'primaryKey' && cursor.kind === 'primaryKey') {
      ...
      if (names.length !== columns.length || columns.some((name) => !Object.prototype.hasOwnProperty.call(cursor.values, name))) {
        throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Complete primary-key cursor is required.');
      }
    ...
    throw new ActorResourceError('DB_RESOURCE_ROW_IDENTITY_REQUIRED', 'Cursor identity does not match this table.');
  }
  ...
  #expectedForUpdate(
    ...
    if (Object.keys(updates).some((name) => !Object.prototype.hasOwnProperty.call(expected, name))) {
      throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Expected original values must include every updated column.');
    }
    return expected;
  }
  ...
  #expectedForDelete(
    ...
    if (Object.keys(expected).length !== visible.length || visible.some((name) => !Object.prototype.hasOwnProperty.call(expected, name))) {
      throw new ActorResourceError('DB_OPERATION_PARAMETERS_INVALID', 'Expected original values must include the complete visible row before deletion.');
    }
    return expected;
  }
  ...
  async #structuredSql(database: Database, operation: TDbDraftOperation): Promise<string> {
    ...
    if (operation.kind === 'createIndex') {
      if (operation.columns.length < 1 || operation.columns.length > RESULT_COLUMN_MAX_COUNT) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'An index requires columns.');
      ...
      if (operation.columns.some((column) => !available.has(column))) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Index column was not found.');
  ...
  async #rebuildTableSql(database: Database, operation: Extract<TDbDraftOperation, { kind: 'alterColumn' | 'createForeignKey' | 'dropForeignKey' }>): Promise<string> {
    ...
    if (
      ...
      throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'This table contains structure that the structured rebuild cannot preserve losslessly; use advanced draft SQL.');
    }
    ...
    if (operation.kind === 'alterColumn') {
      ...
      if (index < 0) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Column to alter was not found.');
      ...
      if (next.name !== operation.column) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Structured alter-column cannot rename a column; use the dedicated rename-column operation.');
      columns[index] = next;
      ...
      if (operation.columns.length < 1 || operation.columns.length !== operation.referencedColumns.length) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Foreign-key column lists must be non-empty and equal in length.');
      const available = new Set(columns.map((column) => column.name));
      if (operation.columns.some((column) => !available.has(column))) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Foreign-key source column was not found.');
      ...
      if (operation.referencedColumns.some((column) => !referencedAvailable.has(column))) throw new ActorResourceError('DB_RESOURCE_SCHEMA_OPERATION_INVALID', 'Foreign-key referenced column was not found.');
  ...
  #rowWrite<T>(resourceIdValue: string, write: (database: Database) => Promise<T>): Promise<T> {
    ...
    return this.#track(resourceId, this.#serializeWrite(resourceId, async () => {
      ...
      try { return await transaction(); }
        ...
        if (error instanceof ActorResourceError) throw error;
        throw new ActorResourceError('DB_EXECUTE_FAILED', 'Live row write failed.');
      }
    }));
  }
  ...
  async #queryNative(
    ...
    try {
      ...
      let bytes = 0;
      for await (const raw of statement.iterate([...parameters], { queryTimeout: QUERY_TIMEOUT_MS })) {
        if (rows.length >= RESULT_ROW_MAX_COUNT) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database management query returned too many rows.');
        const row = normalizeNativeRow(raw, columnMax);
        bytes += nativeRowBytes(row);
        if (bytes > byteMax) throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database management query exceeded the response size limit.');
        rows.push(row);
      }
      return rows;
    } finally { statement.close(); }
  ...
  async #foreignKeyViolations(database: Database): Promise<Set<string>> {
    try {
      ...
      const tableNames = new Set(tableRows.map((row) => {
        if (typeof row.name !== 'string') throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign-key inspection returned an invalid table name.');
      ...
      for (const tableRow of tableRows) {
        if (typeof tableRow.name !== 'string') throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign-key inspection returned an invalid table name.');
        ...
        for (const row of foreignRows) {
          ...
          if (!Number.isSafeInteger(id) || id < 0 || !Number.isSafeInteger(sequence) || sequence < 0 || typeof row.table !== 'string' || typeof row.from !== 'string') {
            throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign-key inspection returned invalid metadata.');
          }
          const parentTable = validateIdentifier(row.table);
          const current = foreignKeys.get(id) ?? { parentTable, parts: [] };
          if (current.parentTable !== parentTable) throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign-key inspection returned inconsistent metadata.');
          current.parts.push({
            sequence,
        ...
        for (const [foreignKeyId, foreignKey] of foreignKeys) {
          ...
          if (parts.length === 0 || parts.some((part, index) => part.sequence !== index)) {
            throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign-key inspection returned a non-contiguous key definition.');
          }
          ...
          if (parentExists && parts.some((part) => part.parentColumn === null)) {
            ...
            if (primaryKey.length !== parts.length || parts.some((part) => part.parentColumn !== null)) {
              throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign key does not resolve to a complete parent primary key.');
            }
          ...
          if (parentExists) {
            ...
            if (!eligibleKeys.some((key) => key.length === referencedColumns.length && key.every((column, index) => column === referencedColumns[index]))) {
              throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Foreign key references parent columns that are not an eligible primary or unique key in the declared order.');
            }
          }
          ...
          for (const row of await this.#queryNative(database, sql, [])) {
            ...
            if (!Number.isSafeInteger(count) || Number(count) < 1 || violationCount + Number(count) > RESULT_ROW_MAX_COUNT) {
              throw new ActorResourceError('DB_RESULT_LIMIT_EXCEEDED', 'Database has too many foreign-key violations to verify safely.');
            }
      ...
      if (error instanceof ActorResourceError) throw error;
      throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'Database foreign-key verification is unavailable or failed.');
    }
  }
  ...
  async #verifyDatabase(database: Database, baselineForeignKeys: ReadonlySet<string>): Promise<void> {
    ...
    if (!healthy) throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'DbResource database health check failed.');
    const violations = await this.#foreignKeyViolations(database);
    if ([...violations].some((violation) => !baselineForeignKeys.has(violation))) throw new ActorResourceError('DB_RESOURCE_APPLY_FAILED', 'DbResource apply introduced a foreign-key violation.');
  }
  ...
  async #withDatabase<T>(databasePath: string, readonly: boolean, operation: (database: Database) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (readonly) {
        ...
        if (queryOnly.length !== 1 || !Object.values(queryOnly[0]).some((value) => Number(value) === 1)) {
          throw new ActorResourceError('DB_RESOURCE_UNAVAILABLE', 'Database readonly connection could not be enforced.');
        }
      } else {
  ...
  async #copyDatabaseFiles(sourcePath: string, destinationPath: string): Promise<void> {
    ...
    if (!sourceEntries.includes(sourceName)) throw new ActorResourceError('DB_RESOURCE_RECOVERY_FAILED', 'DbResource snapshot source is missing its database file.');
  ...
  #open(resourceId: string, fileMustExist: boolean): Promise<Database> {
    ...
    void opening.catch(() => { if (this.#handles.get(resourceId) === opening) this.#handles.delete(resourceId); });
    return opening;
  }
  ...
  #track<T>(resourceId: string, call: Promise<T>): Promise<T> {
    ...
    calls.add(call);
    void call.finally(() => { calls.delete(call); if (calls.size === 0) this.#inflight.delete(resourceId); }).catch(() => undefined);
    return call;
  }
  ...
  #assertAvailable(resourceId?: string): void {
    ...
    if (resourceId && this.#blocked.has(resourceId)) throw new ActorResourceError('DB_RESOURCE_MIGRATING', 'DbResource is unavailable while coordinated database work is running.');
  }
}
