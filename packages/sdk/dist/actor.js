...
function formatValidationErrors(errors) {
  if (!errors || errors.length === 0)
    return "payload did not match schema";
...
function getOutputSchema(definition) {
  if (!definition)
    return;
  if (typeof definition === "object" && "schema" in definition)
...
function createActorRuntime(portal = {}) {
  ...
  function receive(input, handler) {
  ...
  function defineRuntimeActor(nextDefinition) {
    ...
    Object.entries(nextDefinition.inputs ?? {}).forEach(([input, inputDefinition]) => {
      const entry = inputDefinition;
      const validate = compileSchema(entry.schema);
      if (validate)
        inputValidators.set(input, validate);
      if (entry.handle)
        receive(input, entry.handle);
    });
    Object.entries(nextDefinition.outputs ?? {}).forEach(([output, outputDefinition]) => {
      ...
      if (validate)
        outputValidators.set(output, validate);
    });
    return { receive, send };
  }
...
function normalizeActorHandlerResult(args) {
  return {
