...
export function fnValidateWidgetBuildIntegrity(
  ...
  if (
    args.build.uiArtifact.artifactHash !== runtime.artifactHash
    ...
    || !sameJson(
      runtime.apiContract,
      fnNormalizeWidgetRuntimeApiContract(runtime.apiContract),
    )
    || !sameJson(
      runtime.budgets,
      fnNormalizeWidgetRuntimeBudgetRequest(args.manifest.ui.budgets ?? {}),
    )
  ...
  const capabilityContractDigestSha256 = args.digestSha256(
    fnCanonicalizeWidgetCapabilityRequests(runtime.capabilityRequests),
  );
  ...
  const channelContractDigestSha256 = args.digestSha256(
    fnCanonicalizeWidgetChannelContract(runtime.channels),
  );
