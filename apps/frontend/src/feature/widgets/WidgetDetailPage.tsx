...
export type TWidgetDetailPageProps = { source: TWidgetSource | null; name: string | null };
...
export const WidgetDetailPage: Component<TWidgetDetailPageProps> = (props) => {
  ...
  const activeTab = (): TTab => searchParams.tab === 'config'
    || searchParams.tab === 'messages'
    || searchParams.tab === 'states'
    || searchParams.tab === 'files'
    ? searchParams.tab
    : 'overview';
  ...
  const loadDetail = async () => {
    ...
    const [loadError, value] = await orpcWebsocketService.apiService.api.agent.widgets.detail({ name, source });
  ...
  const loadFiles = async () => {
    ...
    setFilesError('');
    const [loadError, value] = await orpcWebsocketService.apiService.api.agent.widgets.files({ name: props.name, source: props.source });
  ...
  const loadPreview = async (path: string) => {
    ...
    setPreview(null);
    setPreviewError('');
    const [loadError, value] = await orpcWebsocketService.apiService.api.agent.widgets.file({ name: props.name, source: props.source, path });
  ...
  const selectTab = (value: string) => {
    ...
    setSearchParams(tab === 'files' ? { tab, path: selectedPath() || undefined } : { tab, path: undefined });
  };
  ...
  const editAsDraft = async () => {
    ...
    setSaving(true);
    const [ensureError] = await orpcWebsocketService.apiService.api.agent.widgets.ensureDraft({
      name: current.name,
      expectedPublishedFingerprint: current.variant.contentFingerprint ?? undefined,
    });
    setSaving(false);
    if (ensureError) { showErrorToast(ensureError.message); return; }
    await catalogState.refresh();
    navigate(`/widgets/draft/${encodeURIComponent(current.name)}?tab=config`);
  };
  ...
  const saveMetadata = async () => {
    ...
    if (!nextName) { showErrorToast('Widget name is required.'); return; }
    if (!nextLabel) { showErrorToast('Tool label is required.'); return; }
    if (nextPriority !== null && !Number.isFinite(nextPriority)) { showErrorToast('Priority must be a number.'); return; }
    setSaving(true);
    const [saveError, result] = await orpcWebsocketService.apiService.api.agent.widgets.patchDraftMetadata({
      name: current.name,
    ...
    showSuccessToast('Widget draft configuration saved');
    await catalogState.refresh();
    if (result.name !== current.name) {
      navigate(`/widgets/draft/${encodeURIComponent(result.name)}?tab=config`, { replace: true });
      return;
    }
    await loadDetail();
  };
  ...
  const removeWidget = async () => {
    ...
    if (!current) return;
    setDeleting(true);
    const [deleteError, result] = await orpcWebsocketService.apiService.api.agent.widgets.delete({ name: current.name, source: current.source });
    setDeleting(false);
    if (deleteError) { showErrorToast(deleteError.message); return; }
    setDeleteOpen(false);
    ...
    if (result.issues.length > 0) {
      ...
      if (selectedSourceDeleted) showSuccessToast(title, issueDescription);
      else showErrorToast(title, issueDescription);
    } else {
      showSuccessToast(current.source === 'published' ? 'Published widget, draft, and instances deleted' : 'Widget draft deleted');
    }
    await catalogState.refresh();
    if (selectedSourceDeleted) navigate('/');
  ...
  const publish = async () => {
    ...
    if (!current || current.source !== 'draft') return;
    setSaving(true);
    const [publishError, result] = await orpcWebsocketService.apiService.api.agent.widgetPublish.publish({ draftId: current.name, expectedRevision: current.variant.revision });
    setSaving(false);
    if (publishError) {
    ...
    showSuccessToast('Widget published');
