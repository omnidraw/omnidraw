...
import { ThemeService, THEME_ID_DARK } from '@omnidraw/theme';
import {
  fnWidgetHostTheme,
  fnWidgetPreviewTitleBarColor,
} from './fn.widget-host-theme';

describe('widget host theme projection', () => {
  test('exposes only the fixed safe semantic token set', () => {
    const service = new ThemeService({ initialThemeId: THEME_ID_DARK });
    const projected = fnWidgetHostTheme(service.getTheme());
