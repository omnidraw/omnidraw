...
describe("database resource workbench functions", () => {
  ...
  test("reads stable management error codes from the API envelope", () => {
    const error = new PrivateRpcError({ code: "DB_RESOURCE_ROW_CONFLICT", status: 409, message: "Resource operation failed.", details: null });
  ...
  test("recognizes the typed mutation approval challenge", () => {
    const error = new PrivateRpcError({ code: "DB_LIVE_SQL_APPROVAL_REQUIRED", status: 409, message: "Approval required.", details: null });
    expect(fnLiveSqlApprovalRequired(error)).toBe(true);
    expect(fnLiveSqlApprovalRequired(new PrivateRpcError({ code: "OTHER", status: 400, message: "Other", details: null }))).toBe(false);
  });
