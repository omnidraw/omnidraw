...
describe('OSS canvas document transport', () => {
  test('returns the underlying oRPC event iterator during cancellation', async () => {
    let closeCount = 0;
    ...
    const transport = createCanvasDocumentTransport({
      async snapshot({ canvasId }) {
        return [null, { canvasId, revision: 0, items: [] }];
      },
  ...
  test('settles cancellation before a deferred oRPC stream finishes opening', async () => {
    let closeCount = 0;
    let openStream!: (
  ...
  test('waits for an opened oRPC stream to finish closing', async () => {
