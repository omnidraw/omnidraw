...
import {
  ...
  fnTimestamp,
  fnWorkbenchTabLabel,
} from "@/core/resources/fn.db-resource";
import {
  fxApplies,
  fxActiveDraft,
  fxApply,
  ...
  fxRow,
  fxRows,
} from "@/core/resources/fx.db-resource";
import { fxPollDbOperation } from "@/core/resources/fx.poll-db-operation";
import {
  txBulkDeleteRows,
  txConfirmApply,
  txCreateDraft,
  ...
  txRestoreBackup,
  txUpdateRow,
} from "@/core/resources/tx.db-resource";
import type {
  TDbApplyDetails,
  TDbApplyPreview,
  TDbBackup,
...
export const DbResourcePage: Component<TDbResourcePageProps> = (props) => {
  ...
  const loadMeta = async () => {
    ...
    const applyDetailsResponses = await Promise.all((applyValues ?? []).map(async (run) => ({
      run,
  ...
  const rename = async () => {
    ...
    setBusy(true);
    const [renameError] = await runtime.runSafe(txRename({ resourceId: props.resourceId, name: trimmed }));
    setBusy(false);
  ...
  const deleteResource = async () => {
    setBusy(true);
    const [deleteError] = await runtime.runSafe(txDeleteResource({ resourceId: props.resourceId }));
    setBusy(false);
  ...
  const createDraft = async () => {
    setBusy(true);
    const [draftError] = await runtime.runSafe(txCreateDraft({ resourceId: props.resourceId, name: "Schema draft" }));
    setBusy(false);
  ...
  const discardDraft = async () => {
    ...
    if (!draft) return;
    setBusy(true);
    const [discardError] = await runtime.runSafe(txDiscardDraft({ draftId: draft.id }));
    setBusy(false);
  ...
  const commitStructureOperation = async (operation: Record<string, unknown>) => {
    ...
    if (!draft) return;
    setBusy(true);
    const [changeError] = await runtime.runSafe(txDraftChange({ draftId: draft.id, operation }));
    setBusy(false);
  ...
  const executeSql = async (approved = false, approvedSql?: string) => {
    ...
    if (!statement) return;
    setBusy(true);
    const [sqlError, result] = await runtime.runSafe(txExecuteLiveSql({ resourceId: props.resourceId, sql: statement, approved }));
    setBusy(false);
    if (sqlError) {
  ...
  const openRowEditor = async (mode: "create" | "edit", row: TDbRowPreview | null) => {
    ...
    if (mode === "edit") {
      ...
      if (editableColumns.length > 0) {
        setBusy(true);
        const [rowError, projectedRow] = await runtime.runSafe(fxRow({ resourceId: props.resourceId, objectName: object.name, identity: row.identity, columns: editableColumns.map((column) => column.name) }));
        setBusy(false);
  ...
  const openRowDelete = async (row: TDbRowPreview) => {
    ...
    if (!object || !row.identity) return;
    setBusy(true);
    const [rowError, fullRow] = await runtime.runSafe(fxRow({ resourceId: props.resourceId, objectName: object.name, identity: row.identity }));
    setBusy(false);
  ...
  const saveRow = async (values: TDbRow["values"]) => {
    ...
    const [rowError] = rowDialogMode() === "create"
      ? await runtime.runSafe(txCreateRow({ resourceId: props.resourceId, objectName: object.name, values }))
      : await runtime.runSafe(txUpdateRow({
  ...
  const deleteRow = async () => {
    ...
    if (!object || !row?.identity) return;
    setBusy(true);
    const [rowError] = await runtime.runSafe(txDeleteRow({
      resourceId: props.resourceId,
      objectName: object.name,
      identity: row.identity,
      expected: row.values,
  ...
  const bulkDelete = async () => {
    ...
    setBusy(true);
    const hydratedResponses: Array<TApiResult<TDbRow>> = [];
    for (const batch of fnBatches(rows, 4)) {
      hydratedResponses.push(...await Promise.all(batch.map((row) => runtime.runSafe(fxRow({
        resourceId: props.resourceId,
        objectName: object.name,
        identity: row.identity,
      })))));
    ...
    const [bulkError] = await runtime.runSafe(txBulkDeleteRows({
      resourceId: props.resourceId,
      objectName: object.name,
  ...
  const openApply = async () => {
    ...
    setOperationError("");
    const [previewError, preview] = await runtime.runSafe(fxApplyPreview({ draftId: draft.id }));
  ...
  const openRestore = async () => {
    ...
    setOperationRun(null);
    setOperationError("");
    const [previewError, preview] = await runtime.runSafe(fxRestorePreview({ resourceId: props.resourceId, applyId: retained.applyId }));
    setOperationLoading(false);
  ...
  const discardBackup = async () => {
    ...
    if (!retained) return;
    setBusy(true);
    const [discardError] = await runtime.runSafe(txDiscardBackup({ resourceId: props.resourceId, applyId: retained.applyId }));
    setBusy(false);
    if (discardError) return showErrorToast(discardError.message);
    setConfirmState(null);
          ...
          <Tabs.Root value={activeTab()} onChange={selectTab} class={styles.tabsRoot}>
            <header class={styles.header}>
              ...
              <div class={styles.headerActions}>
                ...
                <Button class={`${styles.button} ${styles.iconButton}`} aria-label="Toggle sidebar" onClick={() => runtime.store.set("sidebarVisible", (visible) => !visible)}><PanelLeft size={15} /></Button>
