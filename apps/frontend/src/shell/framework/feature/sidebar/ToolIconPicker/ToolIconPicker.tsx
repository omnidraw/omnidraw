...
export function ToolIconGlyph(props: { icon?: string }) {
  ...
  return <Show when={props.icon}>{(icon) => (
    <Show when={svg()} fallback={<span class="omnidraw-tool-icon-picker__text">{firstGrapheme(icon())}</span>}>
      {(markup) => <span class="omnidraw-tool-icon-picker__glyph" innerHTML={markup()} aria-hidden="true" />}
    </Show>
  )}</Show>;
}
...
export function ToolIconPicker(props: {
  ...
  return <div class="omnidraw-tool-icon-picker">
    <span class="omnidraw-tool-icon-picker__label">Icon</span>
    <button type="button" class="omnidraw-tool-icon-picker__trigger" aria-haspopup="listbox" aria-expanded={open()} onClick={() => setOpen((value) => !value)}>
      <span class="omnidraw-tool-icon-picker__value"><ToolIconGlyph icon={selected().icon} /><span>{selected().label}</span></span><span aria-hidden="true">⌄</span>
    </button>
    <Show when={open()}>
      <div class="omnidraw-tool-icon-picker__menu">
        <TextField value={query()} onChange={setQuery}>
          <TextField.Input class="omnidraw-tool-icon-picker__search" placeholder="Filter Lucide icons…" autofocus />
        </TextField>
        <div role="listbox" aria-label="Icon choices">
          <For each={filtered()}>{(option) => <button type="button" class="omnidraw-tool-icon-picker__option" classList={{ "omnidraw-tool-icon-picker__option--selected": selectedId() === option.id }} role="option" aria-selected={selectedId() === option.id} onClick={() => select(option)}><ToolIconGlyph icon={option.icon} /><span>{option.label}</span></button>}</For>
        </div>
      </div>
    </Show>
    <Show when={selectedId() === ICON_CUSTOM_ID}>
      <TextField class="omnidraw-tool-icon-picker__custom" value={customIcon()} onChange={(value) => { setCustomIcon(value); const normalized = normalizeCustomToolIcon(value); props.onChange(normalized ? { svgIcon: normalized } : null); }}>
        <TextField.Label class="omnidraw-tool-icon-picker__label">Custom SVG / emoji / text</TextField.Label>
        <TextField.TextArea class="omnidraw-tool-icon-picker__textarea" rows={4} spellcheck={false} />
        <TextField.Description class="omnidraw-tool-icon-picker__help">SVG is preserved. Emoji or text uses its first character.</TextField.Description>
      </TextField>
    </Show>
  </div>;
}
