...
Secret-store database pages and write-ahead logs are encrypted at rest. This protects copied database files and data-directory-only backups, but it does not protect plaintext displayed on screen, plaintext used by an authorized actor, or a machine/process compromise. Secret names, resource directories, and database headers are not confidential.
