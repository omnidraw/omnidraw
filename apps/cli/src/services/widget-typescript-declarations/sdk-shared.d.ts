...
export type TActorRuntimeState = 'booting' | `booting.${string}` | 'ready' | `ready.${string}` | 'busy' | `busy.${string}` | 'waiting' | `waiting.${string}` | 'error' | `error.${string}`;
export type TActorSystemStatus = 'created' | 'starting' | 'running' | 'paused' | 'stopping' | 'stopped' | 'error' | 'blocked';
