...
implements IService, TPreviewInspectionBrowserPort {
  ...
  async #preflight(): Promise<TPreviewInspectionBrowserPreflight> {
    if (
      this.#config.releaseManifestRequired === true
      && this.#config.expectedExecutableSha256 === undefined
    ) {
      return Object.freeze({
        ok: false,
        code: 'BROWSER_RELEASE_MANIFEST_MISSING',
        message: 'The compiled Preview inspection runtime manifest is missing or invalid.',
        remediation: 'Reinstall this exact Omnidraw release for the current platform and architecture.',
      });
    }
    ...
    try {
      ...
      return Object.freeze({
        ...
        remediation: 'Run `bun --cwd apps/cli x playwright@1.61.1 install chromium`, then restart.',
      });
    }
    if (this.#stopping) {
      return Object.freeze({
    ...
    if (
      this.#config.expectedExecutableSha256 !== undefined
      && evidence.executableSha256 !== this.#config.expectedExecutableSha256
    ) {
      return Object.freeze({
        ok: false,
        code: 'BROWSER_CHECKSUM_MISMATCH',
        message: 'The installed Preview inspection browser checksum does not match the release manifest.',
        remediation: 'Reinstall this exact Omnidraw release for the current platform and architecture.',
      });
    }
    ...
    if (executable === null || !executable.isFile()) {
      return Object.freeze({
        ...
        remediation: 'Run `bun --cwd apps/cli x playwright@1.61.1 install chromium` and restart Omnidraw.',
      });
    }
    ...
    if (shellEntry === null || !shellEntry.isFile()) {
      return Object.freeze({
        ...
        remediation: 'Run `bun run --cwd apps/preview-inspection-shell build` and restart Omnidraw.',
      });
    }
    if (this.#stopping) {
      return Object.freeze({
