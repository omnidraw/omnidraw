...
export type TPlaywrightRuntimeIdentityPortal = Readonly<{
...
const DEFAULT_IDENTITY_PORTAL: TPlaywrightRuntimeIdentityPortal = Object.freeze({
 ...
 * it. Compiled preflight must compare this evidence to its release manifest
 * before calling `readPlaywrightRuntimeIdentityFromEvidence`.
 */
export async function readPlaywrightRuntimeExecutableEvidence(
  portal: TPlaywrightRuntimeIdentityPortal = DEFAULT_IDENTITY_PORTAL,
): Promise<TPlaywrightRuntimeExecutableEvidence> {
  const packageVersion = portal.packageVersion();
  const path = portal.executablePath();
  ...
  try {
    rawSha256 = await portal.executableSha256(path);
  } catch {
...
export async function readPlaywrightRuntimeIdentityFromEvidence(
  ...
  portal: Pick<TPlaywrightRuntimeIdentityPortal, 'executableVersion'> = DEFAULT_IDENTITY_PORTAL,
): Promise<TPlaywrightRuntimeIdentity> {
  let rawVersion: unknown;
  try {
    rawVersion = await portal.executableVersion(evidence.executablePath);
  } catch {
    throw identityError('The Playwright Chromium executable could not be verified.');
  }
...
/** Source-mode convenience composition; compiled preflight must use both phases. */
export async function readPlaywrightRuntimeIdentity(
  portal: TPlaywrightRuntimeIdentityPortal = DEFAULT_IDENTITY_PORTAL,
): Promise<TPlaywrightRuntimeIdentity> {
  const evidence = await readPlaywrightRuntimeExecutableEvidence(portal);
  return await readPlaywrightRuntimeIdentityFromEvidence(evidence, portal);
}
