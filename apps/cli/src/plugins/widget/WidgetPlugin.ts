...
function upsertWidgetActor(args: {
  ...
  const functionsRelativePath = asString(actor.functionsPath, './actor/functions.ts');
  ...
  const slug = asString(args.widgetConfig.slug, fallbackSlug);
  const widgetId = asString(args.widgetConfig.id, fallbackSlug);
  const name = asString(args.widgetConfig.name, slug);
  const version = asVersion(args.widgetConfig.version);
  const description = typeof args.widgetConfig.description === 'string' ? args.widgetConfig.description : null;
