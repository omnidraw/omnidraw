...
describe('managed widget joined production flow', () => {
  test('joins resource persistence, Automerge projection, publication, and invocation with zero residue', async () => {
    const root = await mkdtemp(join(tmpdir(), 'vibecanvas-managed-v3-joined-'));
    roots.push(root);
    ...
    const tenant = fnFreezeTenantContext({
      ...
      roles: ['owner'],
      capabilities: ['*'],
      requestId: 'managed-v3-joined-flow',
      canvasId: uuid(990),
    });
    ...
    try {
      ...
      const manifest: TWidgetManifestV3 = {
        schemaVersion: 3,
        name: 'Managed v3 joined flow',
        slug: 'managed-v3-joined-flow',
      ...
      const accepted = await functionInvocation.invokeFunction(tenant, {
        ...
        input: { key: 'joined/result', value: 'persisted-through-v3' },
        idempotencyKey: 'managed-v3-joined-flow-key',
      });
      ...
      expect(terminal).toMatchObject({
        ...
        widgetInstanceId: uuid(994),
        status: 'succeeded',
        output: { value: 'persisted-through-v3', revision: 1 },
        failure: null,
      });
