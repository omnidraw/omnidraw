...
import { PreviewInspectionBrowserService } from '../src/services/preview-inspection/PreviewInspectionBrowserService';
import { fnPreviewInspectionChromiumLaunchOptions } from '../src/services/preview-inspection/fn.browser-launch';
import type { TPlaywrightRuntimeExecutableEvidence } from '../src/services/preview-inspection/playwright-runtime-identity';
import type {
...
describe('PreviewInspectionBrowserService', () => {
  ...
  test('compares independently observed runtime identity and release checksum pins', async () => {
    ...
    const checksumService = new PreviewInspectionBrowserService({
      tempRoot: join(root, 'checksum-mismatch-jobs'),
      shell: fakes.shell,
      launcher: fakes.launcher,
      internals: fakes.internals,
      driver: fakes.driver,
      expectedExecutableSha256: SHA_B,
    });
    await expect(checksumService.preflight()).resolves.toMatchObject({
      ok: false,
      code: 'BROWSER_CHECKSUM_MISMATCH',
    });
    expect(fakes.state.runtimeVersionReads).toBe(0);
    await checksumService.stop();

    const manifestService = new PreviewInspectionBrowserService({
      tempRoot: join(root, 'manifest-missing-jobs'),
      shell: fakes.shell,
      launcher: fakes.launcher,
      internals: fakes.internals,
      driver: fakes.driver,
    ...
    await expect(manifestService.preflight()).resolves.toMatchObject({
      ok: false,
