...
import { txTerminateWidgetBuildProcessTree } from '../src/services/tx.terminate-widget-build-process-tree';

describe('txTerminateWidgetBuildProcessTree', () => {
  test('kills a complete POSIX process group', async () => {
    const calls: string[] = [];
    await txTerminateWidgetBuildProcessTree({
      platform: 'darwin',
  ...
  test('uses Windows task-tree termination and waits for confirmation', async () => {
    ...
    const pending = txTerminateWidgetBuildProcessTree({
      platform: 'win32',
      killProcessGroup(pid) {
        calls.push(`group:${pid}`);
      },
