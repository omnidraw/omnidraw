...
} from '#backend/shell/widget-runtime/capabilities';
import { createOmnidrawGuestChannelContract } from './build/create-capability-contracts';

const decoder = new TextDecoder();

function documentFor(
  contract: Awaited<ReturnType<typeof createOmnidrawGuestChannelContract>>,
  hash: string,
...
describe('Omnidraw Capsule guest channels', () => {
  test('binds props, theme, and output to distinct deterministic bounded schemas', async () => {
    const first = await createOmnidrawGuestChannelContract({
      localStore: 'ephemeral',
    });
    const second = await createOmnidrawGuestChannelContract({
      localStore: 'ephemeral',
    });
    const references = [
    ...
    expect(theme).toMatchObject({
      root: {
        ...
        maxProperties: 3,
        properties: {
          format: { type: 'literal', value: 'omnidraw.widget-theme.v1' },
        },
      },
    });
  ...
  test('fails closed when a routed value is not the one notification action', () => {
    expect(fnOmnidrawWidgetNotificationOutput({
      type: 'notification',
      tone: 'success',
      message: 'Saved',
    })).toEqual({
