...
describe('neutral resource contracts', () => {
  ...
  test('allows plaintext only in the strict one-secret reveal response', () => {
    expect(resourceContract.resources.dataRevealSecret.routeMetadata).toEqual({ method: 'POST' });
  ...
  test('preserves stable safe resource codes through the private error envelope', async () => {
