...
export const resourceContract = pc.errors({
  RESOURCE_ERROR: {
    status: 409,
  ...
  resources: {
    ...
    get: pc.input(z.object({ resourceId: ZResourceId })).output(ZResource),
    create: pc.input(ZCreateResourceInput).output(ZResource),
    rename: pc
  ...
  dbDrafts: {
    create: pc.input(z.object({ resourceId: ZResourceId, name: ZResourceName })).output(ZDbDraftDetails),
    list: pc.input(z.object({ resourceId: ZResourceId, before: ZCursor.optional(), limit: z.number().int().min(1).max(100).optional() })).output(z.array(ZDbResourceDraft)),
    get: pc.input(z.object({ draftId: ZHostId })).output(ZDbDraftDetails),
    active: pc.input(z.object({ resourceId: ZResourceId })).output(ZDbDraftDetails.nullable()),
    inspect: pc.input(z.object({ resourceId: ZResourceId, draftId: ZHostId.optional() })).output(ZDbInspection.nullable()),
    change: pc.input(z.object({ draftId: ZHostId, operation: ZDbDraftOperation })).output(ZDbResourceDraftChange),
    executeSql: pc.input(z.object({ draftId: ZHostId, sql: ZDraftSql })).output(ZDbResourceDraftChange),
    discard: pc.input(z.object({ draftId: ZHostId })).route({ method: 'DELETE' }).output(ZDbResourceDraft),
  },
  dbApplies: {
    preview: pc.input(z.object({ draftId: ZHostId })).output(ZDbApplyPreview),
    confirm: pc.input(z.object({ draftId: ZHostId })).output(ZDbResourceApplyRun),
    get: pc.input(z.object({ applyId: ZHostId })).output(ZDbApplyDetails),
    list: pc.input(z.object({ resourceId: ZResourceId, before: ZCursor.optional(), limit: z.number().int().min(1).max(100).optional() })).output(z.array(ZDbResourceApplyRun)),
  },
  dbBackups: {
    get: pc.input(z.object({ resourceId: ZResourceId })).output(z.object({ resourceId: z.string(), applyId: z.string(), createdAtSec: z.string() }).nullable()),
    discard: pc.input(z.object({ resourceId: ZResourceId, applyId: ZHostId })).route({ method: 'DELETE' }).output(z.object({ discarded: z.boolean() })),
    previewRestore: pc.input(z.object({ resourceId: ZResourceId, applyId: ZHostId })).output(z.object({
      backup: z.object({ resourceId: z.string(), applyId: z.string(), createdAtSec: z.string() }),
      impact: ZDbImpact,
      warning: z.string(),
    })),
    restore: pc.input(z.object({ resourceId: ZResourceId, applyId: ZHostId })).output(ZDbResourceApplyRun),
    restoreStatus: pc.input(z.object({ restoreId: ZHostId })).output(ZDbApplyDetails),
  },
});
