...
export type { TAgentEvent } from '#backend/shell/events/types';
...
const ZAgentChatHistoryItem: z.ZodType<TAgentChatHistoryItem> = z.object({
  entryId: z.string().min(1).max(200),
  message: z.json(),
}).strict();

const ZAgentChatConnect: z.ZodType<TAgentChatConnect> = z.object({
  vcJson: ZWidgetManifestV1.nullable(),
  messageHistory: z.array(ZAgentChatHistoryItem).max(100_000),
}).strict();

const ZAgentEvent: z.ZodType<TAgentEvent> = z.union([
  z.object({
  ...
  z.object({
    kind: z.literal('widgetupdate'),
    widgetId: z.string().min(1).max(200),
    sessionId: z.string().min(1).max(200),
    cwd: z.string().max(4_096),
    files: z.array(z.string().max(4_096)).max(20_000),
  }).strict(),
  z.object({
    kind: z.literal('approval'),
    widgetId: z.string().min(1).max(200),
    sessionId: z.string().min(1).max(200),
    type: z.enum(['created', 'resolved', 'canceled']),
    approval: ZAgentApproval,
    decision: z.enum(['approve', 'reject']).optional(),
    reason: z.string().max(2_000).optional(),
  }).strict(),
  z.object({
    kind: z.literal('widget-catalog'),
    type: z.literal('changed'),
  }).strict(),
]);

const ZSequencedAgentEvent = z.object({
  sequence: z.number().int().positive(),
}).passthrough().superRefine((value, context) => {
  const { sequence: _sequence, ...event } = value;
  const result = ZAgentEvent.safeParse(event);
  if (!result.success) {
    context.addIssue({
      code: 'custom',
...
export const agentContract = pc.router({
  settings: {
    get: pc
  ...
  chat: {
    ...
    dbChange: {
      approve: pc.input(ZAgentChatScope.extend({
        proposalId: z.string().min(1),
        confirmedRisk: z.literal(true),
      })).output(ZAgentChatDbChangeProposal),
      reject: pc.input(ZAgentChatScope.extend({ proposalId: z.string().min(1) })).output(ZAgentChatDbChangeProposal),
    },
    approval: {
      list: pc.input(ZAgentChatScope).output(ZAgentApproval.array()),
      get: pc.input(ZAgentChatScope.extend({ approvalId: z.string().min(1) })).output(ZAgentApproval.nullable()),
      resolve: pc.input(ZAgentChatScope.extend({
    ...
    cancel: pc.input(ZAgentChatScope).output(ZAgentChatCancel),
    newSession: pc.input(ZAgentChatScope).output(z.null()),
  },
  approval: {
    list: pc.input(ZAgentChatScope).output(ZAgentApproval.array()),
    get: pc.input(ZAgentChatScope.extend({ approvalId: z.string().min(1) })).output(ZAgentApproval.nullable()),
  ...
  auth: {
    ...
    abort: pc.input(z.object({ loginId: z.string() })).output(z.null()),
    apiKey: {
      set: pc
  ...
  events: pc
    ...
    .route({ method: 'GET' })
    .output(eventIterator(ZSequencedAgentEvent)),
});
