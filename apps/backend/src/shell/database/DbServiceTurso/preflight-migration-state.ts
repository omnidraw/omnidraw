...
import { verifyDatabaseSchemaContract } from './verify-database-schema-contract';
...
async function preflightMigrationState(
  effects: TEffects,
  args: TArgs,
  ...
  const [applicationIdRow, userVersionRow, tableRows, schemaObjectRows] = await Promise.all([
    (await effects.db.prepare(DATABASE_STATEMENTS.migrationStateReadPragmaApplicationId)).get() as Promise<Record<string, unknown> | undefined>,
    (await effects.db.prepare(DATABASE_STATEMENTS.migrationStateReadPragmaUserVersion)).get() as Promise<Record<string, unknown> | undefined>,
    (await effects.db.prepare(DATABASE_STATEMENTS.migrationStateReadPragmaTableList)).all() as Promise<TPragmaTableListRow[]>,
    (await effects.db.prepare(DATABASE_STATEMENTS.migrationStateReadSqliteSchema)).all() as Promise<TSchemaObjectRow[]>,
  ]);
  ...
  let schemaContract;
  try {
    schemaContract = await verifyDatabaseSchemaContract(
  ...
  try {
    ledgerRows = await (
      await effects.db.prepare(DATABASE_STATEMENTS.migrationStateReadSchemaMigrations)
...
export { preflightMigrationState };
