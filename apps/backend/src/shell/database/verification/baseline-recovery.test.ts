...
describe('single-user managed database preflight and recovery', () => {
  test('requires an explicit application version at construction', () => {
    expect(() => new DbServiceTurso({
      applicationVersion: '   ',
      databasePath: ':memory:',
      dataDir: '/tmp',
      cacheDir: '/tmp',
    })).toThrow('applicationVersion must not be empty');
  });
  ...
  test('starts a fresh home with exactly one baseline and directly preflights the resulting database', async () => {
    ...
    expect(await (await service.db.prepare(`
      SELECT version, name, application_version FROM schema_migrations ORDER BY version
    `)).all()).toEqual([expect.objectContaining({
      version: 0,
      name: '000-initial.sql',
  ...
  test('rejects current-schema drift without durable DB or sidecar mutation', async () => {
    const root = await temporaryRoot('schema-drift');
    const home = path.join(root, 'home');
    await mkdir(path.join(home, 'cache'), { recursive: true });
    const databasePath = path.join(home, 'main.db');
    const source = serviceFor(home);
    await source.start();
    await source.db.exec(`ALTER TABLE canvases ADD COLUMN unexpected_value TEXT;`);
  ...
  test('rejects orphan coordinators and wrong-type or symlink entries without mutation', async () => {
