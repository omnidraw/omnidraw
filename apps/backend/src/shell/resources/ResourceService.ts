...
import type { TDb } from '#backend/shell/database/DbServiceTurso/DbServiceTurso';
...
class ResourceService {
  ...
  constructor(config: TResourceServiceConfig) {
    ...
    const cryptoEffects = config.crypto;
    this.#crypto = cryptoEffects;
    this.#nowMs = config.nowMs;
    this.#managementWriteCapability = cryptoEffects.randomUUID();
    ...
    const secretStoreKeyProvider = new SecretStoreDatabaseKeyProvider({
      ...
      randomUUID: cryptoEffects.randomUUID.bind(cryptoEffects),
    });
    this.#kvResource = new KvResource(new ResourceKeyValueStore({
      ...
      nowMs: this.#nowMs,
      scheduleIdleSweep: config.scheduleIdleSweep,
    }));
