...
import { DATABASE_STATEMENTS } from '../../database/statement-registry';
import type {
...
const RESOURCE_METADATA_SCHEMA_SQL = DATABASE_STATEMENTS.resourceKvCreateMetadataTable;
const RESOURCE_ENTRIES_SCHEMA_SQL = DATABASE_STATEMENTS.resourceKvCreateEntriesTable;
const RESOURCE_UPDATED_AT_TRIGGER_SQL = DATABASE_STATEMENTS.resourceKvCreateUpdatedAtTrigger;

const RESOURCE_KEY_VALUE_PRAGMA_STATEMENTS = Object.freeze([
...
async function applyResourceKeyValuePragmas(
  database: Database,
  queryTimeout: number,
): Promise<void> {
  for (const statement of RESOURCE_KEY_VALUE_PRAGMA_STATEMENTS) {
    await database.exec(statement, { queryTimeout });
  }
}

async function createResourceKeyValueSchema(
  database: Database,
  queryTimeout: number,
): Promise<void> {
  await database.exec(DATABASE_STATEMENTS.resourceKvCreateMetadataTable, { queryTimeout });
  await database.exec(DATABASE_STATEMENTS.resourceKvCreateEntriesTable, { queryTimeout });
  await database.exec(DATABASE_STATEMENTS.resourceKvCreateUpdatedAtTrigger, { queryTimeout });
}
...
export class ResourceKeyValueStore implements IResourceKeyValuePersistence {
  ...
  async #provision(resourceIdValue: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        await applyResourceKeyValuePragmas(database, this.#queryTimeoutMs);
        await createResourceKeyValueSchema(database, this.#queryTimeoutMs);
        await (
          await database.prepare(DATABASE_STATEMENTS.resourceKvInsertMetadata)
  ...
  async has(args: { readonly resourceId: string; readonly key: string }): Promise<boolean> {
    ...
    return this.#track(resourceId, this.#withHandle(resourceId, async (database) => {
      const row = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvReadEntryPresent)
      ).get(args.key);
  ...
  async count(args: { readonly resourceId: string; readonly prefix?: string; readonly search?: string }): Promise<number> {
    ...
    return this.#track(resourceId, this.#withHandle(resourceId, async (database) => {
      ...
      const row = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvCountEntries)
  ...
  async list(args: {
    ...
    return this.#track(resourceId, this.#withHandle(resourceId, async (database) => {
      ...
      const rows = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvListEntries)
  ...
  async listMetadata(args: {
    ...
    return this.#track(resourceId, this.#withHandle(resourceId, async (database) => {
      ...
      const cursor = args.cursor ?? null;
      const rows = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvListEntryMetadata)
      ).all(prefix, prefix, prefix, search, search, cursor, cursor, limit + 1);
  ...
  async set(args: { readonly resourceId: string; readonly key: string; readonly value: TJson }): Promise<TResourceKeyValueEntry> {
    ...
    return this.#scheduleWrite(resourceId, async () => {
      await this.#withHandle(resourceId, async (database) => {
        await (
          await database.prepare(DATABASE_STATEMENTS.resourceKvUpsertEntry)
  ...
  async delete(args: {
    ...
    return this.#scheduleWrite(resourceId, () => this.#withHandle(resourceId, async (database) => {
      const result = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvDeleteEntry)
    ...
    return this.#scheduleWrite(resourceId, async () => {
      const changes = await this.#withHandle(resourceId, async (database) => {
        const result = args.expectedRevision === null
          ? await (
            await database.prepare(DATABASE_STATEMENTS.resourceKvInsertEntryIfAbsent)
          ).run(args.key, serialized)
          : await (
            await database.prepare(DATABASE_STATEMENTS.resourceKvUpdateEntryIfRevision)
  ...
  async #verifyDatabase(database: Database, resourceId: string, formatVersion: number): Promise<void> {
    const health = await (
      await database.prepare(DATABASE_STATEMENTS.databaseCheckReadPragmaQuickCheck)
    ).all();
    ...
    const metadata = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadMetadata)
    ).all() as TMetadataRow[];
    ...
    await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvValidateEntryShape)
    ).all();
    const metadataColumns = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadMetadataTableInfo)
    ).all() as TTableInfoRow[];
    const entryColumns = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadEntriesTableInfo)
    ).all() as TTableInfoRow[];
    ...
    const tableList = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadTableList)
    ).all() as TTableListRow[];
    ...
    const schemaObjects = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadSchemaObjects)
  ...
  async #convertPlaintextDatabase(resourceId: string, databaseHexKey: string): Promise<void> {
    ...
    try {
      await source.exec(DATABASE_STATEMENTS.pragmaCheckpointWalTruncate, {
      ...
      try {
        ...
        await applyResourceKeyValuePragmas(destination, this.#queryTimeoutMs);
        await createResourceKeyValueSchema(destination, this.#queryTimeoutMs);
        await (
          await destination.prepare(DATABASE_STATEMENTS.resourceKvInsertMetadata)
        ).run(resourceId, 'secretStore', SECRET_STORE_FORMAT_VERSION);
        await this.#conversionCheckpoint('temporary-created', resourceId);
        await destination.exec(DATABASE_STATEMENTS.transactionBeginImmediate, {
          queryTimeout: this.#queryTimeoutMs,
        });
        try {
          await this.#copyEntries(source, destination);
          await destination.exec(DATABASE_STATEMENTS.transactionCommit, {
            queryTimeout: this.#queryTimeoutMs,
          });
          await this.#conversionCheckpoint('entries-copied', resourceId);
        } catch (error) {
          await destination.exec(DATABASE_STATEMENTS.transactionRollback, {
            queryTimeout: this.#queryTimeoutMs,
          }).catch(() => undefined);
          throw error;
        }
        await destination.exec(DATABASE_STATEMENTS.pragmaCheckpointWalTruncate, {
          queryTimeout: this.#queryTimeoutMs,
        });
  ...
  async #copyEntries(source: Database, destination: Database): Promise<void> {
    ...
    const insert = await destination.prepare(DATABASE_STATEMENTS.resourceKvInsertCopyEntry);
    while (true) {
  ...
  async #copyEntryPage(database: Database, cursor: string | null): Promise<readonly {
    ...
    const rows = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadCopyPage)
  ...
  async #assertEncryptedAccessRejected(
    ...
    for (const options of [
      ...
      try {
        await database.connect();
        await (
          await database.prepare(DATABASE_STATEMENTS.resourceKvReadResourceIdentity)
        ).get(resourceId);
      } catch {
        accessRejected = true;
      } finally {
  ...
  async #getEntryFromDatabase(
    ...
    const row = await (
      await database.prepare(DATABASE_STATEMENTS.resourceKvReadEntry)
    ).get(key);
  ...
  #getEntryMetadata(resourceId: string, key: string): Promise<TResourceKeyValueEntryMetadata | null> {
    return this.#withHandle(resourceId, async (database) => {
      const row = await (
        await database.prepare(DATABASE_STATEMENTS.resourceKvReadEntryMetadata)
      ).get(key);
