...
} from '../../database/statement-registry';
...
const DB_RESOURCE_PRAGMA_STATEMENTS = Object.freeze([
  DATABASE_STATEMENTS.pragmaSetForeignKeysOn,
  DATABASE_STATEMENTS.pragmaSetJournalModeWal,
  DATABASE_STATEMENTS.pragmaSetBusyTimeout,
  DATABASE_STATEMENTS.pragmaSetSynchronousFull,
  DATABASE_STATEMENTS.pragmaSetTempStoreMemory,
]);

async function applyDbResourcePragmas(database: Database): Promise<void> {
  for (const statement of DB_RESOURCE_PRAGMA_STATEMENTS) {
    await database.exec(statement, { queryTimeout: QUERY_TIMEOUT_MS });
  }
}

function applySqliteDbResourcePragmas(database: SQLiteDatabase): void {
  for (const statement of DB_RESOURCE_PRAGMA_STATEMENTS) database.exec(statement);
}
...
export class DbResource implements ILocalResourceProvider {
  ...
  constructor(config: TDbResourceConfig) {
    ...
    this.#scheduleIdleSweep = config.scheduleIdleSweep;
  ...
  async provision(resource: TDbProviderResource, _args: TResourceProviderCreateArgs): Promise<void> {
    ...
    try {
      ...
      await database.exec(DATABASE_STATEMENTS.dbResourceCreateApplyMarkerTable);
  ...
  async listRows(args: { resourceId: string; object: string; cursor?: TDbRowIdentity | null; limit?: number }): Promise<TDbRowsPage> {
    ...
    return this.#withDatabase(this.#databasePath(resourceId), true, async (database) => {
      ...
      const sql = renderDatabaseStatement('dbResourceReadBrowseRows', {
        __SELECT_COLUMNS__: selectColumns.join(', '),
        __TABLE__: quoteIdentifier(object.name),
        __CURSOR__: cursor.sql,
        __ORDER__: order || '1',
        __LIMIT__: String(limit + 1),
      });
  ...
  async getRow(args: { resourceId: string; object: string; identity: TDbRowIdentity; columns?: readonly string[] }): Promise<TDbRow> {
    ...
    return this.#withDatabase(this.#databasePath(resourceId), true, async (database) => {
      ...
      try {
        nativeRows = await this.#queryNative(database, renderDatabaseStatement('dbResourceReadRow', {
          __COLUMNS__: visibleColumns.map(quoteIdentifier).join(', '),
          __TABLE__: quoteIdentifier(object.name),
          __IDENTITY__: identity.sql,
        }), identity.parameters, RESULT_COLUMN_MAX_COUNT, ROW_HYDRATION_BYTES_MAX);
      } catch (error) {
  ...
  createRow(args: { resourceId: string; object: string; values: Readonly<Record<string, TDbCellValue>> }) {
    return this.#rowWrite(args.resourceId, async (database) => {
      ...
      const sql = columns.length === 0
        ? renderDatabaseStatement('dbResourceInsertDefaultRow', {
          __TABLE__: quoteIdentifier(object.name),
        })
        : renderDatabaseStatement('dbResourceInsertRow', {
          __TABLE__: quoteIdentifier(object.name),
          __COLUMNS__: columns.map(quoteIdentifier).join(', '),
          __VALUES__: databaseParameterPlaceholders(columns.length),
        });
  ...
  updateRow(args: { resourceId: string; object: string } & Omit<TDbRowUpdate, 'kind'>) {
    return this.#rowWrite(args.resourceId, async (database) => {
      ...
      const result = await this.#runNative(database, renderDatabaseStatement('dbResourceUpdateRow', {
        __TABLE__: quoteIdentifier(object.name),
        __ASSIGNMENTS__: names.map((name) => `${quoteIdentifier(name)} = ?`).join(', '),
        __IDENTITY__: identity.sql,
        __EXPECTED__: optimistic.sql,
      }), [...names.map((name) => values[name]), ...identity.parameters, ...optimistic.parameters]);
  ...
  deleteRow(args: { resourceId: string; object: string } & Omit<TDbRowDelete, 'kind'>) {
    return this.#rowWrite(args.resourceId, async (database) => {
      ...
      const identity = this.#identityPredicate(object, args.identity);
      const optimistic = this.#expectedPredicate(expected);
      const result = await this.#runNative(database, renderDatabaseStatement('dbResourceDeleteRow', {
        __TABLE__: quoteIdentifier(object.name),
        __IDENTITY__: identity.sql,
        __EXPECTED__: optimistic.sql,
      }), [...identity.parameters, ...optimistic.parameters]);
      ...
      for (const operation of args.operations) {
        if (operation.kind === 'create') {
          ...
          const result = names.length === 0
            ? await this.#runNative(database, renderDatabaseStatement('dbResourceInsertDefaultRow', {
              __TABLE__: quoteIdentifier(object.name),
            }), [])
            : await this.#runNative(database, renderDatabaseStatement('dbResourceInsertRow', {
              __TABLE__: quoteIdentifier(object.name),
              __COLUMNS__: names.map(quoteIdentifier).join(', '),
              __VALUES__: databaseParameterPlaceholders(names.length),
  ...
  async createDraft(resourceIdValue: string, draftIdValue: string): Promise<void> {
    ...
    try {
      ...
      await this.#serializeWrite(resourceId, async () => {
        ...
        await live.exec(DATABASE_STATEMENTS.pragmaCheckpointWalTruncate, { queryTimeout: QUERY_TIMEOUT_MS });
        await this.#closeHandle(resourceId);
  ...
  async applyDraftChange(draftIdValue: string, operation: TDbDraftOperation): Promise<TDbDraftChangeEvidence> {
    ...
    const prepared = await this.#withDatabase(path, false, async (database) => {
      ...
      const baselineForeignKeys = await this.#foreignKeyViolations(database);
      await database.exec(DATABASE_STATEMENTS.pragmaCheckpointWalTruncate, { queryTimeout: QUERY_TIMEOUT_MS });
    ...
    return this.#withDatabase(path, false, async (database) => {
      ...
      const apply = database.transaction(async (): Promise<TDbDraftChangeEvidence> => {
        ...
        await this.#runNative(database, DATABASE_STATEMENTS.dbResourceInsertDraftEvidence, [Number(sequence), 'structure', sql]);
        await this.#verifyDatabase(database, baselineForeignKeys);
      ...
      try {
        ...
        if (rebuild) await database.exec(DATABASE_STATEMENTS.pragmaSetForeignKeysOn).catch(() => undefined);
      }
    });
  }
    ...
    try {
      ...
      try {
        ...
        const apply = database.transaction(() => {
          ...
          if (!Number.isSafeInteger(sequence) || Number(sequence) < 1) throw new ResourceError('DB_RESOURCE_DRAFT_INVALID', 'Draft change evidence sequence is invalid.');
          database.exec(sql);
          database.query(DATABASE_STATEMENTS.dbResourceInsertDraftEvidence).run(Number(sequence), 'structure', sql);
          evidence = { sequence: Number(sequence), kind: 'structure', sql };
        });
        apply();
        if (rebuild) database.exec(DATABASE_STATEMENTS.pragmaSetForeignKeysOn);
      } finally {
        database.close();
      }
  ...
  async executeDraftSql(
    ...
    return this.#withDatabase(this.#draftDatabasePath(draftId), false, async (database) => {
      ...
      const apply = database.transaction(async (): Promise<TDbDraftChangeEvidence> => {
        ...
        await this.#runNative(database, DATABASE_STATEMENTS.dbResourceInsertDraftEvidence, [Number(sequence), 'sql', sql]);
        await this.#verifyDatabase(database, baselineForeignKeys);
  ...
  async listDraftChangeEvidence(draftIdValue: string): Promise<TDbDraftChangeEvidence[]> {
    ...
    return this.#withDatabase(this.#draftDatabasePath(draftId), true, async (database) => {
      const rows = await this.#queryNative(database, DATABASE_STATEMENTS.dbResourceListDraftEvidence, []);
        ...
        try {
          ...
          if (orderedChanges.some((change) => requiresCombinedTableOptionsCompatibility(change.sql))) {
            ...
            const transaction = database.transaction(async () => {
              ...
              await database.exec(DATABASE_STATEMENTS.dbResourceCreateApplyMarkerTable);
              await database.run(DATABASE_STATEMENTS.dbResourceInsertApplyMarker, applyId, { queryTimeout: QUERY_TIMEOUT_MS });
            });
            try {
              await transaction();
            } finally {
          ...
          const database = await this.#open(resourceId, true);
          await this.#verifyDatabase(database, baselineForeignKeys);
          const marker = await this.#queryNative(database, DATABASE_STATEMENTS.dbResourceReadApplyMarker, [applyId]);
  ...
  #applyChangesWithSqlite(
    ...
    try {
      ...
      const apply = database.transaction(() => {
        ...
        database.exec(DATABASE_STATEMENTS.dbResourceCreateApplyMarkerTable);
        database.query(DATABASE_STATEMENTS.dbResourceInsertApplyMarker).run(applyId);
      });
      apply();
      if (rebuild) database.exec(DATABASE_STATEMENTS.pragmaSetForeignKeysOn);
    } finally {
    ...
    try {
      ...
      await this.#serializeWrite(resourceId, async () => {
        ...
        const markRestore = database.transaction(async () => {
          await database.exec(DATABASE_STATEMENTS.dbResourceCreateApplyMarkerTable);
          await database.run(DATABASE_STATEMENTS.dbResourceInsertApplyMarker, restoreId, { queryTimeout: QUERY_TIMEOUT_MS });
        });
        await markRestore();
      });
    } catch {
  ...
  async hasApplyMarker(resourceIdValue: string, applyIdValue: string): Promise<boolean> {
    ...
    try {
      const rows = await this.#withDatabase(this.#databasePath(resourceId), true, (database) => this.#queryNative(
        database,
        DATABASE_STATEMENTS.dbResourceReadApplyMarker,
        [applyId],
      ));
        ...
        if (recoveryId !== null) {
          try {
            ...
            if (options.restoreSourceApplyId) {
              ...
              const markRestore = database.transaction(async () => {
                await database.exec(DATABASE_STATEMENTS.dbResourceCreateApplyMarkerTable);
                await database.run(DATABASE_STATEMENTS.dbResourceInsertApplyMarkerIfAbsent, applyId, { queryTimeout: QUERY_TIMEOUT_MS });
              });
              await markRestore();
            }
  ...
  async #inspectDatabase(database: Database): Promise<TDbObject[]> {
    const schemaRows = await this.#queryNative(
      database,
      DATABASE_STATEMENTS.dbResourceInspectObjects,
      [INSPECTION_OBJECT_MAX + 1],
    );
    ...
    for (const schemaRow of schemaRows) {
      ...
      const columnRows = await this.#pragmaNative(database, renderDatabaseStatement(
        'schemaContractReadTableInfo',
        { __TABLE_IDENTIFIER__: quoteIdentifier(name) },
      ));
      ...
      if (kind === 'table') {
        const indexRows = await this.#pragmaNative(database, renderDatabaseStatement(
        ...
        for (const indexRow of indexRows) {
          ...
          const indexSchema = await this.#queryNative(database, DATABASE_STATEMENTS.dbResourceReadIndexSchema, ['index', indexName]);
          const indexColumnRows = await this.#pragmaNative(database, renderDatabaseStatement(
            'dbResourceReadIndexXinfo',
            { __INDEX_IDENTIFIER__: quoteIdentifier(indexName) },
          ));
      ...
      const foreignRows = kind === 'table' ? await this.#pragmaNative(database, renderDatabaseStatement(
      ...
      const triggers = (await this.#queryNative(database, DATABASE_STATEMENTS.dbResourceReadTriggers, [name]))
  ...
  async #structuredSql(database: Database, operation: TDbDraftOperation): Promise<string> {
    if (operation.kind === 'createTable') {
      ...
      return `${renderDatabaseStatement('dbResourceCreateTable', {
        __TABLE__: quoteIdentifier(operation.table),
        __DEFINITIONS__: definitions.join(', '),
        __OPTIONS__: tableOptionsSql({ strict, withoutRowid: operation.withoutRowid ?? false }),
      })};`;
    }
    if (operation.kind === 'renameTable') return `${renderDatabaseStatement('dbResourceRenameTable', {
      __TABLE__: quoteIdentifier(operation.table),
      __NEW_TABLE__: quoteIdentifier(operation.newName),
    })};`;
    if (operation.kind === 'dropTable') return `${renderDatabaseStatement('dbResourceDropTable', {
      __TABLE__: quoteIdentifier(operation.table),
    })};`;
    if (operation.kind === 'addColumn') {
      ...
      return `${renderDatabaseStatement('dbResourceAddColumn', {
        __TABLE__: quoteIdentifier(operation.table),
        __COLUMN__: columnSql(operation.column),
      })};`;
    }
    if (operation.kind === 'renameColumn') return `${renderDatabaseStatement('dbResourceRenameColumn', {
      __TABLE__: quoteIdentifier(operation.table),
      __COLUMN__: quoteIdentifier(operation.column),
      __NEW_COLUMN__: quoteIdentifier(operation.newName),
    })};`;
    if (operation.kind === 'dropColumn') return `${renderDatabaseStatement('dbResourceDropColumn', {
      __TABLE__: quoteIdentifier(operation.table),
      __COLUMN__: quoteIdentifier(operation.column),
    })};`;
    if (operation.kind === 'createIndex') {
      ...
      return `${renderDatabaseStatement('dbResourceCreateIndex', {
        __UNIQUE_PREFIX__: operation.unique ? 'UNIQUE ' : '',
        __INDEX__: quoteIdentifier(operation.name),
        __TABLE__: quoteIdentifier(operation.table),
        __COLUMNS__: operation.columns.map(quoteIdentifier).join(', '),
      })};`;
    }
    if (operation.kind === 'dropIndex') return `${renderDatabaseStatement('dbResourceDropIndex', {
      __INDEX__: quoteIdentifier(operation.name),
    })};`;
  ...
  async #rebuildTableSql(database: Database, operation: Extract<TDbDraftOperation, { kind: 'alterColumn' | 'createForeignKey' | 'dropForeignKey' }>): Promise<string> {
    ...
    return [
      `${renderDatabaseStatement('dbResourceDropTableIfExists', {
        __TABLE__: quoteIdentifier(temporary),
      })};`,
      `${renderDatabaseStatement('dbResourceCreateTable', {
        __TABLE__: quoteIdentifier(temporary),
        __DEFINITIONS__: definitions.join(', '),
        __OPTIONS__: tableOptionsSql(originalOptions),
      })};`,
      `${renderDatabaseStatement('dbResourceCopyTableRows', {
        __TARGET_TABLE__: quoteIdentifier(temporary),
        __TARGET_COLUMNS__: targets.map(quoteIdentifier).join(', '),
        __SOURCE_COLUMNS__: targets.map((name) => quoteIdentifier(sourceByTarget.get(name) ?? name)).join(', '),
        __SOURCE_TABLE__: quoteIdentifier(object.name),
      })};`,
      `${renderDatabaseStatement('dbResourceDropTable', {
        __TABLE__: quoteIdentifier(object.name),
      })};`,
      `${renderDatabaseStatement('dbResourceRenameTable', {
        __TABLE__: quoteIdentifier(temporary),
        __NEW_TABLE__: quoteIdentifier(object.name),
      })};`,
      indexes,
      triggers,
    ].filter(Boolean).join('\n');
  ...
  async #foreignKeyViolations(database: Database): Promise<Set<string>> {
    try {
      const tableRows = await this.#queryNative(
        database,
        DATABASE_STATEMENTS.dbResourceReadUserTables,
        [],
      );
      ...
      for (const tableRow of tableRows) {
        ...
        const foreignRows = await this.#pragmaNative(database, renderDatabaseStatement(
          'schemaContractReadForeignKeyList',
          { __TABLE_IDENTIFIER__: quoteIdentifier(table) },
        ));
        ...
        for (const [foreignKeyId, foreignKey] of foreignKeys) {
          ...
          if (parentExists && parts.some((part) => part.parentColumn === null)) {
            ...
            if (!primaryKey) {
              primaryKey = (await this.#pragmaNative(database, renderDatabaseStatement(
                'schemaContractReadTableInfo',
                { __TABLE_IDENTIFIER__: quoteIdentifier(foreignKey.parentTable) },
              )))
          ...
          if (parentExists) {
            ...
            if (!eligibleKeys) {
              ...
              const parentSchema = await this.#queryNative(
                database,
                DATABASE_STATEMENTS.dbResourceReadTableSchema,
              ...
              for (const indexRow of await this.#pragmaNative(database, renderDatabaseStatement(
                'schemaContractReadIndexList',
                { __TABLE_IDENTIFIER__: quoteIdentifier(foreignKey.parentTable) },
              ))) {
                if (Number(indexRow.unique) !== 1 || Number(indexRow.partial) === 1 || typeof indexRow.name !== 'string') continue;
                const indexColumns = (await this.#pragmaNative(database, renderDatabaseStatement(
                  'dbResourceReadIndexXinfo',
                  { __INDEX_IDENTIFIER__: quoteIdentifier(validateIdentifier(indexRow.name)) },
                )))
                  .filter((row) => Number(row.key) === 1)
          ...
          const parentMatch = parentExists
            ...
            : '';
          const commonMarkers = {
            __SELECTED__: selected.join(', '),
            __CHILD_TABLE__: quoteIdentifier(table),
            __CHILD_VALUES_PRESENT__: childExpressions
              .map((expression) => `${expression} IS NOT NULL`)
              .join(' AND '),
            __CHILD_EXPRESSIONS__: childExpressions.join(', '),
          };
          const sql = parentExists
            ? renderDatabaseStatement('dbResourceFindForeignKeyViolations', {
              ...commonMarkers,
              __PARENT_TABLE__: quoteIdentifier(foreignKey.parentTable),
              __PARENT_MATCH__: parentMatch,
            })
            : renderDatabaseStatement('dbResourceFindMissingParentTableViolations', commonMarkers);
  ...
  async #verifyDatabase(database: Database, baselineForeignKeys: ReadonlySet<string>): Promise<void> {
    const health = await this.#pragmaNative(
      database,
      DATABASE_STATEMENTS.databaseCheckReadPragmaQuickCheck,
    );
  ...
  async #runWithDatabase<T>(
    ...
    try {
      ...
      if (readonly) {
        await database.exec(DATABASE_STATEMENTS.pragmaSetQueryOnly, {
          queryTimeout: QUERY_TIMEOUT_MS,
        });
        const queryOnly = await this.#pragmaNative(
          database,
          DATABASE_STATEMENTS.pragmaReadQueryOnly,
        );
        ...
        await applyDbResourcePragmas(database);
      }
