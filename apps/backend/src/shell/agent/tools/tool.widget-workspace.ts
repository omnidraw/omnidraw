...
import { validateWidgetFiles } from './validate-widget-files';
      ...
      try {
        return await args.workspace.withDraftAuthoringOperation(params.name, async () => {
          const created = await args.workspace.createDraft(args.chatId, params, async ({ cwd, name, description, template, server }) => {
            ...
            const files = await writeWidgetScaffold({ mkdir, writeFile, join }, {
              cwd,
              manifest,
            ...
            const validation = await validateWidgetFiles({ readdir, readFile, writeFile, rm, execFile, join, relative }, { cwd });
            if (!validation.ok) {
