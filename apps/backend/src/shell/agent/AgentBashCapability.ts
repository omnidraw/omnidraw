...
export type TAgentBashProcessEffects = Readonly<{
  env: NodeJS.ProcessEnv;
...
const DEFAULT_PROCESS_PORTAL: TAgentBashProcessEffects = Object.freeze({
  env: process.env,
...
export function createBunAgentBashCapability(
  effects: TAgentBashProcessEffects = DEFAULT_PROCESS_PORTAL,
) {
  return {
    async run(args: TAgentBashRunArgs): Promise<TAgentBashResult> {
      const startedAtMs = Date.now();
      const bashPath = effects.which('bash');
      if (bashPath === null) {
      ...
      const terminal = effects.createTerminal({
        cols: 120,
        rows: 40,
        name: 'xterm-256color',
        data(data) {
      ...
      try {
        process = effects.spawn([bashPath, '-lc', args.command], {
          cwd: args.cwd,
          env: effects.env,
          signal: args.signal,
          timeout: timeoutMs,
