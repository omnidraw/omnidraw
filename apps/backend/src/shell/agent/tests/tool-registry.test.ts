...
describe('AI Chat tool registry', () => {
  ...
  test('fences one mounted draft once after multi-file and failed Bash mutations', async () => {
    ...
    await workspace.init();
    const cwd = await workspace.ensureChat(CHAT_ID);
    await workspace.createDraft(CHAT_ID, { name: 'Bash Clock' }, async ({ cwd: draftCwd }) => {
    ...
    await workspace.createDraft(CHAT_ID, { name: 'Second Clock' }, async ({ cwd: draftCwd }) => {
      await mkdir(join(draftCwd, 'ui'), { recursive: true });
      await writeFile(
        join(draftCwd, 'omnidraw.json'),
        `${JSON.stringify({
    ...
    expect(await workspace.inspectMounts(CHAT_ID)).toEqual([
