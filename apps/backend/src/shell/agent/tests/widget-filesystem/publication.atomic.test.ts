...
} from '../../widget-filesystem/publication';
...
type TTestEffects = TPublicationEffects & {
...
function createEffects(): TTestEffects {
  ...
  const effects: TTestEffects = {
    ...
    async lstat(path) {
      ...
      const override = effects.deviceOverride?.(path, value.dev) ?? value.dev;
      return {
        dev: override,
        size: value.size,
    ...
    async observeFence() {
      return effects.fence;
    },
    async validateReopenedPublication({ slug, path }) {
      return validateFolder(slug, path, effects.rejectCurrentVersion, effects.rejectManifestName);
    },
    ...
    async onTransition(event) {
      ...
      if (effects.pauseTransition?.(event)) await effects.pauseTransition(event);
      if (effects.failTransition?.(event)) throw new Error(`Injected ${event.timing}:${event.transition}`);
    },
  };
  return effects;
}
...
describe('atomic filesystem widget publication', () => {
  ...
  test('writes release.json last, syncs, reopens, and atomically installs one complete folder', async () => {
    ...
    const result = await publishAtomicPublication(
      effects,
    ...
    const stageWrites = effects.writes.filter((path) => path.includes('.stage/'));
    expect(stageWrites.at(-1)).toEndWith('/release.json');
    expect(effects.transitions.filter((event) => event.transition === 'stage-reopen-validation')).toHaveLength(2);
    expect(effects.transitions.filter((event) => event.transition === 'current-reopen-validation')).toHaveLength(2);
  ...
  test('holds an in-process reader across the two-rename gap and exposes only the new folder', async () => {
    ...
    const effects = createEffects();
    const barrier = new PublicationReadWriteBarrier();
    await publishAtomicPublication(effects, publicationInput(root, barrier, 'v1', 'publish-v1'));
    ...
    const publishing = publishAtomicPublication(
      effects,
  ...
  test('rechecks draft and catalog digests immediately before replacement', async () => {
    ...
    await expect(publishAtomicPublication(
      effects,
  ...
  test('rejects a stage whose observed device differs from the pinned widget root', async () => {
    const root = await createRoot();
    const effects = createEffects();
    effects.deviceOverride = (path, device) => path.endsWith('.stage') ? Number(device) + 1 : device;
    await expect(publishAtomicPublication(
      effects,
  ...
  test('allows only one concurrent cross-process-style wx writer lock', async () => {
    ...
    const first = publishAtomicPublication(
      effects,
  ...
  test('preserves an earlier root-barrier poison across an unrelated successful publication', async () => {
    ...
    await publishAtomicPublication(effects, publicationInput(root, barrier, 'v1', 'other-widget'));
    expect(await publicationVersion(join(root, 'published', 'counter'))).toBe('v1');
    expect(barrier.isPoisoned()).toBe(true);
    await expect(barrier.withRead(() => 'entered')).rejects.toBe(priorGap);
  });

  test('runs through the production confined no-follow Node effects adapter', async () => {
  ...
  test('production compare-remove atomically claims the old lease and never unlinks a raced replacement', async () => {
    ...
    const lease = await acquireWidgetRootWriterLease(filesystem, {
    ...
    await lease.release();
    const raced = await readPublicationWriterLock(filesystem, {
      widgetRoot: filesystem.rootPath,
    });
    expect(raced?.serialized).toBe(replacement);
    expect(raced?.record.ownerToken).toBe('raced-owner');
    await clearStalePublicationWriterLock(filesystem, {
      widgetRoot: filesystem.rootPath,
  ...
  test('never guesses stale locks from PID or age and clears only exact explicitly confirmed bytes', async () => {
    ...
    const observed = await readPublicationWriterLock(effects, { widgetRoot: root });
    ...
    await expect(clearStalePublicationWriterLock(effects, {
      widgetRoot: root,
      operationToken: 'clear-stale',
      expectedSerializedLock: `${serialized}changed`,
      confirmation: 'explicitly-confirmed-no-live-writer',
    })).rejects.toMatchObject({ code: 'STALE_LOCK_CHANGED' });
    expect(await readPublicationWriterLock(effects, { widgetRoot: root })).not.toBeNull();
  ...
  test('faults around critical transitions never expose a partial current folder', async () => {
    ...
    for (const [index, transition] of faultPoints.entries()) {
      ...
      const effects = createEffects();
      const barrier = new PublicationReadWriteBarrier();
      await publishAtomicPublication(effects, publicationInput(root, barrier, 'v1', `base-${index}`));
      let injected = false;
      effects.failTransition = (event) => {
...
describe('publication restart recovery', () => {
  test('restores exactly one validated replaced folder and never promotes the staged replacement', async () => {
    ...
    await publishAtomicPublication(effects, publicationInput(root, oldBarrier, 'v1', 'base-v1'));
    ...
    effects.failTransition = null;
    const recovery = await recoverAtomicPublications(effects, {
      widgetRoot: root,
  ...
  test('quarantines an invalid interrupted current and restores its one validated predecessor', async () => {
    ...
    await publishAtomicPublication(effects, publicationInput(root, barrier, 'v2', 'interrupted-v2'));
    ...
    effects.rejectCurrentVersion = 'v2';
  ...
  test('poisons readers when an invalid interrupted current cannot be quarantined', async () => {
    ...
    await expect(recoverAtomicPublications(effects, {
      widgetRoot: root,
...
describe('checked metadata-only publication', () => {
  test('atomically replaces only omnidraw.json and retains every executable byte', async () => {
    ...
    const writeIndex = effects.writes.length;

    const result = await publishWidgetMetadata(effects, {
      widgetRoot: root,
      slug: 'counter',
      operationToken: 'metadata-v2',
      lockOwnerToken: 'metadata-owner',
      expectedFence: effects.fence,
    ...
    const metadataWrites = effects.writes.slice(writeIndex).filter((path) => !path.endsWith('.writer.lock'));
  ...
  test('poisons readers when an invalid metadata replacement cannot roll back', async () => {
    ...
    const barrier = new PublicationReadWriteBarrier();
    await publishAtomicPublication(effects, publicationInput(root, barrier, 'v1', 'base-v1'));
    effects.rejectManifestName = 'Bad Metadata';
