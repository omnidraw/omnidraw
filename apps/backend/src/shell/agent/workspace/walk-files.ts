...
export async function walkFiles(effects: TEffectsWalkFiles, args: TArgsWalkFiles): Promise<string[]> {
  const current = args.current ?? args.root;
  const atRoot = current === args.root;
  const entries = (await effects.readdir(current, { withFileTypes: true }))
    .toSorted((left, right) => (
  ...
  for (const entry of entries) {
    const absPath = effects.join(current, entry.name);
    if (entry.isDirectory()) {
      if (isExcludedDirectory(entry.name, atRoot)) continue;
      files.push(...await walkFiles(effects, { root: args.root, current: absPath }));
      continue;
    }

    if (entry.isFile()) {
      files.push(effects.relative(args.root, absPath));
    }
  }
