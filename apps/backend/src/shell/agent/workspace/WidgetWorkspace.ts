...
import { readWidgetManifestRecord } from './read-draft-manifest';
import { readWidgetCatalog } from './read-widget-catalog';
import { ensureChatStorage } from './ensure-chat-storage';
import type {
...
export class WidgetWorkspace {
  ...
  async ensureChat(chatId: string): Promise<string> {
    const storage = await ensureChatStorage({ join, mkdir, readFile, writeFile }, {
  ...
  async #readDraftIdentity(
    ...
    const manifest = await readWidgetManifestRecord({ lstat, readFile, join }, { draftPath });
