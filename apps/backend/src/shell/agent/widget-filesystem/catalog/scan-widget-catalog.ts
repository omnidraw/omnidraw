...
type TManifestFacts = Readonly<{
  ...
  presentation: ReturnType<TEffects['contracts']['projectPresentation']> | null;
  presentationDigestSha256: string | null;
  executable: ReturnType<TEffects['contracts']['projectExecutable']> | null;
...
function selectedText(
  ...
  return effects.filesystem.decodeUtf8({ bytes });
}

function digestText(effects: TEffects, value: string): string {
  return digestValue(effects, value);
}

function digestValue(effects: TEffects, value: string | Uint8Array): string {
  const digest = effects.hash.digestSha256({ value });
  if (!/^[0-9a-f]{64}$/.test(digest)) {
    throw new TypeError('Widget catalog hash effects returned an invalid SHA-256 digest.');
  }
  return digest;
}
...
function manifestFacts(
  ...
  try {
    const text = effects.filesystem.decodeUtf8({ bytes });
    const manifest = effects.contracts.parseManifestJson(text);
    ...
    const digestSha256 = (value: string) => digestText(effects, value);
    return {
      manifest,
      manifestDigestSha256: effects.contracts.manifestDigest({ manifest, digestSha256 }),
      presentation: effects.contracts.projectPresentation(manifest),
      presentationDigestSha256: digestSha256(
        effects.contracts.canonicalizePresentation(manifest),
      ),
      executable: effects.contracts.projectExecutable(manifest),
      executableManifestDigestSha256: effects.contracts.executableManifestDigest({
        manifest,
        digestSha256,
      }),
    };
...
async function scanTree(
  ...
  while (queue.length > 0 && !stopped) {
    ...
    try {
      observation = await effects.filesystem.readDirectory(args.root, {
    ...
    for (const entry of [...observation.entries].sort((left, right) => (
      ...
      const normalized = effects.contracts.normalizeRelativePath(widgetPath);
      ...
      try {
        const bytes = await effects.filesystem.readFile(args.root, {
          relativePath: `${widgetRoot}/${normalized}`,
        ...
        files.push(Object.freeze({
          ...
          sha256: digestValue(effects, bytes),
        }));
        if (
  ...
  for (const observation of observations.reverse()) {
    try {
      await effects.filesystem.assertDirectoryUnchanged(args.root, {
        observation,
        maxEntries: args.limits.maxEntriesPerDirectory,
      });
    } catch (error) {
...
async function scanDraft(
  ...
  const tree = args.initialIssues === undefined
    ...
    : Object.freeze({
        ...
        treeDigestSha256: digestValue(effects, '[]'),
        issues: args.initialIssues,
      });
  const issues = [...tree.issues];
  const facts = manifestFacts(effects, { form: 'draft', slug: args.slug, tree, issues });
...
async function scanPublished(
  ...
  const issues = [...tree.issues];
  const facts = manifestFacts(effects, { form: 'published', slug: args.slug, tree, issues });
  ...
  if (releaseBytes === undefined) {
    ...
    releaseDescriptorDigestSha256 = digestValue(effects, releaseBytes);
    try {
      const text = selectedText(effects, tree, WIDGET_CATALOG_RELEASE_PATH);
      if (text === null) throw new TypeError('release.json exceeds the catalog text limit.');
      release = effects.contracts.parseReleaseJson(text);
    } catch (error) {
      issues.push(fnWidgetCatalogIssue({
        scope: 'published',
  ...
  let inspectedCapsule: Awaited<ReturnType<TEffects['capsule']['inspectCapsuleArtifact']>> | null = null;
  if (facts.manifest !== null && capsuleBytes !== undefined && release !== null) {
    try {
      inspectedCapsule = await effects.capsule.inspectCapsuleArtifact({
        bytes: capsuleBytes,
        ...
        canonicalUnsignedReleaseJson: effects.contracts.canonicalizeUnsignedRelease(release),
  ...
  if (
    ...
    releaseValidation = effects.contracts.validateRelease({
...
export async function scanPublishedWidgetFolder(
  effects: TEffects,
...
async function scanForm(
  ...
  const observation = await effects.filesystem.readDirectory(args.root, {
...
export async function scanWidgetCatalog(
  effects: TEffects,
  args: TArgs,
  ...
  const limits = fnResolveWidgetCatalogScanLimits(args.limits);
  await effects.filesystem.assertRoot(args.root, {});
  const layoutObservation = await effects.filesystem.readDirectory(args.root, {
    relativePath: '',
