...
export type TPublicationEffects = Readonly<{
...
export type TNodeWidgetPublicationFilesystemHooks = Readonly<{
  observeFence: TPublicationEffects['observeFence'];
  validateReopenedPublication: TPublicationEffects['validateReopenedPublication'];
  /** Must strictly parse the manifest and recompute its executable digest. */
  validateMetadataCandidate: TPublicationEffects['validateMetadataCandidate'];
  onTransition?: TPublicationEffects['onTransition'];
...
export type TClearStaleWriterLockInput = Readonly<{
  widgetRoot: string;
  operationToken: string;
  /** Exact bytes returned by `readPublicationWriterLock`. */
