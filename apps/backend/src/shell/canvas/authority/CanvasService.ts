...
} from '#backend/core/canvas/fn.command';
import type { TCommandLimits } from '#backend/core/canvas/fn.command';
import { fnReduceCanvasCommandItems } from '#backend/core/canvas/fn.reduce-command';
import { CanvasAuthorityError } from '#backend/core/canvas/errors';
  ...
  async #executeSerialized(
    ...
    if (prior !== null) {
      return {
        ...
        changedItems: prior.changedItems,
        deletedItemIds: prior.deletedItemIds,
      };
    }
    ...
    for (let attempt = 0; attempt < this.#options.maxCommitAttempts; attempt += 1) {
      ...
      let itemReduction;
      try {
        itemReduction = fnReduceCanvasCommandItems({
          command,
          items: [...original.values()].filter(
            (item): item is TCanvasItemSnapshot => item !== null,
          ),
        });
      } catch (error) {
        if (error instanceof CanvasAuthorityError) {
          throw new CanvasServiceError(
            error.code === 'CONFLICT' ? 'CONFLICT' : 'INVALID_COMMAND',
            error.message,
            error.details,
            { cause: error },
          );
      ...
      const attemptContext: TAttemptContext = {
        command,
        state,
        original,
        finalItems: new Map(itemReduction.changes.map((change) => [change.id, change.item])),
        changedIds: new Set(itemReduction.changes.map((change) => change.id)),
        authorityIds,
      };
