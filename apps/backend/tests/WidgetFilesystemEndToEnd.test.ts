...
import { CanvasService } from '#backend/shell/canvas/authority';
import {
...
function publishedWidgetNode(args: Readonly<{
  id: string;
...
function runtimeTarget(args: Readonly<{
  canvasId: string;
  element: TWidgetFrameNode;
}>) {
  const extension = fnReadCanvasWidgetExtension(args.element);
...
async function openDatabase(path: string): Promise<Database> {
  ...
  const migration = await Bun.file(new URL(
    '../src/shell/database/migrations/000-initial.sql',
    import.meta.url,
  )).text();
...
describe('clean-home filesystem widget integration', () => {
  test('loads exact mount inputs and preserves state without turning the database into catalog authority', async () => {
    ...
    const element = publishedWidgetNode({
      id: ELEMENT_ID,
      parentId: null,
      orderKey: 'a',
      position: { x: 10, y: 20 },
    ...
    const target = runtimeTarget({ canvasId: CANVAS_ID, element });
