...
} from '../src/shell/preview/fn.validate-browser-job';
import type {
...
describe('preview inspection browser job validation', () => {
  ...
  test('rejects unsupported roles and private runtime-descriptor identity drift', () => {
    expect(fnValidatePreviewInspectionBrowserJob(job({
      actions: [{
        type: 'click',
  ...
  test('rejects the retired capsuleArtifactHash wire field', () => {
    const current = job().artifact;
    ...
    }))).toMatchObject({ ok: false, code: 'BROWSER_JOB_INVALID' });
    expect(fnValidatePreviewInspectionBrowserJob(job({
      artifact: {
        ...current,
        capsuleArtifactHash: current.artifactHash,
