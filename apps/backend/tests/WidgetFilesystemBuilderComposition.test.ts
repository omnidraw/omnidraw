...
describe('production filesystem widget builder composition', () => {
  test('publishes UI-only and bun-v1 server widgets through one catalog service', async () => {
    ...
    try {
      const { widgetBuildGeneration, widgetCatalog } = await runtime.runPromise(
        Effect.gen(function*() {
          return {
            widgetBuildGeneration: yield* LiveWidgetBuildGeneration,
            widgetCatalog: yield* LiveWidgetCatalog,
          };
        }),
      );
