...
import { fnMutableRegistryPackageLock } from '../src/shell/widget/fn.mutable-registry-package-lock';
  ...
  test('drops integrity only for Omnidraw packages from the mutable registry', () => {
    const source = `${JSON.stringify({
      ...
      packages: {
        ...
        'node_modules/tool/node_modules/@omnidraw/canvas-contract': {
          resolved: 'http://127.0.0.1:4873/@omnidraw/canvas-contract/-/canvas-contract-0.12.0.tgz',
    ...
    expect(
      refreshed.packages['node_modules/tool/node_modules/@omnidraw/canvas-contract'].integrity,
    ).toBeUndefined();
