...
import {
  ...
  type TAgentBashProcessEffects,
} from '../src/shell/agent/AgentBashCapability';
...
function createCapturingEffects(terminals: Bun.Terminal[]): TAgentBashProcessEffects {
  return {
    env: process.env,
...
describe('Bun Agent Bash capability', () => {
  test('starts in the exact cwd while preserving traversal, combined PTY output, UTF-8, and TTY behavior', async () => {
    ...
    const capability = createBunAgentBashCapability(createCapturingEffects(terminals));
    const command = [
      'printf "stdout-line\\n"',
      'printf "stderr-line\\n" >&2',
      '[ -t 1 ] && printf "TTY_TRUE\\n"',
