...
import { fnBuildHomePreflightError } from '../src/core/cli/fn.home-preflight-error';

const HOME_DIR = '/tmp/omnidraw-home';
...
describe('fnBuildHomePreflightError', () => {
  test('surfaces a nested multiprocess WAL conflict with lock-specific guidance', () => {
    ...
    const error = new Error(
      `Refusing to open Omnidraw database after a read-only preflight failed: ${HOME_DIR}/main.db`,
      { cause },
    );
    ...
    expect(fnBuildHomePreflightError({ homeDir: HOME_DIR, error })).toEqual({
      ...
      message: `Refusing selected Omnidraw home '${HOME_DIR}': ${cause.message}`,
  ...
  test('keeps archive guidance for unknown schema refusals without legacy actor wording', () => {
    const result = fnBuildHomePreflightError({
      homeDir: HOME_DIR,
      error: new Error('Refusing to open Omnidraw database: found a non-empty unknown database.'),
    });
