...
import { checkWidgetPrerequisites } from '../src/shell/cli/widget-prerequisites/check-widget-prerequisites';
import type { TExecFile } from '../src/shell/cli/widget-prerequisites/interface';
...
describe('widget startup prerequisites', () => {
  test('requires npm but no container engine', async () => {
    ...
    await expect(checkWidgetPrerequisites(value.effects, {
      command: 'serve',
      helpRequested: false,
  ...
  test('warns without blocking startup when npm is missing', async () => {
    ...
    const result = await checkWidgetPrerequisites(value.effects, {
      command: 'serve',
      helpRequested: false,
      versionRequested: false,
    });
