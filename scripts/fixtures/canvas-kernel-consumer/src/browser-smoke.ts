...
async function runComposition(
  ...
  try {
    ...
    await page.locator('.vc-canvas-host').waitFor({ state: 'visible' })
    await page.waitForFunction(() => window.canvasKernelSmoke.status().activeSubscriptions === 1)
    ...
    const engineHost = page.locator('.vc-canvas-engine-host')
    ...
    await page.locator('.vc-selection-style-menu').waitFor({ state: 'visible' })
    ...
    const beforeTheme = await page.locator('.vc-canvas-host').getAttribute('data-theme-id')
    await page.evaluate(() => window.canvasKernelSmoke.switchTheme())
    await page.waitForFunction((previous) => (
      document.querySelector('.vc-canvas-host')?.getAttribute('data-theme-id') !== previous
    ), beforeTheme)
    const themeEvidence = await page.locator('.vc-canvas-host').evaluate((element) => ({
...
console.log('[canvas-kernel-consumer] memory/no-extension and fake-Cell/host-contribution browser smokes passed')
