...
> **Status note:** Everything here is **shipped** in the 2.0 prerelease line. This document collects the server-rendering entry points and the HTTP exchange surface of `@solidjs/web` — pieces that previously lived in SolidStart or in scattered subpath lore. The per-primitive hydration policy (`ssrSource` / `deferStream`) is documented with async data in [RFC 05](llm.05-async-data.md); `clientOnly` with control flow in [RFC 03](llm.03-control-flow.md); server functions and server components in [RFC 10](llm.10-server-functions.md) / [RFC 11](llm.11-server-components.md).
