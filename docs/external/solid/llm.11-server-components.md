...
**Start here:** If you’re migrating an app, read the migration guide first: [MIGRATION.md](llm.MIGRATION.md). This RFC builds directly on [10 — Server functions](llm.10-server-functions.md).
...
(`renderToStream`'s result offers `pipe`, `pipeTo`, or a `Response`-body-ready `readable` — exactly one per render; see [12 — SSR and the HTTP exchange](llm.12-ssr-http.md).)
