...
`deep()` and `snapshot()` both return plain objects, but `deep()` sets up deep tracking while `snapshot()` does not. See [RFC 04 — Stores](llm.04-stores.md) for full details.
