...
For hyperscript JSX, set `jsxImportSource` to `@solidjs/h`. Custom renderers should provide their own `jsx-runtime` and `jsx-dev-runtime` type entries. See [RFC 09 — TypeScript and JSX ownership](llm.09-typescript-jsx.md) for the full design.
...
These are **dev-only diagnostics** meant to catch bugs earlier. Some are warnings (console); others are errors (throw). See [RFC 08](llm.08-dev-diagnostics.md) for the full reference.
...
| `mutate()`           | `createOptimisticStore` + `action` (see [RFC 06](llm.06-actions-optimistic.md)) |
...
- **`ssrSource` option** (`"server"` | `"hybrid"` | `"client"`) — per-primitive hydration policy on memos, function-form signals/stores, projections, and effects: whether the client seeds from the serialized server value (`"server"`, default), seeds then re-runs the compute (`"hybrid"`), or skips the server value and computes only after hydration (`"client"` — the compute never runs on the server). Pairs with **`deferStream: true`**, which holds the SSR stream flush until the primitive's first value resolves (server-only). See [RFC 05](llm.05-async-data.md).
- **`renderToStream(...).readable`** — a `Response`-body-ready `ReadableStream` (Uint8Array chunks) alongside the existing `pipe`/`pipeTo`; exactly one consumer per render. See [RFC 12](llm.12-ssr-http.md).
