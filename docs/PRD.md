...
`@omnidraw/capsule` is an exact direct implementation dependency of the SDK, currently `0.15.1`. New widget scaffolds depend on and import `@omnidraw/sdk` only. A Capsule major requires SDK ABI conformance and artifact-compatibility review before the SDK dependency is updated.
...
| `sdk` | `@omnidraw/capsule` | Exact direct dependency, currently `0.15.1`; no Capsule types escape |
