...
- [ ] Add red causal tests for missing CLI/RPC host validation and inspection.
- [ ] Freeze the shared application-private authoring DTO, limits, errors, and generation fences.
- [ ] Extract one shared validation/accepted-build capability and adapt AI Chat to it.
- [ ] Add the automation inspection subject without weakening the existing AI Chat subject.
- [ ] Register exact resolve, validate, and inspect private operations through the authoritative registry.
- [ ] Add the `omnidraw widget` CLI group, strict help/argv/JSON/exit/cancellation behavior, and short-lived RPC client reuse.
- [ ] Add the bounded single-use screenshot lease and atomic verified CLI file output.
- [ ] Build and verify the inspection shell before supported dev backend readiness.
- [ ] Make failed Preview browser preflight safely retryable without restart.
- [ ] Add authority, drift, cancellation, write-denial, redaction, screenshot, and no-Canvas-mutation tests.
- [ ] Run the full acceptance matrix and update widget-system/CLI documentation.
- [ ] Close E43 and update the overview only after the end-to-end external repair loop passes.
