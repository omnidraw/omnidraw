...
> **2026-08-11 B84 product amendment:** the filesystem-first authority remains,
> but a configured installation now stores each exact local `resourceId` in the
> resource declaration in `omnidraw.json`. The logical resource contract stays
> portable and `resourceId` is excluded from executable identity; a configured
> manifest is intentionally installation-specific. There is no canvas-item or
> per-instance binding map, placement picker, Connect/Rebind UI, or silent
> fallback. Draft Preview and FunctionService resolve only the exact accepted
> or published manifest. The older resource-selection passages below are kept
> as historical exploration context and are superseded by this amendment and
> [B84](../b/B84.md).
...
| `resource-contract` | slot, kind, required/effect/operation ceiling | Rebuild and publish |
| `resource-binding` | concrete local `resourceId` only | New portable receipt; reuse executable bytes; explicitly publish configuration |
...
### Canvas/runtime behavior (superseded resource-binding detail)

A committed widget canvas extension is now conceptually:
...
item and catalog generation before mounting. Resource calls resolve only the
current manifest declaration and revalidate its lifecycle and policy; canvas
JSON has no resource authority. Publication invalidation refreshes handles for
that key. Missing/corrupt folders render a local repairable placeholder without
changing geometry or instance state.
...
- [x] **Q44 (amended by B84):** Requirements and concrete local `resourceId`
  references stay in the manifest. Canvas instances carry no resource
  configuration; all instances share the current published reference.
...
- [x] **Q60:** Render a missing-widget placeholder and preserve the canvas item
  and instance state until the folder appears and validates.
