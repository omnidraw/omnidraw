...
An author-maintained `dist/` is not required and should not be part of the normal draft/publish contract. Requiring it would create two source authorities, invite AI Chat to edit generated files, permit stale output, and complicate fingerprints. The project may still own its build command, compiler configuration, and plugins; the captured build output is the promotion authority.
...
- normal package source/bytes resolved with ordinary lifecycle/install scripts;
...
- project-selected TypeScript/JSX/Vue transforms and plugins;
- capability and budget requests;
- actor contract descriptor/hash;
- build-runner identity, provenance, and reliability limits.

The build and server-function processes are operator-trusted host execution,
outside Capsule's browser security boundary. Guest install/build scripts,
project configuration, plugins, and normal host access are permitted. The
default runner may execute on the application host; operators may select Docker
or another isolated runner. CPU, memory, process, file, byte, depth, output, and
wall-clock limits remain valuable for reliability and UX, not as a security
claim. Preview and publication reuse the exact captured build outputs.
...
- Update the Capsule integration references in [`llm.widget-system.md`](../../docs/internal/llm.widget-system.md) from `normalizedManifest.widget` to `normalizedManifest.ui`.
...
E40 fixes the authoring-build constraints used by A96: Capsule is the browser
guest security boundary, while install/build scripts, project plugins, Preview
server functions, and their runner authority are accepted operator trust.
Operators may choose a host or Docker runner. Source maps remain private
sidecar metadata because Capsule external-distribution v1 rejects them, and
exact source/UI/server/contract outputs remain rooted by the Preview frame until
explicit publication or frame-owned cleanup.
...
- 2026-07-28: Imported E40's host-authority runner, private source-map sidecar,
  frame-owned exact Preview promotion, and pinned-instance decisions.
