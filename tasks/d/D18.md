...
- [ ] Add the build-free production `start` command and remove `server:prod`.
- [ ] Make port `7496` the documented default and preserve `--port` forwarding.
- [ ] Print the actual browser URL only after successful server readiness.
- [ ] Preserve and test `--data-dir`, `OMNIDRAW_HOME`, and `~/.omnidraw` behavior.
- [ ] Make `omnidraw/omnidraw` publicly cloneable and standardize all five public package manifests and active links.
- [ ] Choose and enforce a complete Git LFS clone policy with no runtime dependency on documentation assets.
- [ ] Rewrite end-user and contributor documentation around install, build, start, and dev.
- [ ] Add regression coverage preventing retired binary/global-install/update/uninstall release paths from returning.
- [ ] Add clean-clone Linux and macOS release CI, and keep OS claims identical to the passing matrix.
- [ ] Preserve public-lockfile and untracked `.npmrc` gates.
