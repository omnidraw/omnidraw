...
`@omnidraw/service-db` is deliberately excluded from this public set. Managed
deployments must reach its OSS-owned persistence behavior through public
contracts and service/API boundaries rather than installing the package.
...
- [x] Keep `@omnidraw/service-db` private, unversioned, and outside npm release tooling.
...
- Do not use npm unpublish as rollback for supported release packages. The
  explicit retirement of `@omnidraw/service-db` is the exception; removing its
  already-published registry versions remains a manual maintainer action.

### LOGS

- 2026-08-02: Requirement changed: `@omnidraw/service-db` is internal to the
  OSS monorepo and will no longer be released on npm. Restored `private: true`,
  removed its package version, public metadata, staged build and prepublish
  scripts, deleted its release-only build files, and removed it from the D5
  release graph. Version-driven deployment and distribution verification now
  exclude it automatically. Deleted the stale ignored `service-db/dist`
  artifact; the live deployment report built and checked exactly 12 remaining
  versioned packages without Service DB. Its typecheck and all 148 internal
  tests pass.
- 2026-08-02: Changed `bun run deploy:packages:list` to rebuild every
  versioned workspace package sequentially in dependency order before querying
  npm or reporting deployment status. The command now fails immediately on a
  stale or broken package build instead of presenting publish commands for old
  `dist/` contents. The live command rebuilt all 13 release packages
  successfully before reporting 10 current and 3 ready packages.
- 2026-08-02: Kept the resource WAL recovery fixture compatible with Turso
  versions on both sides of the experimental-option typing change by using
  `@ts-ignore` instead of self-invalidating unused `@ts-expect-error`
  directives. The package typecheck passes.
