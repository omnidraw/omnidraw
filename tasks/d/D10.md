...
- [x] Define the portable SDK build CLI and strict receipt schema.
- [x] Capture/stage coherent repository input and compute exact digests.
- [x] Promote generated output and completion receipt atomically.
- [x] Add portable concurrent-build serialization without locking editors.
- [x] Add host watcher plus bounded smart polling/backoff observer.
- [x] Revalidate source/manifest/output/Capsule before accepting a generation.
- [x] Track dirty/building/failed/validating/ready/stale states without a DB table.
- [x] Integrate automatic scaffold build and accepted-generation Preview refresh.
- [x] Require AI check/build workflow after edits.
- [x] Add Config manual Rebuild through the same portable command.
- [x] Fence publication to one current accepted generation.
- [x] Add causal external-edit, receipt, polling, dedup, stale, failure, and shutdown tests.
- [x] Update widget architecture/prompts/UI docs and `FILES.md`.
- [x] Apply required public-package version bumps and regenerate lock/dist.
- [x] Run full tests, packed external composition, builds, lint, and browser acceptance.
...
- 2026-08-11: Implemented `omnidraw-widget build .`, the strict atomic receipt,
  host observation/acceptance state, automatic scaffold builds, manual Rebuild,
  stale-generation publication fences, and last-good Preview preservation.
  The host constructs its own signed Preview authority; acceptance compares
  stable receipt/source/output evidence rather than requiring the portable and
  host full-stack distribution bytes to be identical.
- 2026-08-11: Full monorepo tests, production build, packed public composition,
  functional-core lint, isolated agent scenario, and Capsule browser acceptance
  passed. Public runtime changes are reflected in the version ledger recorded
  by the coordinated B84/A118 work.
