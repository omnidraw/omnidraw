...
- [ ] Introduce separate fenced build-admission and frame-runtime lifecycle states.
- [ ] Start ordinary Preview requests in loading/restoring instead of building.
- [ ] Stop presenting backend restart admission as transpilation.
- [ ] Add the shared accessible loading surface for Preview and published frames.
- [ ] Preserve last-good content while a replacement loads and fill cold frames while waiting.
- [ ] Prove valid cold Preview recovery never calls `rebuildDraft` or `buildPortable`.
- [ ] Clarify that Build and Publish leaves the current Canvas frame as a Preview.
- [ ] Add focused reducer, orchestration, backend recovery, and real-browser restart tests.
- [ ] Update the transient lifecycle descriptions in the screen atlas after implementation.
