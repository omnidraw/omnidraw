...
This is implementation order **2 of 5**. It depends on the shared draft writer
authority in [D10](../d/D10.md) and provides target identity to
[B84](B84.md), [A118](../a/A118.md), and [A119](../a/A119.md).
...
9. Widget mention resolution does not grant resource authority, publication,
   or access to concrete canvas bindings. Those remain separate host checks.
...
- Do not store selected target element IDs yet. [A119](../a/A119.md) owns exact
  existing-instance targeting and must use CanvasService as authority.
...
- keep host paths, release bytes, resource IDs, and catalog internals out of
...
- Complete mounts inside [D10](../d/D10.md)'s authoring boundary so Config or
  deletion cannot change the identity halfway through resolution.
...
- never include absolute paths, concrete resource IDs, secret values, or
  untrusted widget file content;
...
- Multiple refs all enter prompt-local comparison context. They may all be
  mounted readably, but mutation tools still require an exact mounted name and
  no binding reconciliation runs until one target is explicit.
...
- [ ] Carry verified canvas identity through AI-chat connect/create/reconnect.
- [ ] Define and inject the server-side widget-reference resolver.
- [ ] Resolve all refs against one catalog snapshot with typed safe failures.
- [ ] Mount healthy mutable drafts without exposing published folders as source.
- [ ] Track one unambiguous active editable target separately from prompt-local refs.
- [ ] Add the Pi `before_agent_start` selection-context extension.
- [ ] Clear queued context on cancellation and preserve correct branch semantics.
- [ ] Update the system prompt and remove unconditional mention rejection.
- [ ] Add catalog, API, workspace, Pi-context, continuation, multiple-ref, reconnect, and branch regression tests.
- [ ] Update `docs/internal/llm.widget-system.md` and `FILES.md` with the target-resolution trace.
...
- A healthy draft is mounted before the model can call file tools, and every
  structured/Bash mutation composes with [D10](../d/D10.md).
...
- No resource authority, canvas binding mutation, or publication authority is
  granted by this task.

## Non-goals

- Creating concrete resource bindings; [B84](B84.md) owns draft/Preview/new
  placement binding intent and [A119](../a/A119.md) owns existing instances.
...
- Publishing, republishing, or choosing a resource on the user's behalf.
