...
- [ ] Scaffold `apps/widget-debug-tools` (package.json, tsconfig, src)
- [ ] Wire services mirroring setup-services (db, WidgetService, signing keys)
- [ ] Wire WidgetWorkspace + WidgetDraftController + tool factories
- [ ] CLI entry: call any tool by name with JSON args; pretty-print result
- [ ] Reproduce the `npm ci` EUSAGE failure via `lab validate` on existing draft
- [ ] Fix `WidgetNpmDistributionBuild` local `file:` dep staging + lock regen
- [ ] Surface bounded npm stderr in build failure diagnostics
- [ ] Green: lab create+validate loop on fresh widget
- [ ] Green: existing widget/capsule test suites
- [ ] Update FILES.md for new app
