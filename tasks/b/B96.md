...
- [x] Replace direct draft-local build execution with the host-owned private build workspace.
- [x] Make initial create, validate, Rebuild, and Build & Publish share the exact build operation.
- [x] Atomically fence and accept portable output without persistent draft dependencies.
- [x] Preserve last-known-good generations and clean failed/cancelled build workspaces.
- [x] Add a persistent in-frame state for pre-guest Preview failures.
- [x] Add plain/React, local-registry, failure, cancellation, stale-source, and browser regressions.
- [x] Run backend widget/agent/API, frontend sidebar/runtime, Canvas portal, typecheck, and architecture suites.
- [x] Update the Preview/build sections of the widget-system authority and screen atlas if presentation changes.
...
- 2026-08-14: Filed from a live browser and repository investigation. Confirmed
  ...
  the same source.
- 2026-08-14: Replaced the draft-local command with the existing isolated npm
  distribution/Capsule boundary, added exact-source recapture, receipt/output
  validation, atomic `dist/` promotion with crash recovery, shared caller
  coalescing, and last-known-good preservation. Creation, validation, Preview
  **Rebuild**, and **Build & Publish** now converge on that authority.
- 2026-08-14: Added the backend build-state error payload and an accessible
  persistent pre-guest frame surface. The live browser exercised **Build
  required** with **Rebuild**/**Remove**, rebuilt a lockfile-only draft, mounted
  the accepted guest, and confirmed the shared draft still had no
  `node_modules`.
- 2026-08-14: Focused build/scaffold/generation/API/local-registry coverage
  passed (61 tests), the broader backend widget/Preview/API set passed (50),
  frontend unit/conformance passed (123), Canvas passed (169), backend and
  frontend typechecks passed, and architecture/package qualification passed
  (62). The full live browser run passed every B96 and route assertion, then
  hit the existing post-disconnect **Add canvas** dialog timeout; removing the
  B96 scenario produced the identical late timeout, so it is recorded as an
  unrelated reconnect-tail gate failure rather than a B96 regression.
- 2026-08-14: Merge review found two durability gaps. Recovery now treats a
  projected generation as committed only after a durable host marker whose
  live receipt and outputs independently validate; an unmarked crash restores
  `previous-dist`. Projection now recaptures and receipt-fences draft source
  before writing that marker while rollback remains possible. Deterministic
  regressions cover both `liveDist + previous-dist` before verification and
  source drift on the post-projection capture. Focused correction coverage
  passed (53 tests), the complete backend passed (726), architecture passed
  (62), frontend passed (123 plus production build), Canvas passed (169), and
  backend/frontend/Canvas/browser typechecks passed.
