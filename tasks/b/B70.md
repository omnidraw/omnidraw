...
- [x] Add the exact-revision readiness type and pure transition/projection functions.
- [x] Make successful guest mount the only transition to ready.
- [ ] Route Chat, Preview, sidebar, and toast presentation through the shared projection.
- [x] Keep Publish in Manage and gate it by stable frame ownership.
...
- 2026-08-04: Validated against current code. Core bug fixed by A96/A103; task
  remains open with narrowed scope.
  - Durable exact-revision authority exists:
    [`types.ts`](../../packages/service-agent/src/widget-drafts/types.ts:95)
    owner descriptor (status, active/pending revision, build sequence, CAS
    fences) plus `TWidgetPreviewAgentStatus` (attempted vs displayed
    revision); `ready` requires a confirmed mount lease
    ([`WidgetDraftController.ts`](../../packages/service-agent/src/widget-drafts/WidgetDraftController.ts:3057)
    `#previewStatusFor`). A fallback can never satisfy readiness.
  - Chat over-claim fixed by tool contract + prompt:
    [`tool.widget-preview.ts`](../../packages/service-agent/src/tools/tool.widget-preview.ts)
    wait/test keyed to exact revision; `od_widget_validate` reworded to
    `previewExecution: retained-success | not-run`. Enforced by tool
    semantics, not a UI projection (deliberate A103 design).
  - Preview fallback display fixed: candidate mounts hidden, swaps only after
    lease + `ready()` + two frames; failure keeps last good with explicit
    `Build <rev> failed` / `Showing <rev>` status
    ([`PreviewPortalRuntime.ts`](../../packages/ui-ai-chat/src/canvas-extension/PreviewPortalRuntime.ts:1322)).
  - Publish-in-Manage kept, readiness intentionally not a click gate (S133).
  - STILL OPEN: (1) no failure/ready transition toasts - the toast port only
    fires for user-initiated actions
    ([`index.ts`](../../packages/ui-ai-chat/src/canvas-extension/index.ts:435));
    (2) sidebar drops the catalog `preview` summary
    ([`fn.widget-catalog.ts`](../../packages/ui-ai-chat/src/sidebar/widgets/components/fn.widget-catalog.ts:15))
    and shows no failed-current-draft indicator
    ([`WidgetsSidebarSection.tsx`](../../packages/ui-ai-chat/src/sidebar/widgets/components/WidgetsSidebarSection.tsx:140));
    (3) UI surfaces do not consume `TWidgetPreviewAgentStatus` -
    PreviewPortalRuntime derives its own fenced presentation;
    (4) disabled Publish menu item has no accessible instruction
    (index.ts:952); (5) missing toast-once, sidebar-indicator, and
    disabled-Publish pointer/keyboard tests.
