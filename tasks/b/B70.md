...
remain visible, but it must never make a failed newer revision look ready or
publishable.
...
### 4. Keep publication inside Manage and fail closed

- Keep **Publish** only inside the existing Preview **Manage** dropdown created
  by [`S123`](../s/S123.md).
- Derive its disabled state from `state === "ready"` for the exact current
  revision and binding revision.
- Keep **Cancel build** disabled when no cancellable build is active.
- Recheck the authoritative revision server-side when Publish is invoked; a
  stale UI event must not publish an older fallback.
- When disabled, expose an accessible reason such as “Latest revision failed
  Preview.”
...
- [ ] Keep Publish in Manage and gate it by exact ready revision.
...
- Publish remains inside **Manage** and is unavailable unless the exact current
  revision and bindings are live-ready.
