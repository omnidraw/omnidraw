...
- [x] Add `multiprocess_wal` to `preflightDbServiceDatabase()` on-disk open options.
- [x] Surface underlying Turso/cause text in preflight failures.
- [x] Refactor `fnBuildHomePreflightError()` to classify failures and remove the blanket actor-era hint.
- [x] Add service-db preflight regression for an already-open multiprocess WAL holder.
- [x] Add CLI/unit coverage for contextual hint selection.
- [x] Update `scripts/test-binary.ts` home-preflight refusal expectations.
- [x] Run `bun run db:recovery:test` and affected CLI/binary gates.
- [x] Regenerate `FILES.md` if new test/helper files are added.
...
- 2026-07-28: Shared the on-disk Turso feature list between runtime and
  read-only preflight, including `multiprocess_wal`, and exposed native open
  failure text without changing explicit schema refusals.
- 2026-07-28: Added cause-chain-aware CLI guidance for schema/layout, lock, and
  fallback failures plus service, CLI, and binary regressions.
- 2026-07-28: Passed `bun run db:recovery:test`, affected CLI tests,
  functional-core lint, architecture tests, a host binary build, and the full
  compiled-binary acceptance suite. Regenerated `FILES.md`; the new test is
  intentionally excluded by the index generator, so no index diff remained.
