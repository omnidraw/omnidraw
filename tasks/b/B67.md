...
- [x] Add the equal-digest/different-capture cache regression.
- [x] Reproduce validation twice in one live production-wired service graph.
- [x] Reproduce Preview and validation sharing the construction cache in both call orders.
- [x] Define and document content identity versus capture/event identity.
- [x] Repair shared construction provenance without weakening integrity checks.
- [x] Cover every trusted construction-key input and call-count reuse invariant.
- [x] Return safe field-specific integrity diagnostics.
- [x] Exclude dependencies/generated files and bound validation tool output.
- [x] Add persistent isolated session/scenario support to widget-debug-tools.
- [x] Add a lab-owned headless Preview operation backed by the production controller.
- [x] Implement the permanent React-counter author scenario using only widget/file tools.
- [x] Prove cold, warm, unchanged, source-edit, dependency-edit, error, and repair paths.
- [x] Run relevant widget-contract, service-agent, CLI, Capsule, and lab gates.
- [x] Update `llm.widget-system.md`, `FILES.md`, and A96 findings/latency notes.
...
- 2026-07-28: Made new snapshot IDs content-addressed while retaining optional
  capture-event metadata, canonicalized legacy captures at the construction
  cache boundary, and added safe per-field immutable-input diagnostics.
- 2026-07-28: Shared one warm dependency workspace between trusted validation
  and Preview, bounded authored-file summaries, and preserved useful compiler
  output while removing host workspace paths.
- 2026-07-28: Added the isolated persistent JSONL lab, headless controller
  Preview, telemetry/assertions, and the permanent counter scenario. The final
  clean run passed create/read/edit, two unchanged validations, Preview reuse,
  a 442 ms source-only rebuild without install, an explicit dependency install,
  deliberate source failure, repair, and final ready Preview.
- 2026-07-28: Verified exact UI/server Preview promotion publishes the retained
  construction under release signing with one guest build and no publication
  rebuild; focused contract, controller, CLI, Capsule, typecheck, and
  functional-core gates passed.
- 2026-07-28: Replayed authoring in the browser. A more complex React widget
  with a deterministic server function reached validated Preview in 85.8
  seconds versus 217 seconds for the reported UI-only counter. Added
  `template: "react"` and `server: true` create modes so exact React
  dependencies, entry points, the server manifest section, and an editable
  server-function starter are installed/scaffolded once instead of being
  retrofitted through extra reads, edits, installs, and patch retries. The
  exact ready Preview invoked `double({ value: 21 })` through the production
  short-lived function runtime and returned `{ doubled: 42 }`, then published
  that retained Preview as revision
  `56882e5e-efda-4006-99d2-0f33a5bc9829`.
