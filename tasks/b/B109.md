...
- [x] Make chat stream abort on unmount silent (`startStream` / `AiChat`).
- [x] Stop treating `agent.events` drop as lost session; remove or gate
      `connect` in stream recovery / reconnect so a live prompt is not
      `CHAT_BUSY`-failed into a red banner.
- [x] Collapse duplicate reconnect connect paths.
- [x] Make `resumableStream` abort-closed; prove canvas stream abort does not
      imply app socket death.
- [x] Add the three causal tests above. Update chat conformance only if the
...
- 2026-08-17: Stream recovery now rereads history only. `CHAT_BUSY` keeps the
  event iterator listening. Mount/reconnect `connect` that hits a live prompt
  falls back to history instead of a stream banner. `resumableStream` abort
  completes quietly without recovery or generation change.
- 2026-08-17: Verified with frontend recovery/conformance tests, AI Chat
  vitest (96), adapter CHAT_BUSY coverage, frontend and AI Chat typecheck, and
  `git diff --check`. Bumped `@omnidraw/component-ai-chat` to `0.8.2`.
- 2026-08-17: Review fix: `CHAT_BUSY` history fallback is reuse-only so settings
  replace still fails closed. Workspace lockfile now records AI Chat `0.8.2`.
