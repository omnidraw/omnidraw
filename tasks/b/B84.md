...
- [ ] Extend the strict manifest contract with bounded optional `resourceId`.
- [ ] Exclude `resourceId` from executable identity and add binding-only publication.
- [ ] Expose authoritative resource IDs to scoped AI resource authoring safely.
- [ ] Update agent prompts for direct manifest linking and no user picker.
- [ ] Require offline check plus portable build before Preview presents manifest edits.
- [ ] Make Preview resolve only draft-manifest resource references.
- [ ] Remove resource selection from Preview and placement UI/API.
- [ ] Remove canvas-item `resourceBindings` and normalize legacy canvas JSON.
- [ ] Make FunctionService resolve only published-manifest references.
- [ ] Remove branch binding intent, reconciliation, picker, and Connect/Rebind code.
- [ ] Update E41, widget-system docs, prompts, API docs, screen docs, and `FILES.md`.
- [ ] Apply required public-package versions and regenerate lock/dist artifacts.
- [ ] Add causal, migration, redaction, packed-package, and continuous browser tests.
- [ ] Capture final no-picker Preview/placement/reload evidence.
