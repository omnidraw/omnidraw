...
- [x] runtime input gate swallowing key-down/key-up (stopRouting) while maximized or content-focused
- [x] auto-focus maximized portal content host; restore canvas focus on exit
- [x] fix `isWidgetContentTarget` selector to `data-vibecanvas-portal-id`
- [x] runtime + component regression tests (Backspace/letter/nudge leaks, focus)
- [x] verify wheel leak while maximized (gate or document as separate bug)
...
- 2026-08-06: implemented. `runtime.ts` gains a `key-down`/`key-up`/`wheel`
  input gate (`engine.input.subscribe`, subscribed before
  `editorSession.attach()`) that returns `{handled: true, stopRouting: true}`
  while maximized or content-focused; wheel is only gated while maximized
  (content-focused wheel stays with the engine's own per-hit guard). Shell
  transitions now move DOM focus into the maximized widget's portal content
  host on entry and back to the canvas (`engine.input.focus()`) on exit, via
  new pure helpers `fnCanvasInputGateSwallowsKeys`,
  `fnCanvasInputGateSwallowsWheel`, and `fnCanvasShellFocusTransition` in
  `fn.canvas-shell.ts`. Fixed `isWidgetContentTarget` in `Canvas.tsx` to match
  `data-vibecanvas-portal-id`. Added runtime + component regression tests;
  `runtime.test.ts`'s `engine.input` mock now models real multi-subscriber
  dispatch (registration order, `stopRouting`/`stopPropagation` short-circuit)
  since the gate always subscribes alongside the optional trace subscriber.
  `packages/canvas` typecheck/test and `packages/ui-ai-chat` typecheck/test
  all green.
  Found `packages/canvas/package.json` is at `0.6.0` while
  `scripts/architecture-boundaries.test.ts` already pins `@omnidraw/canvas` at
  `0.6.1` (drift from A112, whose LOG claims the bump but the commit never
  touched `package.json`) — `bun test scripts/architecture-boundaries.test.ts`
  fails on `main` independent of this task. Bumping to `0.6.1` here would have
  fixed both this change's own version requirement and that pre-existing
  drift in one move, but the user explicitly said **no version upgrade, keep
  0.6.0** for this task, so `package.json`/`bun.lock` were left untouched and
  the drift is still unresolved — needs its own follow-up.
