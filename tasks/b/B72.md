...
- [x] Reproduce the reported repeated `RESOURCE_MIGRATING` failure end to end.
- [x] Correlate catalog, apply, draft, drain, and physical migration state.
- [x] Prove the missing or rejected terminal transition and its restart behavior.
- [x] Implement exact-generation terminal lifecycle ownership.
- [ ] Add bounded diagnostics without leaking resource contents.
- [x] Add success, failure, interruption, restart, and concurrency regressions.
- [x] Run affected verification and record the root cause and results.
...
- 2026-08-04: Hypothesis confirmed as the deterministic root cause. Live
  `.omnidraw/main.db` shows both db resources (`foo`, `bbb-sql`) stuck at
  catalog status `migrating` while their `db_resource_apply_runs` rows are
  `succeeded`. Three compounding gaps proven:
  1. `ResourceManager.#coordinateResourceMigration`
     ([`ResourceManager.ts`](../../packages/resource-runtime/src/local/ResourceManager.ts)
     lines 999-1035) sets `migrating` (line 1016) but the success path
     (`return await operation(migrating)`, line 1025) never writes `ready`
     back; only the error path writes a terminal state.
  2. `ResourceManager.#reconcileStartup` explicitly skips db rows in
     `migrating` (line 367), delegating to the db coordinator.
  3. `DbResourceCoordinator.reconcileStartup`
     ([`DbResourceCoordinator.ts`](../../packages/resource-runtime/src/local/DbResourceCoordinator.ts)
     lines 469-492) reconciles apply runs and drafts but never touches the
     catalog status.
  The row is orphaned by design: owner A skips it assuming owner B handles it,
  and B does not. Git history shows `ResourceManager.ts` never wrote `ready`
  on the migration success path, so this has existed since the single-owner
  resource runtime landed; it appeared intermittent because startup
  reconciliation and later calls masked it in some sequences. It became
  obvious after B75 made the write tool await the apply. Repair plan:
  (a) CAS the catalog row back to `ready` after `operation(migrating)`
  succeeds, mirroring the provisioning path at line 423;
  (b) make the db coordinator's startup/stale reconciliation also reset
  `migrating` catalog rows whose apply runs are all terminal, closing the
  crash window between physical apply completion and the `ready` write.
- 2026-08-04: Root cause confirmed and fixed. The success path of
  `ResourceManager.#coordinateResourceMigration` wrote `status: "migrating"`
  but never restored `ready`, and no other component transitioned out of
  `migrating` (startup reconciliation explicitly skipped db-migrating rows;
  the coordinator only reconciled apply records). Every successful apply or
  restore therefore wedged the resource and all later calls failed with
  `RESOURCE_MIGRATING`, persisting across restarts. Reproduced with a failing
  regression test (`Expected: "ready", Received: "migrating"`). Fixes:
  (1) success path now compare-and-sets `migrating -> ready` via a new
  optional `expectedStatus` on `IResourceManagerStore.catalog.updateProviderState`;
  (2) `DbResourceCoordinator.#reconcileApply` settles the catalog through the
  new `ResourceManager.settleResourceMigration` (ready on committed/recovered,
  error on unrecoverable), covering interruption, stale-apply, and restart
  recovery; (3) `reconcileStartup` settles resources still `migrating` with no
  remaining active apply, un-wedging already-broken resources. Regression
  tests added in `tests/resource-manager.test.ts` (success admits the next
  call; failure lands on error) and `tests/db-resource-coordinator.test.ts`
  (interrupted apply settles ready; unrecoverable settles error). Verified:
  resource-runtime (44 pass), service-db (126 pass), CLI resource suites
  (19 pass), `test:resource-runtime` script, and package typechecks; the 4
  widget-bundle CLI failures and the OrpcPlugin type error are pre-existing on
  HEAD. `@omnidraw/resource-runtime` bumped to 0.5.4. Bounded-diagnostics work
  remains open as a separate improvement.
