...
- [x] Add the server-function import/binding runtime reproduction.
- [x] Preserve typed Capsule errors through host coordination.
- [x] Add trusted generated-to-`widget://` source mapping.
- [x] Add safe actionable messages and remediation categories.
- [x] Deduplicate console/report emissions by revision and fingerprint.
- [x] Add security, reload, and end-to-end diagnostic regressions.
...
- 2026-08-04: Completed the remaining scope.
  - Browser acceptance now builds a plain DOM Preview that retains a generated
    server-function proxy in a click handler, mounts before interaction, and
    successfully invokes after mount. The coordinator regression preserves the
    rejected capability and operation.
  - [`fn.preview-diagnostic.ts`](../../packages/ui-ai-chat/src/canvas-extension/fn.preview-diagnostic.ts)
    now emits product-owned messages for generated binding, capability denial,
    channel rejection, and unsupported-operation failures, with bounded
    retryability and remediation categories.
  - [`diagnostic-schema.ts`](../../packages/widget-contract/src/diagnostic-schema.ts)
    now admits only the bounded remediation enum. Malformed identifiers,
    arbitrary guest text, host paths, and unsupported remediation strings are
    covered by contract and normalization regressions.
  - Verification passed: widget-contract, ui-ai-chat, service-agent, API,
    external-composition, and full Playwright Capsule browser acceptance tests.
    The repository architecture suite remains blocked by pre-existing
    `@omnidraw/resource-runtime` fixture version drift (workspace `0.5.4`,
    fixture `0.5.2`), unrelated to B71.
