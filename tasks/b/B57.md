...
- [x] Document the commit/cancel policy for every active text state.
- [x] Resolve active text state before dependent services are torn down.
- [x] Guard late editor callbacks and repeated destroy calls.
- [x] Add replacement/unmount tests for new and existing text.
- [x] Run focused text tests and the canvas package suite.
...
- 2026-07-24: Added a pure teardown policy: cancel new-empty, commit new or
  existing meaningful edits, and close unchanged existing text without a
  write. Teardown now resolves pending creations before cleanup, is idempotent,
  blocks late callbacks, and passes 11 focused tests, TypeScript, and
