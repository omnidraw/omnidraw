...
- [ ] Classify server-only paths in SDK `check` with the host builder convention.
- [ ] Keep `@omnidraw/sdk/server` forbidden in UI and shared modules.
- [ ] Add the causal offline-check regression (UI imports `server/main.server.ts`).
- [ ] Patch-bump `@omnidraw/sdk` and regenerate the lockfile.
