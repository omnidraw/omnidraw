...
- [x] Record one canonical server-module ABI and the wrapper-not-rebuild rule
- [x] Update the PRD, widget-system authority, and repository guide
- [x] Mark the old managed Microsandbox function decision as superseded
- [x] Preserve the explicit trusted-local OSS execution decision
- [x] State that widget functions run only in a WFP user Worker, never Sandbox/Containers
- [x] Make OSS and managed accepted widget artifact languages identical
- [x] Define the public SDK versus OSS backend versus managed ownership table
- [x] Add dependency and import-boundary enforcement for cloud/private code
- [x] Record the external managed qualification that OSS cannot perform
...
- 2026-08-17: Implementation started after reading the complete S140-S147
  group and its architecture authorities. Auditing the clean worktree and
  current SDK/backend portability surfaces before the first hard cut.
- 2026-08-17: Recorded the fixed SDK server module, exact-bytes managed wrapper,
  Worker-only invocation, trusted-local OSS and Capsule browser-only split in
  the repository, PRD, widget, and application architecture authorities. Added
  a focused negative dependency/import inventory for public packages and the
  OSS function/local-resource adapters. The focused architecture test passed
  (1 pass); the complete file additionally reported the pre-existing stale
  `packages/sdk/dist/package.json` version (`0.8.1` versus qualified `0.8.4`).
- 2026-08-17: Group closure rebuilt the qualified SDK/package distributions
  and passed the complete architecture suite (67 tests, 707 assertions),
  including the repository-wide managed-cloud/credential negative inventory.
  The managed WFP/Turso live gate remains explicitly external and private.
