...
## Superseded by E40 and A96

This task is a completed historical record of the 2026-07-22 cutover
simplification. The 2026-07-28 product decision in [`E40`](../e/E40.md)
supersedes its UI-only, stateless, rebuild-on-restart outcome.

[`A96`](../a/A96.md) must restore the current Cangine equivalents of durable
frame-owned Preview identity, active Preview revisions, source/UI/server
artifact roots, Preview function subjects and invocation/idempotency state,
real selected resource bindings, leases, and cleanup. The active revision lives
without a fixed TTL for as long as at least one Preview frame owns it, survives
application restart, and is the exact full-stack output promoted by Publish.

The decisions retained from S108 are the useful boundaries that do not conflict:
publication remains explicit, published revisions remain immutable, canvas
persistence remains under CanvasService, published widget-instance state remains
under WidgetStateService, and Capsule remains the browser guest sandbox.
Package install/build scripts and Preview server functions are outside
Capsule's security model and may run on the host or an operator-selected Docker
runner.
...
- 2026-07-28 E40 supersession: A96 restores a durable full-stack authoring model
  owned by the Preview frame. The previous process-owned hard-TTL cache
...
- 2026-07-28: E40 first narrowed the cache wording, then superseded the
  UI-only/stateless result after the product decision changed to frame-lifetime
  full-stack Preview with real bindings.
