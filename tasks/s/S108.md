...
- [ ] Replace revision-owned Preview contracts with one stateless current-draft UI build contract.
- [ ] Make Preview build return verified UI bytes without durable artifact writes.
- [ ] Make Draft Preview server functions/resources explicitly unavailable before Publish.
- [ ] Delete `IWidgetPreviewStore` and Preview persistence from `AgentAuthoringStoreTurso`.
- [ ] Remove Preview roots and activation logic from artifact retention and GC.
- [ ] Remove Preview artifact read capabilities and policy branches.
- [ ] Delete the `agent_preview` function subject and Preview invocation API/service/UI paths.
- [ ] Remove Preview fields/scopes/indexes/FKs from function invocations and idempotency records.
- [ ] Remove get/close/CAS/expiry/cleanup lifecycle from `WidgetDraftController` and agent API.
- [ ] Simplify Draft Preview canvas payload, mounting, refresh, placement, and deletion.
- [ ] Preserve current-draft Publish/Republish confirmation independently from Preview bytes.
- [ ] Delete all three Preview SQL tables and update schema expectations/fixtures.
- [ ] Remove obsolete Preview retention/configuration/tests and add stateless-build coverage.
- [ ] Update widget architecture docs, SCREENS.md wording, package guidance, and FILES.md where product/source files change.
- [ ] Run the complete affected verification matrix.
