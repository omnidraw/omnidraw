...
- [ ] scope: `@vibecanvas/*` → `@omnidraw/*` everywhere (package.json, imports, filters, tests)
- [ ] npmrc: drop `@vibecanvas` registry line
- [ ] folders: apps/vibecanvas, packages/capsule-vibecanvas, vibecanvas-config renamed
- [ ] binary: npm package names + `bin` command renamed to omnidraw
- [ ] runtime: `~/.omnidraw` home dir + resolve-home function/file rename
- [ ] runtime: `VIBECANVAS_*` → `OMNIDRAW_*` env vars
- [ ] data: `vibecanvas:widget` → `omnidraw:widget` JSONB key in SQL + code
- [ ] ui: `.vibecanvas*` DOM class names renamed
- [ ] agent: `vc_*` → `od_*` tool names + prompts + tests
- [ ] registry: remove repo-package publishing (VIBECANVAS_PACKAGES, publish-vibecanvas, registry:publish:vibecanvas script)
- [ ] registry: widget npm userconfig routes `@omnidraw/sdk` to npmjs
- [ ] ci: workflows renamed, `omnidraw-v*` tag scheme, npm availability checks, codesign/entitlements
- [ ] install: scripts/install.sh + apps/web/public/install.sh rewritten
- [ ] web: domain, astro site, content URLs, SEO/sitemap
- [ ] docs: README, AGENTS.md, docs/internal, FILES.md regenerated
- [ ] verify: full test suite + build + test-binary green; grep shows no stray vibecanvas
