...
- [x] Capture import, manifest, packed-output, CSS, asset, and test baselines.
- [x] Add failing permanent gates for every known workspace-split violation.
- [x] Add the minimal `TCanvasDescriptor` to canvas-contract.
- [x] Move `TCanvasDocumentTransport` to canvas-contract and document stream cancellation.
- [x] Version and make canvas-contract independently publishable.
- [x] Remove service-theme cross-package source imports and private tapable dependency.
- [x] Export a stable theme service capability and keep isolated instances.
- [x] Version and make service-theme independently publishable.
- [x] Introduce the explicit canvas dependency bundle and stable host scope key.
- [x] Inject ID, retry/wait, diagnostics, image, notification, transport, and theme behavior.
- [x] Move OSS browser tenant/storage helpers out of canvas.
- [x] Replace database canvas metadata with the public descriptor.
- [x] Remove sidebar and AI semantics from base canvas props/runtime/UI.
- [x] Add narrow host toolbar/tool contributions preserving AI Chat and sidebar UX.
- [x] Scope canvas CSS and remove all shell-global resets.
- [x] Package canvas fonts/assets and remove root-relative asset assumptions.
- [x] Build standard ESM/declarations/CSS assets with Solid as a peer.
- [x] Version and make canvas independently publishable.
- [x] Add one OSS canvas composition root and keep the canvas page thin.
- [x] Add a reusable in-memory document transport.
- [x] Add a distinct fake managed-Cell transport adapter.
- [x] Add and pack-install a clean external browser consumer with no path aliases.
- [x] Run real-browser smoke against in-memory and fake-Cell compositions.
- [x] Extend architecture, packed-package, release, and final-acceptance gates.
- [x] Update package docs and regenerate `FILES.md` for code/config changes.
- [x] Run and log all focused, repository-wide, build, browser, and diff checks.
...
- 2026-07-31: Published the workspace-split boundary locally as independently
  built `@omnidraw/canvas-contract@0.5.0`,
  `@omnidraw/service-theme@0.5.0`, and `@omnidraw/canvas@0.5.0`. Their public
  manifests use exact release dependencies, deliberate `dist` exports/files,
  package-owned licenses and documentation, and package-local build,
  typecheck, test, and prepublish gates.
- 2026-07-31: Replaced canvas database, tenant, shell, global ID/timer,
  diagnostics, global CSS, and root-font assumptions with the public
  descriptor/dependency bundle, host scope, cancelable wait, diagnostics
  owner, scoped host styles, and packaged fonts. The OSS frontend now owns one
  explicit composition root and narrow AI Chat/sidebar contributions while
  preserving the `C` and `Ctrl+B` shortcuts.
- 2026-07-31: Added the standalone packed canvas-kernel consumer with
  in-memory and distinct fake-Cell transports. A clean install outside the
  repository resolved one Solid runtime, typechecked, passed 2 transport
  tests, production-built, and passed both real-browser smokes, including
  rectangle creation/editing, event delivery, theme/CSS/font loading,
  diagnostics ownership, cancellation, and clean unmount.
- 2026-07-31: Final focused package gates passed: canvas-contract 11 tests,
  service-theme 6 tests, canvas 19 files / 116 tests, frontend 43 Bun tests and
  6 Vitest tests, plus frontend typecheck/build. The architecture/integration
  suite passed 17 tests / 426 assertions; the existing packed public
  composition and widget/canvas regression suites also passed.
- 2026-07-31: Final repository verification passed with
  `bun install --frozen-lockfile`, the complete `bun run test`,
  `bun run lint:functional-core`, the four-platform `bun run build`, both clean
  packed-consumer gates, and `git diff --check`. Tenant switching additionally
  has an exact deferred-retirement race test proving every canvas lifecycle is
  disposed before the previous transport disconnects. Regenerated `FILES.md`.
