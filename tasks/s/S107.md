...
- [ ] Make service-agent manifests, tools, validation, Preview, publication, and catalog strictly v2-only.
- [ ] Delete service-agent legacy actor capability, methods, prompts, lints, fixtures, and tests.
- [ ] Remove the SDK actor subpath, proxy, types, generated declarations, and examples.
- [ ] Rename actor-shaped generic resource models and move catalog access to neutral stores.
- [ ] Remove actor-definition binding/status APIs while preserving revision resource references.
- [ ] Simplify database-resource apply to invocation drain/fencing without actor stop/restart recovery.
- [ ] Remove legacy actor canvas elements, metadata, cloning, adapters, and host branches.
- [ ] Delete ui-actor-legacy and frontend legacy capability startup.
- [ ] Remove actor-only widget detail tabs and state-machine UI.
- [ ] Delete the actor API domain and actor/draft-actor event streams.
- [ ] Remove CLI actor configuration, IPC mode, plugin, pool, health diagnostics, and composition.
- [ ] Delete service-actor and all package/dependency/build/test references.
- [ ] Drop all five legacy_actor tables and actor DB repositories/models/tests from the initial schema.
- [ ] Update embedded migrations, expected schema, recovery, constraints, and binary fixtures.
- [ ] Delete compatibility matrices/baselines and rewrite retained behavioral coverage against v2 functions.
- [ ] Update active docs, package guidance, SCREENS.md, FILES.md, and architecture guards.
- [ ] Run the complete relevant verification matrix and record results below.
