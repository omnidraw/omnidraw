...
- [x] Make durable active revisions and source artifacts the sole published authority.
- [x] Materialize editable drafts exclusively from verified source artifacts.
- [x] Remove `WidgetWorkspace.publishedRoot` and directory-backed published operations.
- [x] Remove installed-folder reconciliation and obsolete publication backup roots.
- [x] Replace legacy/v2 catalog merging with one revision-backed catalog.
- [x] Reduce `WidgetManagement` to draft-folder management.
- [x] Remove `published-legacy` placement descriptors and consumers.
- [x] Remove compatibility parsing, fallbacks, fixtures, and tests.
- [x] Audit and remove `artifacts/widgets/` if it has no non-compatibility runtime owner.
- [x] Update architecture docs, package guidance, comments, and `FILES.md`.
- [x] Add restart, corruption, draft-materialization, catalog, deletion, and placement coverage.
- [x] Run relevant tests, typechecks, functional-core lint, and frontend build.
...
- 2026-07-22: Implemented the clean cutover. Published catalog/detail/files/deletion/placement now resolve only active database revisions and verified source artifacts; editing materializes a draft from that artifact. Removed published/installed workspace roots, reconciliation, backups, fallbacks, legacy placement descriptors, and mutable `artifacts/widgets` ownership. Updated architecture guidance, product docs, tests, and `FILES.md`.
- 2026-07-22: Verified widget-contract (59), service-agent task coverage (52 before an unrelated Bun port-0 bind failure), CLI widget/publication coverage within 122 passing tests, UI AI Chat (286), API (49), frontend (36), canvas (214), service-db/schema/recovery (129), and Automerge (107); all changed-package typechecks, functional-core lint, frontend build, and web build passed.
