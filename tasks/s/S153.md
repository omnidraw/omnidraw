...
- [`@kobalte/core`](https://www.npmjs.com/package/%40kobalte%2Fcore?activeTab=versions)
  ...
  `2.0.0-rc.0` exactly.
- [`@solidjs/vite-plugin`](https://www.npmjs.com/package/%40solidjs%2Fvite-plugin?activeTab=versions)
  uses its `3.0.0-next.*` line for the Solid-2 compiler; the former
  `vite-plugin-solid` name is only a forwarding wrapper on that line.
...
`@kobalte/core@2.0.0-alpha.0`, `@solidjs/vite-plugin@3.0.0-next.31`, and
`babel-preset-solid@2.0.0-rc.0`. Tags must not enter manifests or the lockfile.
...
- Because Canvas public runtime/generated output and its peer contract change,
  reserve the next unpublished breaking `0.x` release. Prior runtime work had
  already consumed Canvas `0.9.0` and AI Chat `0.2.0`, so this cut uses Canvas
  `0.10.0` and AI Chat `0.3.0`. Update README compatibility,
  `public-package-set.json`, and the lockfile. Never republish an existing
  version.
...
- [x] Record the green Solid 1 baseline and add stale-Solid-1 source/dist guards
- [x] Select and pin one exact coherent Solid 2 core/web/compiler/router family and primitive strategy
- [x] Update root, frontend, Canvas, AI Chat, Vitest, inspection, and fixture dependency/config graphs
- [x] Remove Solid-1-only devtools and replace `lucide-solid` with bounded Solid-2-native icons
- [x] Migrate renderer/store imports, JSX type ownership, contexts, DOM events, classes, and lifecycle APIs
- [x] Split every migrated effect into pure compute and side-effect apply phases
- [x] Redesign frontend store persistence and audit every imperative setter/read boundary
- [x] Apply the minimum coordinated Solid 2 migration to the AI Chat package used by frontend
- [x] Update component tests and add dev-diagnostic, deferred-write, event-root, and cleanup regressions
- [x] Update public package qualification, packed fixtures, declarations, release markers, and lockfile
- [x] Pass focused package, frontend, conformance, architecture, packed-consumer, and live browser gates
- [x] Record final versions, commands, browser evidence, and any approved behavior change in this task log
...
- 2026-08-20: Pinned one exact reachable graph: `solid-js`,
  `@solidjs/signals`, `@solidjs/web`, and `babel-preset-solid` at
  `2.0.0-rc.0`; Router `2.0.0-next.16`; and
  `@solidjs/vite-plugin` `3.0.0-next.31`. Architecture qualification resolves
  the compiler's installed preset path and rejects mixed RC peers. The final
  lock contains neither RC.1 nor loopback registry URLs.
- 2026-08-20: Qualified Kobalte `2.0.0-alpha.0`, then removed it when its used
  primitives emitted Solid 2 development diagnostics. Replaced only the used
  menus, dialogs, tabs, fields, selects, switches, and toast surface with
  owned native primitives, including portaled collision-aware positioning,
  topmost modal focus containment, keyboard traversal, focus return, and
  pause/dismiss timing tests. This is the only implementation-time behavior
  decision; no visual redesign was intended.
- 2026-08-20: Migrated frontend, Canvas, and AI Chat to `@solidjs/web`, Solid
  2 JSX ownership, context-as-provider, draft-first stores, camel-case DOM
  events, object/array classes, `onSettled`, and compute/apply effects. Added
  lazy-route loading ownership, root/portal Escape containment, Canvas
  disposal cleanup, eager placement composition, and settled test helpers.
- 2026-08-20: Removed Solid devtools and `lucide-solid`. Canvas and AI Chat now
  ship bounded local SVG components derived from `lucide-static`; both public
  LICENSE files carry the required Lucide ISC and Feather/Cole Bemis MIT
  notices, and package verification asserts them.
- 2026-08-20: Fenced widget deletion planning/commit, Db metadata/inspection/
  rows/polling, Generic resource mutations, create-resource continuation, AI
  approval saving, chat preference snapshots, and secret reveal to captured
  route/open/request generations and disposal. Added deferred A-to-B,
  close/reopen, pending-unmount, owner-document, and zero-diagnostic
  regressions.
- 2026-08-20: Final public release markers are Canvas `0.10.0` and AI Chat
  `0.3.0`, propagated through both manifests, README compatibility, fixtures,
  the lockfile, and `public-package-set.json`. Their exact peers are
  `solid-js@2.0.0-rc.0` and `@solidjs/web@2.0.0-rc.0`.
- 2026-08-20: `bun run --cwd packages/canvas typecheck && bun run --cwd
  packages/canvas test && bun run --cwd packages/canvas build` passed: 29
  files / 184 tests. AI Chat typecheck, build, and all 17 files / 110 tests
  passed. Emitted public JS/declarations contain no `solid-js/web`,
  `solid-js/store`, old Solid JSX runtime, or `lucide-solid` reference.
- 2026-08-20: `bun run --cwd apps/frontend test` passed on the settled final
  tree: 131 Bun tests, 23 Vitest files / 114 tests, production build, Canvas
  critical graph 18 chunks / 426,026 gzip bytes under the 471,040 budget, and
  verified inspection hash
  `c998f6f2194603f00a326852a36d913d837cb0b5f14236d322130a0da3e3bd41`.
- 2026-08-20: Architecture/public qualification passed 82 tests / 816
  assertions. Frontend conformance passed 13 scenarios. Root workspace
  typechecks, source-run build stability, packed public composition, and the
  packed Canvas browser smoke passed. The real
  `bun run check:published-lockfile` gate confirmed public npm URLs.
- 2026-08-20: `bun run verify:package-dists` rebuilt all five public packages,
  performed npm dry-run and isolated packs, installed them cleanly, and passed
  Bun plus portable Node ESM import smoke for Canvas `0.10.0` and AI Chat
  `0.3.0` in the single Solid RC.0 graph.
- 2026-08-20: The final `bun run test:browser:live` passed all 16 route
  families, AI stream/history and preference reload, Preview function/KV
  execution, cold/restart recovery, failure usability, and WebSocket
  generation fencing with no uncaught error, bad response, or Solid dev
  diagnostic. Vite uses `--force` in this gate so mutable workspace package
  distributions cannot be hidden by an older dependency prebundle.
- 2026-08-20: Chrome plugin verification on an isolated home placed AI Chat by
  pointer drag; traversed action, approval, model, thinking, Canvas, and
  resource menus by keyboard; proved Escape focus return and Alt+T modal
  containment; created/read a KV value; opened Db Schema/Data; and persisted
  dark theme across reload. The fresh forced-bundle interval contained zero
  warning, error, or Solid diagnostic entries.
- 2026-08-20: A fresh critical reviewer found and drove fixes for lockfile
  registry leakage, request-generation races, third-party notices, menu/modal/
  toast accessibility and geometry, and compiler peer coherence. Its final
  bounded passes signed off the request snapshots, AnchoredMenu refs, and
  model-category focus fix with no remaining actionable finding.
