...
- [x] Expose `createPathInteractionController` from `CanvasEngineAdapter`
- [x] Construct, attach (before editor), subscribe, and destroy paths in `CanvasEditorBridge`
- [x] Compose `paths.state.cursor` into host cursor policy
- [x] Remove Shape1d box-handle `getTransformPolicy` override
- [x] Verify standard + product transform merge for single and multi path-like selection
- [x] Delete `line-point-edit` session and `fn.point-edit` overlay path
- [x] Preserve `beginConnector` creation and CRDT line/arrow commit
- [x] Map controller commits into Automerge points/bindings (and waypoints if needed)
- [x] Update `fn.shape1d` projection to round-trip edited connector geometry
- [x] Wire Escape / tool switch / remote cancel to `exitEditMode` or equivalent
- [-] Optional: `setSegmentMode` product UI for connector routing
- [x] Update bridge, transform, and Shape1d tests
- [x] Flip compatibility matrix `point-editing` ownership
- [x] Update docs that still call point editing canvas-owned
- [x] Run canvas test, tsc, and functional-core lint gates
...
- 2026-07-26: Mounted `PathInteractionController` in the manual editor bridge,
  mapped tagged geometry/transform commits into Automerge and product history,
  preserved endpoint rebinding through a thin semantic hit adapter, projected
  CRDT interior points as connector waypoints, and deleted the Shape1d
  point-edit session/transients. Added the six-CSS-pixel connector acquisition
  band to the custom semantic input route. Optional segment-mode UI remains
  intentionally unsurfaced; the controller API is available for a future UI.
  Canvas tests (379), TypeScript, and functional-core lint pass.
- 2026-07-26: Fixed default arrows projecting `startMarker: undefined` when
  `startCap` is `"none"`. Absent marker fields are now omitted, preserving
  JSON-serializable projection output; the real default-cap case is covered by
  the full projection test.
- 2026-07-26: Fixed line/arrow move and rotate commits retaining the
  controller-mutated connector child transform after the CRDT root transform
  was projected. The bridge now normalizes that child before publishing the
  durable commit, preventing the live scene from applying the transform twice.
  A measured 100×80 browser drag remains 100×80 after release; 381 canvas
  tests, TypeScript, and functional-core lint pass.
- 2026-07-26: Corrected transform persistence for non-uniformly scaled lines
  and arrows by baking the controller's complete child affine (including
  scale, skew, and origin) into connector points while preserving the root
  element transform. Generic product transform handles are now suppressed
  outside Select mode, so an active line/arrow creation tool cannot restore
  box handles after commit. Regression tests cover both cases; 388 canvas
  tests, TypeScript, and functional-core lint pass.
