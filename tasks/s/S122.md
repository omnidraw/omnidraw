...
- [x] Make the Bash capability contract explicitly host-authority.
- [x] Add exact chat cwd to capability invocation arguments.
- [x] Implement the short-lived Bun PTY Bash capability.
- [x] Bound and stream PTY output with exact exit metadata.
- [x] Forward timeout and cancellation and close every PTY.
- [x] Inject Bash capability into production `AgentService` composition.
- [x] Preserve authorization, mutation fencing, and mount-set repair.
- [x] Add real cwd, traversal, TTY, output, exit, timeout, and cancellation tests.
- [x] Add production-composition and compiled-binary smoke coverage.
- [x] Update prompt, architecture documentation, `FILES.md`, and task logs.
...
- 2026-07-29: Made the capability contract host-authority, forwarded the exact
  conversation cwd, and added the CLI-owned Bun implementation using one fresh
  terminal and `Bun.spawn([bash, "-lc", command])` per call.
- 2026-07-29: Added streaming UTF-8 decoding, bounded head/tail retention,
  coalesced live updates, explicit success/non-zero/signal/timeout/cancellation
  metadata, stable missing-Bash/spawn errors, and unconditional PTY cleanup.
- 2026-07-29: Wired one stateless capability into production tenant
  `AgentService` composition. Preserved per-call authorization, serialized
  before/after draft revision fencing, multi-draft fencing, failure-path
  fencing, and managed mount-set repair.
- 2026-07-29: Added real Bun PTY tests for exact cwd with spaces, unrestricted
  parent traversal, combined stdout/stderr, TTY detection, split UTF-8, exact
  exits/signals, timeout, cancellation, noisy-output bounds, ordered bounded
  updates, missing Bash, spawn failure, and terminal closure. Added an actual
  compiled-mode `setupServices` → `AgentService` registry smoke test.
- 2026-07-29: Updated prompts, package/architecture guidance, S85 history, and
  `FILES.md`. The package runs passed 93 service-agent and 180 CLI tests; their
  six and four sandbox-blocked loopback cases respectively passed in an
  approved local-network rerun. Also verified both affected typechecks,
  functional-core lint, architecture tests, focused production composition,
  and diff checks.
- 2026-07-29: Review follow-up now closes the PTY through its lifecycle callback
  before the final decoder flush, preserves output observed before timeout or
  cancellation, marks failed Bash results through the service-agent error
  details recognized by Pi, and rounds every accepted positive timeout up to at
  least one millisecond. Added focused regressions for all three cases and
  reverified all 284 service-agent and CLI tests, both affected typechecks,
  functional-core lint, architecture tests, and diff checks.
