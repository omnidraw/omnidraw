...
**Blocked on the published Cangine 0.4.0 release.**

Do not start implementation merely because matching code exists in a local
Cangine checkout, an unreleased branch, or a source commit. Unblock this task
...
The Cangine team targets this API for a future 0.4.0 release but has not
committed a release date. Do not put unrelated Cangine 0.3 work behind this
task. In particular, the headless style-menu migration, duplicate resize
observer removal, and Cangine grid background can proceed independently.
...
## Open questions required before unblocking

Send or resolve these questions against the final 0.4.0 documentation:

1. **Semantic no-op revision:** When `changes.length === 0`, does
   `ISceneStore.apply(commands)` advance `scene.revision`? In controlled editor
   mode, must the host still apply the exact batch and return a successor
   revision, or accept it without projection?
2. **Runtime-only scene changes:** If backgrounds, portals, or other
   runtime-only nodes share the retained scene and advance its revision, must
   the application reduction state contain and receive every runtime-only
   scene command, or may it track only authored document nodes while pairing
   itself with the full engine revision?

Do not guess either behavior. Record the upstream answer in `LOGS` and convert
it into explicit tests before checking off the release gate.
...
- [ ] Confirm the released Cangine 0.4.0 `/scene` API and shared-kernel claim.
- [ ] Resolve and log semantic no-op revision behavior.
- [ ] Resolve and log runtime-only node/revision-state behavior.
- [ ] Upgrade Cangine manifests and lockfile.
- [ ] Add the document-owned reduction-state/revision pair.
- [ ] Initialize/recreate state from the exact projected snapshot.
- [ ] Adapt bounded changes to durable planning, history, indexes, and events.
- [ ] Replace affected-ID equality with net-change subset admission.
- [ ] Make local commit state adoption atomic with scene projection.
- [ ] Keep the product node map as a mechanical derived read model if needed.
- [ ] Synchronize remote, runtime-only, reload, recovery, and replacement paths.
- [ ] Delete the local reducer implementation and its semantic test suite.
- [ ] Update `FILES.md`.
- [ ] Run boundary tests, typechecks, builds, architecture checks, and browser QA.
- [ ] Record exact code reduction and verification results.
...
- Replacement preserves full snapshot and 3D-change meaning.
