...
- [x] Mount direct Canvas routes without awaiting `canvas.list` and load catalog metadata in parallel.
- [x] Split the Canvas critical module graph from SDK/Capsule, AI Chat, Fontkit, and Three implementations.
- [x] Add a semantic chunk-graph gate and documented critical-byte budget.
- [x] Replace global boot success with monotonic surface/navigation/scene/document readiness.
- [x] Start engine acquisition and authoritative snapshot retrieval concurrently.
- [x] Present the Cangine grid and attach navigation before document/resource hydration.
- [x] Attach selection/editing immediately after one coherent snapshot is installed.
- [x] Replace all-font blocking preload with referenced-font progressive hydration and fallback repaint.
- [x] Start widget and AI Chat enhancement lanes only after the first interactive paint.
- [x] Isolate optional failures/cancellation from root boot recovery and RPC connection generation.
- [ ] Add ordering, stale-result, failure, teardown, heartbeat, and real-browser responsiveness coverage.
- [x] Update Canvas lifecycle documentation and screen-atlas loading/status descriptions.
- [x] Bump every affected public runtime package semantically and regenerate lockfile/release evidence.
...
- 2026-08-20: Mounted direct route identities independently of the catalog,
  introduced monotonic Canvas readiness, dispatched the snapshot before
  concurrent engine acquisition, and moved referenced fonts plus static and
  dynamically imported extensions into instance-scoped post-paint lanes.
- 2026-08-20: Split lightweight AI Chat frame metadata from its component and
  split frontend sidebar/image ports from the former eager adapter graph. The
  production semantic graph gate measured 415,634 gzip bytes across 16
  critical chunks and verified that SDK/Capsule, AI Chat implementation,
  Fontkit/Emscripten, and Three remain outside that graph.
- 2026-08-20: Verified Canvas typecheck and all 181 Canvas tests, component AI
  Chat typecheck and all 100 tests, frontend typecheck and production build.
  Bumped `@omnidraw/canvas` to 0.9.0 and `@omnidraw/component-ai-chat` to 0.2.0.
