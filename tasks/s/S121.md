...
- [ ] Pin and verify `@omnidraw/cangine@0.2.4`; add no Cangine patch.
- [ ] Add the pure optimistic-document serialized-command reducer.
- [ ] Split server-accepted item state from optimistic local node state.
- [ ] Implement `IEditorSceneMutationPort` with synchronous atomic projection.
- [ ] Route undo/redo through the same local-first mutation primitive.
- [ ] Install `sceneMutationPort` on the standard editor session.
- [ ] Remove recorder-driven persistence and the engine `record` requirement.
- [ ] Route selection-style and AI/widget extension mutations through
      `editor.commitSceneMutation`.
- [ ] Add a direct-scene-writer architecture guard.
- [ ] Add the explicit pending command/ack/remote/conflict ledger.
- [ ] Implement `IEditorImageImportPort` Blob adoption and resource ownership.
- [ ] Enable Cangine clipboard, drop, and file-picker image controllers.
- [ ] Add durable image asset metadata and background upload promotion.
- [ ] Remove duplicate Vibecanvas image preparation/orchestration.
- [ ] Cover local-first timing, rollback, history, ack echo, remote events,
      resync, pending image edits, upload success/failure, and reload.
- [ ] Run canvas, canvas-contract, service-canvas, ui-ai-chat, frontend
      typechecks/tests, functional-core lint, production build, and focused
      browser interaction verification.
- [ ] Record deleted code and final authority evidence in this task.
