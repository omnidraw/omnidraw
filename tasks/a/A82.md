...
- [x] Add safe structured extraction for successful widget-create result details.
- [x] Render **Open Preview** outside collapsed result content and wire direct user activation.
- [x] Resolve the latest draft summary at click time through the typed API port.
- [x] Register the internal draft Preview frame kind and draft-aware title.
- [x] Create/focus one persisted Preview frame per draft beside the originating chat.
- [x] Build and render revision-pinned draft sources through the sandbox bridge.
- [x] Bridge draft actor snapshots, messages, and Preview invalidation events.
- [x] Add stale, refresh, reset, loading, and actionable failure states.
- [x] Add revision-safe backend Preview disposal if the public lifecycle lacks it.
- [x] Clean up sandbox, streams, actors, and owned snapshots on replacement/removal.
- [x] Add tool-result, placement, deduplication, hydration, Preview, event, and cleanup regressions.
- [x] Run focused tests, typechecks, functional-core checks, and the frontend build.
...
- 2026-07-19: Implemented the trusted result action, persisted adjacent Preview frame, shared sandbox bootstrap, draft actor bridge, stale/recovery UI, and owner-scoped revision-safe Preview cleanup.
- 2026-07-19: Added regressions for trusted extraction, collapsed-card activation, placement, deduplication, hydration, actor messages, invalidations, stale requests, teardown, and multi-owner backend isolation.
- 2026-07-19: Live-tested the generated widget in the browser, including opening the companion frame and sending an actor message; captured the completed state for the screen atlas.
- 2026-07-19: Passed ai-chat (144), api-agent (3), focused service-agent Preview/management (29), four affected package typechecks, functional-core lint, `git diff --check`, and the frontend production build.
- 2026-07-19: Ran the repository-wide lane; four unrelated CLI download tests could not bind an ephemeral port in the workspace sandbox, while every A82-affected lane passed independently.
