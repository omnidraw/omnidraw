...
- the legacy adapter is for retained `0.9.4` artifacts only. It is not a
  second source API for new builds.
...
flowchart LR
  ...
  Legacy["Retained 0.9.4 artifact"] --> Adapter["Capsule legacy artifact adapter"]
  Adapter --> Preview
  Adapter --> Audit["Diagnostics: legacy = true"]
  Legacy -. "never reused as a new 0.10 build" .-> Builder
```
...
- Regenerate the lockfile and verify no production workspace resolves
  `0.9.4`. A deliberately retained legacy fixture may contain a frozen
  `0.9.4` artifact, but must not install or build through the old source API.
...
- Update the widget-contract schema, inferred types, examples, fixtures, and
  validation errors together. Choose and document either:
  - a new manifest schema version with an explicit migration; or
  - a one-time trusted normalization of persisted old draft source into the
    new public shape.
- Do not keep both `target` and `apis` as accepted inputs to the normal build
  path. Compatibility conversion, if required for editable persisted drafts,
  belongs at one migration boundary and must emit only the new source shape.
- Keep immutable published `0.9.4` artifacts immutable. They use Capsule's
  artifact adapter and are not rewritten into a synthetic `0.10` source
  request.
...
- Update construction identity, cache keys, integrity checks, serialization,
  and published revision descriptors so a `0.9.4` artifact cannot be mistaken
  for a rebuilt `0.10.0` artifact.
- Rebuild and re-sign all normal new Preview constructions. Do not relabel or
  hash-wrap an old artifact as if it were native `0.10.0`.
...
- Legacy `0.9.4` published artifacts may mount through Capsule's legacy
  artifact adapter when their exact target is supported. Mark diagnostics
  `legacy: true`; editing or republishing their source requires a native
  `0.10.0` rebuild through the new group API.
...
- Include `apiContract.requestedApis`, effective APIs, and `legacy` in stable
  diagnostic data. Keep `resolvedTarget` in a clearly private audit section.
...
- Retain one frozen `0.9.4` artifact fixture and prove:
  - it mounts only through the legacy artifact adapter;
  - diagnostics report `legacy: true`;
  - the normal source builder has no old-API branch;
  - it cannot be promoted as a newly constructed `0.10.0` revision.
...
- A retained `0.9.4` artifact has a tested, explicit legacy mount path, while
  new source construction has no dual old/new API.
- Cleanup and population tests prove no WebGL/GPU runtime leak across failure,
  retry, reset, publication, or destroy.

## TODOs

- [ ] Pin Capsule `0.10.0` across workspaces and regenerate the lockfile.
- [ ] Replace the source manifest's private target with public API groups and
      partial budgets.
- [ ] Migrate persisted editable source at one trusted boundary; keep old
      published artifacts immutable.
- [ ] Rewrite construction around `buildCapsuleGuest({ apis, budgets, policy })`.
- [ ] Persist and verify the signed API contract, digest, and construction
      identity.
- [ ] Rewrite host policy, mount, and GPU population accounting around public
      API groups.
- [ ] Preserve exact native `0.10.0` Preview-to-Publish promotion.
- [ ] Update agent scaffolds, prompts, validation, and diagnostics.
- [ ] Convert Three.js browser acceptance and rerun the capability probes.
- [ ] Add the frozen `0.9.4` legacy-artifact acceptance fixture.
- [ ] Run focused package, boundary, type, browser, and product workflow tests.
- [ ] Update consumer documentation and mark old profile-based guidance as
