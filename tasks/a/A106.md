...
- [x] Define the shared detailed UI-token and six-code canvas color contract.
- [x] Collapse built-in and custom theme registration into one atomic model.
- [x] Generate one coherent CSS-variable snapshot and migrate shell/canvas
      consumers away from raw palette variables and literals.
- [x] Add and validate the versioned semantic canvas color extension.
- [x] Add theme-aware runtime scene projection and deterministic export input.
- [x] Integrate the updated Cangine appearance, creation, and style-mutation
      extension points without rewriting mutation-port commands.
- [x] Replace the canvas menu with exactly six fill and five ink choices.
- [x] Add contract, collaboration, projection, export, accessibility, package,
      and four-theme browser coverage.
- [x] Update the verified selection-menu entry in `SCREENS.md`.
...
- 2026-08-01: Implementation started on Cangine 0.5.3. Added the state-free
  `@omnidraw/theme-contract` package first so theme registration and durable
  canvas validation can share the six-code vocabulary without reversing the
  service dependency direction. Cangine 0.5.4 hook wiring remains intentionally
  deferred until the release is available.
- 2026-08-01: Collapsed the split built-in registries into complete immutable
  ThemeService definitions, generated default CSS from that contract, migrated
  shell/canvas/terminal consumers, and removed silent unknown-theme fallback.
  Added strict semantic-extension fallback validation and separate authored
  versus viewer-theme runtime projection, including collaboration, pending
  reconciliation, literal stability, and deterministic explicit-theme export
  coverage. Package and architecture checks pass on Cangine 0.5.3.
- 2026-08-01: Planning assumes a forthcoming Cangine release with live editor
  ...
  mutation decoration.
- 2026-08-01: Upgraded every active consumer and release assertion to the
  immutable local-registry `@omnidraw/cangine@0.6.0` artifact. Wired live
  selection/path appearance setters, the guarded seven-kind creation
  decorator, and the extension-only selection mutation decorator. Cangine
  continues to plan and dispatch the exact concrete batch; Omnidraw adds or
  removes only its bounded semantic extension inside those supported hooks.
- 2026-08-01: Verified Light, Dark, Sepia, and Graphite in the mounted app,
  including exact picker labels/order, transparent treatment, visible keyboard
  focus, pressed state, a semantic color edit followed by undo, theme-owned
  shell/canvas/terminal variables, and zero browser warnings/errors. Replaced
  the screen-atlas capture with the compact six-fill/five-ink menu.
