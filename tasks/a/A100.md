...
- [x] Define the versioned trace envelope, channels, environment header, and
      correlation IDs.
- [x] Implement the inert-by-default bounded trace owner and injected sink.
- [x] Add development-only composition and toolbar controls.
- [x] Instrument DOM/engine input, picking, transforms, and editor transitions.
- [x] Instrument bounded document transaction, projection, acknowledgement,
      and recovery phases.
- [x] Instrument typed CanvasService transport without global API interception.
- [x] Instrument Vibecanvas-owned widget portal and interaction boundaries.
- [x] Implement deterministic coalescing, ring retention, priority budgets,
      and omission accounting.
- [x] Implement bounded redaction and safe error normalization.
- [x] Implement Mark Failure, anomaly candidates, paste-ready copy, and JSONL
      download.
- [x] Add pure, component, document-service, integration, lifecycle, and
      production-gating coverage.
- [x] Measure synchronous-path overhead and representative trace size.
- [x] Add internal trace documentation and update `FILES.md`.
...
- 2026-07-30: Implemented the schema-v1 bounded collector, deterministic
  compaction/redaction/anomaly pipeline, 128 KiB Markdown copy, 2 MiB JSONL
  download, and explicit development-only toolbar composition.
- 2026-07-30: Injected one trace sink through Cangine input/transform/editor,
  CanvasDocumentService, typed CanvasService transport, and the AI widget-host
  extension. Document observer failures are isolated and payloads contain only
  already-available affected IDs, counts, revisions, and normalized failures.
- 2026-07-30: Added pure, owner, transport, toolbar, document-order/isolation,
  and production-gating coverage. The canvas suite passes 95 tests before final
  repository-wide verification and 96 after lifecycle cleanup coverage.
- 2026-07-30: Measured 2,000 one-node reductions at 100 vs 10,000 total nodes:
  tracing absent measured 10.52 vs 7.27 µs/iteration; tracing active measured
  11.38 vs 10.87 µs/iteration. This supports cost scaling with emitted bounded
  facts rather than total canvas size; exact numbers are machine-dependent.
- 2026-07-30: Reduced Smart mode to causal transitions: passive motion, hover
  crossings, raw drag frames, transform updates, empty observations, and trace
  lifecycle rows are discarded before retention. Copy for Agent now uses terse
  causal rows without per-event schema or retention priority; the fuller JSONL
  keeps one schema version in its header. Advanced remains the explicit sampled
  raw-channel path, and recorder-toolbar interactions are excluded.
- 2026-07-30: Added gesture-level agent reduction with local ID aliases,
  rounded coordinates/durations, successful document/transport chain folding,
  normal capture suppression, and mark-to-latest-gesture correlation. The
  reported 64-fact reproduction compresses to four causal rows and under 1 KiB
  while surfacing the marked click with no transform as a possible
  anomaly.
