...
- [x] Add the `•••` Preview dropdown with Reload, Rebuild, Publish, and Remove
- [x] Route typed Cangine dropdown-item activations to Preview owners
- [x] Expose race-safe explicit Preview `reload()` and `rebuild()` operations
- [x] Wire digest-fenced Build and Publish with progress and notifications
- [x] Share durable frame removal between Remove and the red close control
- [x] Add descriptor, runtime, publication, removal, cleanup, and keyboard tests
- [x] Run affected typecheck, tests, functional-core, and architecture checks
- [x] Capture the finished menu and update the screen atlas
...
- 2026-08-08: Implemented the declarative Preview actions descriptor, typed
  action routing, serialized Reload/Rebuild ownership, digest-fenced Build and
  Publish, and shared durable removal. Added focused descriptor, owner, action,
  publication, cleanup, repeated-activation, stopped-session, and keyboard
  tests. Captured the completed menu in the screen atlas. Package typecheck,
  all 206 UI tests, and functional-core rules pass. The architecture run passes
  27 of 28 checks; its existing canvas-kernel release assertion expects
  `@omnidraw/canvas@0.6.1` while the unchanged workspace manifest is `0.6.0`.
- 2026-08-08: Follow-up aligned Preview chrome with its sidebar draft identity.
  Preview titles now use the `Preview: ` prefix, and existing plus newly placed
  frames track the active theme's exact warning color used by the sidebar.
  Verified the match in light and dark themes, all 207 UI tests, all 41
  frontend tests, both affected type/build checks, and functional-core rules.
- 2026-08-08: Review fixes made custom CSS warning colors non-fatal, moved live
  Preview chrome updates onto the canvas projection path instead of durable
  scene mutations, and made stopped/removal teardown release runtime handles
  and close sessions promptly even across in-flight operations. All 210 UI
  tests, UI typecheck, 42 frontend tests, frontend build, and functional-core
  rules pass.
