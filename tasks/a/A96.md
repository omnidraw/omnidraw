...
**Status: complete.** The durable Preview loop and exact promotion path are
implemented. Integration with Capsule's forthcoming public authoring-location
API and reference-machine latency qualification moved to [A99](./A99.md).
...
- [-] Add verified Preview runtime source-location diagnostics here; moved to
  [A99](./A99.md) while Capsule updates its public authoring API.
...
- [-] Measure the local Preview and Publish latency budgets here; moved to
  [A99](./A99.md).
...
retained-construction publication are implemented. A99 owns the remaining
upstream API integration and final qualification rather than keeping this
merged implementation task open.
...
- S133 supersedes A96's product rule that Publish always promotes the already
  mounted exact-ready selection and therefore never builds. Exact retained
  promotion is still the fast path and runs no guest build; when source or
  authorized bindings moved, Publish now enters the same Preview build path
  before promoting its server-selected exact construction.
...
- 2026-07-28: Left local latency targets unclaimed pending reference-machine
  measurement.
- 2026-07-28: B67's isolated headless counter scenario measured the path shape
  without claiming reference-machine p95: cold trusted validation was
  13.9–17.7 s with an explicit dependency install; unchanged validation was
  19–24 ms; a warm source-only rebuild was 442–456 ms with no install; and the
  following headless Preview cache hit was 279–293 ms. A declared dependency
  edit explicitly installed again. Exact Preview promotion remains covered by
  the production service call-count regression and performs no guest rebuild.
...
- 2026-07-29: Closed A96 after confirming its implementation is merged through
  `a344095b`. Moved the Capsule public authoring-location integration and
  reference-machine latency qualification to A99.
