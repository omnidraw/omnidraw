...
- [x] Add cold multi-widget lifecycle and cache instrumentation.
- [x] Expose authoritative portal visibility through the Canvas widget-host seam.
- [x] Add a bounded visible-first mount queue with cancellation and stable ordering.
- [x] Defer cold off-screen startup and throttle already-mounted off-screen guests.
- [x] Replace hard-coded scheduling fields with host-owned visibility projections.
- [x] Share one bounded artifact cache across isolated hosts owned by one SDK browser host.
- [x] Deduplicate concurrent exact-key cache misses without sharing guest authority.
- [x] Add deterministic unit, integration, and real-browser performance coverage.
- [x] Bump affected public package versions and regenerate release/lockfile evidence.
...
- 2026-08-21: Reimplemented A124 on the Solid 2 base. Canvas now publishes
  authoritative renderer-neutral scheduling, the frontend owns a
  concurrency-three visible-first queue with active-start preemption and
  mounted guest throttle/resume, and viewport metadata no longer uses fixed
  scheduling values.
- 2026-08-21: Moved Capsule's bounded artifact cache to SDK browser-host
  lifetime, partitioned admission by exact bytes and trust policy (including
  signing-key material), deduplicated concurrent admissions, retained one host
  per guest, and made cancellation, eviction fallback, and host disposal own
  all in-flight authority.
- 2026-08-21: Added deterministic scheduling/admission tests and a real Chromium
  gate covering cold off-screen deferral, bounded visible startup, stable guest
  identity across throttle/resume, repeated-artifact reuse, and isolated guest
  instances. Bumped Canvas to 0.11.0 and SDK to the next unpublished version,
  0.14.0, and regenerated package-set/lockfile evidence.
