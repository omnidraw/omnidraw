...
- [x] Define a bounded, ordered Preview log-entry projection.
- [x] Route lifecycle, build, revision/binding, and diagnostic state to it.
- [x] Add a bounded Capsule/host path for guest runtime output if supported.
- [x] Dock the host-owned terminal below guest content.
- [x] Preserve diagnostic resolution and last-known-good behavior.
- [x] Add pure and Preview integration tests.
- [x] Update the Preview screen-atlas capture and description.
...
- 2026-07-29: Created from browser comment 1 after inspecting the current
  ...
  atlas.
- 2026-07-29: Added the pure bounded log projection, docked keyboard-accessible
  host terminal, clear and diagnostic-resolution chrome, structured Capsule
  diagnostic routing, and sibling-boundary coverage. Existing structured
  `onDiagnostic`/`onFatal` delivery remains the only supported bounded guest
  runtime-output path; no console, shell, PTY, props, or collaborative-state
  authority was added.
- 2026-07-29: Verified the branch frontend against the running local backend
  and refreshed the AI draft Preview atlas capture and description.
- 2026-07-29: Fixed review findings by projecting Capsule viewport dimensions
  from the measured guest lane, bounding diagnostic dedupe state to active
  retained records, logging updated diagnostic occurrences, and preserving
  scrollback position when the operator is not following the tail.
