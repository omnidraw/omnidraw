...
- [ ] Pin and publicly qualify exact `@omnidraw/cangine@0.7.0` everywhere.
- [ ] Update `llm.cangine.md`, release guidance, diagnostics, and package assertions to 0.7.0.
- [ ] Add target-relative endpoint attachments and `auto` anchors to Canvas Contract.
- [ ] Add validated orthogonal `fixedSegments` and their waypoint exclusion.
- [ ] Keep Canvas/Cangine serialization lossless with no transient-state leakage.
- [ ] Enable group-aware tail/head binding during Line and Arrow creation.
- [ ] Enable tail/head rebind and detach in Select/path-edit mode.
- [ ] Preserve inside/orbit and Alt/Ctrl/Cmd interaction semantics.
- [ ] Preserve markers, routing modes, target transforms, history, reload, and remote reprojection.
- [ ] Cover target deletion with deterministic unresolved-reference fallback.
- [ ] Add contract, adapter, editor, integration, and real-browser acceptance.
- [ ] Apply required public package version bumps and regenerate package-set/lockfile evidence.
- [ ] Run the complete package, browser, distribution, and final acceptance gates.
