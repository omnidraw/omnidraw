...
- [x] Add the source- and health-aware Preview replacement action.
- [x] Add the pure Preview-to-published node projection with a fresh instance ID.
- [x] Require current-publication placement admission for the Canvas transition.
- [x] Fence replacement against catalog/publication drift.
- [x] Remount the portal when runtime mode or instance identity changes.
- [x] Close Preview ownership only after authoritative Canvas acceptance.
- [x] Add failure, undo/redo, restart, and later-publication-following tests.
- [x] Update the Canvas Preview actions section of the screen atlas.
...
- 2026-08-20: Implemented a catalog-generation and publication-identity
  reservation tied to the fresh instance ID. Canvas authority rechecks it
  under the publication read barrier before accepting the one-node upsert.
- 2026-08-20: Added the pure codec-tested projection, health-aware native
  Preview menu action, unpublished-draft confirmation, acceptance-aware
  success/failure feedback, and post-accept Preview session retirement.
- 2026-08-20: Added authority drift/undo/redo coverage and Canvas portal tests
  proving optimistic replacement retains last-good Preview content while an
  accepted runtime-mode or instance-identity change remounts.
- 2026-08-20: Review hardening made catalog-driven Preview chrome persistence
  non-historical, restricted Preview restoration to an accepted exact
  replacement lineage, admitted redo against the then-current healthy
  publication, and serialized/retried Preview-session closure before the
  published mount proceeds.
- 2026-08-20: Verified backend, frontend, and Canvas typechecks; full Canvas
  and frontend test/build suites; all 782 sandbox-safe backend tests; the 18
  loopback/process/network tests outside the sandbox; and public-package
  packaging/architecture checks.
