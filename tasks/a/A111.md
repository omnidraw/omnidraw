...
- [ ] Rename manifest v4 → v1 in `widget-contract` (schema, typed, constants, digest rules) and update all importers
- [ ] Rewrite the agent manifest prompt and `od_widget_create` description to manifest v1
- [ ] Delete legacy v3/v4 residue: dead event types, `fn.bump-widget-version`, docs, fixtures, and task wording
- [ ] Add repository checks forbidding manifest v3/v4 and migration readers
- [ ] Point `WidgetWorkspace` at the shared drafts root and scaffold `od_widget_create` into `drafts/<slug>/`
- [ ] Resolve chat mounts by manifest slug so display-name paths keep working
- [ ] Prove a chat-created draft appears in the catalog, sidebar, Preview, and Publish
- [ ] Group sidebar rows per widget key: published `+ Add`, draft `+ Preview`, preview hidden when published and no different draft
- [ ] Auto-build the Preview on first placement (`load` NOT_FOUND → `open`)
- [ ] Add an Open Preview action to the widget-create result
- [ ] Report real preview execution from `od_widget_validate`
- [ ] Run catalog, preview, sidebar, and agent tool tests; update docs and `FILES.md`
