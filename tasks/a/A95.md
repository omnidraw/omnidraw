...
- removed `packages/service-agent/src/workspace/tx.materialize-sdk-package.ts`
...
- [x] Add the isolated, pinned Verdaccio bootstrap and lifecycle commands
- [x] Add stable user-level storage, health, ownership, and scoped npm config
- [x] Add verified local-publish commands to Cangine and Capsule
- [x] Publish the Vibecanvas SDK and required contract dependency closure
- [x] Replace Cangine and Capsule absolute `file:` dependencies with versions
- [x] Replace generated-widget SDK/Capsule links with exact versions
- [x] Inject the registry config into draft and distribution npm commands
- [x] Remove `preserveSymlinks` and `.vibecanvas-links` staging
- [x] Add immutable-version and package-integrity checks
- [x] Verify bootstrap, restart, external worktree, and `.vibecanvas` publish
- [x] Run package, typecheck, build, and packed-consumer gates
...
- The canonical registry state is host-owned at
  `~/.local/share/vibecanvas/registry`. The generated npm user config contains a
  loopback-only development token because npm refuses to issue `publish` without
  credentials even when Verdaccio grants `$all`; Verdaccio remains bound to
  `127.0.0.1`.
- Cangine owns `package:publish-local` as of producer commits `668a359` and
  `59ef987`. Capsule owns `package:build`, `package:pack`, `package:verify`, and
  `package:publish:local` as of producer commit `cd5e6b9`.
- The producer commands intentionally reject publishing newly built bytes under
  an occupied version when their sha512 differs. The accepted `0.2.3` and
  `0.9.4` registry artifacts remain immutable and installable; changed producer
  content requires a version bump or development prerelease.
...
- 2026-07-28: Created from the decision to replace sibling `file:` dependencies
  ...
  publishing.
- 2026-07-28: Added pinned Verdaccio 6.9.0 lifecycle/bootstrap tooling, stable
  host storage and npm config, safe PID birth-time ownership checks, immutable
  integrity-aware tarball publication, and SDK dependency-closure publication.
- 2026-07-28: Converted Cangine/Capsule consumers and generated widgets to exact
  versions; removed SDK materialization, `preserveSymlinks`, and local dependency
  staging; injected `--userconfig` into draft and frozen distribution installs.
- 2026-07-28: Bootstrapped and restarted the canonical registry, repeated
  identical publication as an idempotent no-op, regenerated `bun.lock`, created
  and validated a registry-native widget below an isolated `.vibecanvas`, and
  passed frozen install, typecheck, focused tests, full single-target build, and
  packed-public-composition in an external worktree.
- 2026-07-28: Closed the cross-repository boundary after the producer-owned
  Cangine D2 and Capsule A16 commands were merged into their `main` branches.
  Rechecked both producer decision suites (3 Cangine and 7 Capsule tests), the
  live registry metadata for `@omnidraw/cangine@0.2.3` and
  `@omnidraw/capsule@0.9.4`, a frozen 932-package Vibecanvas install, the
  manifest/lock path audit, and 17 focused widget/distribution tests.
