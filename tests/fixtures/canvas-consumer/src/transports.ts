...
function timestampSec(offset: number): string {
...
export function createInMemoryCanvasTransport(args: TTransportArgs): TCanvasTransportHarness {
  ...
  let items = new Map<string, TCanvasItemSnapshot>(
    (args.initialItems ?? []).map((item) => [
      ...
      Object.freeze({
        ...
        item: clone(item),
        itemRevision: 1,
        createdAtSec: timestampSec(clockSec),
        updatedAtSec: timestampSec(clockSec),
      }),
    ]),
  )
  ...
  const transport: TCanvasDocumentTransport = Object.freeze({
    ...
    async query(request) {
      if (request.canvasId !== args.canvasId) throw new Error('Unknown canvas.')
      const ordered = [...items.values()].sort((left, right) => left.id.localeCompare(right.id))
      const filtered = ordered.filter((snapshot) => {
        const filter = request.filter
        if (filter.type === 'all') return true
        if (filter.type === 'ids') return filter.ids.includes(snapshot.id)
        if (filter.type === 'kind') return snapshot.item.kind === filter.kind
        if (filter.type === 'parent') return snapshot.item.parentId === filter.parentId
        const widget = fnReadCanvasWidgetExtension(snapshot.item)
        if (widget?.type !== 'widget-instance' && widget?.type !== 'widget-preview') return false
        if (filter.type === 'widget-instance') return widget.instanceId === filter.instanceId
        return widget.widgetKey === filter.widgetKey
      })
      const afterId = request.cursor?.id
      const offset = afterId === undefined
        ? 0
        : Math.max(0, filtered.findIndex((entry) => entry.id === afterId) + 1)
      const limit = request.limit ?? 100
      const page = filtered.slice(offset, offset + limit).map(clone)
      const last = page.at(-1)
      return Object.freeze({
        items: page,
        nextCursor: offset + page.length < filtered.length && last !== undefined
          ? Object.freeze({ type: 'id' as const, id: last.id })
          : null,
      })
    },
    ...
    async execute(command): Promise<TCanvasItemsChangedEvent> {
      ...
      for (const changedId of changedIds) {
        ...
        const snapshot = Object.freeze({
          ...
          createdAtSec: previous?.createdAtSec ?? timestampSec(clockSec),
          updatedAtSec: timestampSec(clockSec),
...
type TFakeCellRequest =
  ...
  | Readonly<{ method: 'canvas.query'; query: TCanvasItemQuery }>
...
type TFakeCellClient = Readonly<{
  request(request: TFakeCellRequest): Promise<unknown>
...
export function createFakeCellCanvasTransport(
  ...
  const client: TFakeCellClient = Object.freeze({
    async request(request) {
      ...
      if (wireRequest.method === 'canvas.query') {
        return memory.transport.query(wireRequest.query)
      }
  ...
  const transport: TCanvasDocumentTransport = Object.freeze({
    ...
    async query(request) {
      return await client.request({
        method: 'canvas.query',
        query: request,
      }) as Awaited<ReturnType<TCanvasDocumentTransport['query']>>
    },
